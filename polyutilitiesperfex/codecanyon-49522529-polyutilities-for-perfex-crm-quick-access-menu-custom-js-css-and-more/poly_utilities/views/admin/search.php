...
/**
 * TODO: note to check and handle again when core PerfexCRM layout changes
 */
?>
    ...
    foreach($result as $data){
       ...
       <?php foreach($data['result'] as $_result){
        ...
        switch($data['type']){
            ...
            $custom_value = '';
            if (!empty($_result['value'])) {
                $custom_value = '<div class="poly_customize_search_value">'. _l($_result['value']) . '</div>';
            }
            $output      = '<a class="pull-left" href="' . $rel_values['link'] . '">' . e($rel_values['name']) .'<span class="pull-right">'._l($_result['fieldto']).'</span>'.$custom_value.'</a>';
            break;
            case 'invoice_items':
