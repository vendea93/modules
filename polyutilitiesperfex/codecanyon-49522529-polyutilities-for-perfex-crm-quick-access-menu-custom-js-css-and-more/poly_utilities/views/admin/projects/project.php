...
?>
                            ...
                            <?php 
                            // Task Template Field - Only show when creating new project
                            if (!isset($project)) {
                                $this->load->model('poly_utilities/task_templates_model');
                                $task_templates = $this->task_templates_model->get_active_templates_for_dropdown();
                                if (!empty($task_templates)): 
                            ?>
                            <div class="form-group">
                                <label for="task_template_id" class="control-label">
                                    <i class="fa fa-tasks tw-mr-1"></i><?= _l('poly_utilities_task_template'); ?>
                                </label>
                                <div class="tw-flex tw-items-center tw-gap-2">
                                    <div class="tw-flex-1">
                                        <select name="task_template_id" id="task_template_id" class="form-control selectpicker" data-live-search="true" data-width="100%">
                                            <?php foreach ($task_templates as $template_id => $template_name): ?>
                                                <option value="<?= $template_id; ?>"><?= htmlspecialchars($template_name); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <a href="<?= admin_url('poly_utilities/task_templates'); ?>" 
                                       id="manage-template-btn"
                                       target="_blank" 
                                       class="btn btn-default tw-whitespace-nowrap" 
                                       title="<?= _l('poly_utilities_manage_task_templates'); ?>">
                                        <i class="fa fa-cog tw-mr-1"></i><?= _l('poly_utilities_manage_templates'); ?>
                                    </a>
                                </div>
                                <small class="form-text text-muted"><?= _l('poly_utilities_task_template_help_text'); ?></small>
                            </div>
                            <?php 
                                endif;
                            }
...
<script>
    ...
    $(function() {
        ...
        // Task Template Management Button
        var $select = $('#task_template_id');
        var $manageBtn = $('#manage-template-btn');
        if ($select.length && $manageBtn.length) {
            var baseUrl = '<?= admin_url('poly_utilities/task_templates'); ?>';
            
            // Function to update manage button URL
            function updateManageButtonUrl() {
                var selectedValue = $select.val();
                if (selectedValue && selectedValue !== '' && selectedValue !== null) {
                    $manageBtn.attr('href', baseUrl + '/template/' + selectedValue);
                } else {
                    $manageBtn.attr('href', baseUrl);
                }
            }
            
            // Update URL on change
            $select.on('changed.bs.select', function() {
                updateManageButtonUrl();
            });
            
            // Remove duplicate "Nothing selected" option that selectpicker adds
            $select.on('loaded.bs.select', function() {
                var $menu = $select.next('.bootstrap-select').find('.dropdown-menu');
                var $options = $menu.find('li');
                var foundFirst = false;
                $options.each(function() {
                    var $li = $(this);
                    var text = $li.text().trim();
                    if (text === '<?= _l('dropdown_non_selected_tex'); ?>' || text === 'Nothing selected') {
                        if (foundFirst) {
                            $li.remove();
                        } else {
                            foundFirst = true;
                ...
                // Update button URL after selectpicker is loaded
                updateManageButtonUrl();
            });
            
            // Initial update (fallback if selectpicker doesn't fire loaded event)
            setTimeout(function() {
                updateManageButtonUrl();
            }, 100);
        }
