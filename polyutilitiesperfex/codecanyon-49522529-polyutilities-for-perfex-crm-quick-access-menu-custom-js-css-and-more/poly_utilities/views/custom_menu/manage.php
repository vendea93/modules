                            ...
                            <div class="panel-body tw-pb-0">
                                <div id="shared-lists" class="row poly-sidebar-menu">
                                    <div class="col-md-6 poly-menu">
                                        <h4 class="col-12"><?php echo _l('poly_utilities_controls_header_active_sidebar_menu')?></h4>
                                        <div id="poly-active-menu" class="list-group col nested-sortable">
                                            ...
                                            <template v-for="(item, parent_index) in menu_items" :key="item.id">
                                                ...
                                                <div :style="handleDividerStyles(item)" v-if="item.slug && !item.slug.includes('root')" :class="['list-group-item', `nested-${parent_index}`]" :data-id="item.id || item.db_id" :data-slug="item.slug" :data-icon="item.svg ? item.svg : item.icon" :data-badge="JSON.stringify(item.badge)" :data-css="item.css" :data-href_original="item.href_original" :data-href="item.href" :data-target="item.target" :data-rel="item.rel" :data-type="item.type" :data-roles="item.roles" :data-users="item.users" :data-is_custom="item.is_custom" :data-name="handleHtmlContent(item.name)" :data-parent_id="item.parent_id" :data-parent_slug="item.parent_slug" :data-disabled="item.disabled" :data-level="0" :data-popup_description="item.popup_description">
                                                        ...
                                                        <template v-for="(item_child, sub_index) in item.children" :key="item_child.id">
                                                            <div :style="handleDividerStyles(item_child)" v-if="item_child.slug && !item_child.slug.includes('_add')" :class="['list-group-item sub',`nested-${sub_index}`]" :data-id="item_child.id || item_child.db_id" :data-slug="item_child.slug" :data-type="item_child.type" :data-roles="item_child.roles" :data-users="item_child.users" :data-is_custom="item_child.is_custom" :data-name="handleHtmlContent(item_child.name)" :data-href_original="item_child.href_original" :data-css="item_child.css" :data-href="item_child.href" :data-target="item_child.target" :data-rel="item_child.rel" :data-icon="item_child.svg ? item_child.svg : item_child.icon" :data-badge="JSON.stringify(item_child.badge)" :data-parent_id="item_child.parent_id" :data-parent_slug="item_child.parent_slug" :data-disabled="item_child.disabled" :data-level="1" :data-popup_description="item_child.popup_description">
                                                                ...
                                                                <span class="poly-cursor poly-menu-item-disabled relative pull-right" :data-id="item_child.id || item_child.db_id">
                                                                    <div class="onoffswitch">
                                                                        <input type="checkbox" :id="'children_checkbox-'+ parent_index + '_' + sub_index" class="onoffswitch-checkbox" :checked="item_child.disabled === 'true'">
                                                                    ...
                                                                    <template v-for="(item_child2, sub2_index) in item_child.children" :key="item_child2.id">
                                                                        <div :style="handleDividerStyles(item_child2)" v-if="item_child2.slug && !item_child2.slug.includes('_add')" :class="['list-group-item sub sub2',`nested-${sub2_index}`]" :data-id="item_child2.id || item_child2.db_id" :data-slug="item_child2.slug" :data-type="item_child2.type" :data-roles="item_child2.roles" :data-users="item_child2.users" :data-is_custom="item_child2.is_custom" :data-name="handleHtmlContent(item_child2.name)" :data-href_original="item_child2.href_original" :data-css="item_child2.css" :data-href="item_child2.href" :data-target="item_child2.target" :data-rel="item_child2.rel" :data-icon="item_child2.svg ? item_child2.svg : item_child2.icon" :data-badge="JSON.stringify(item_child2.badge)" :data-parent_id="item_child2.parent_id" :data-parent_slug="item_child2.parent_slug" :data-disabled="item_child2.disabled" :data-level="2" :data-popup_description="item_child2.popup_description">
                                                                            ...
                                                                            <span class="poly-menu-block">
                                                                                ...
                                                                                <span><a class="custom-menu-text" v-if="item_child2.type!=='popup' && item_child2.href && item_child2.href!=='#'" :href="item_child2.href" :slug="item_child2.href" v-html="handleHtmlContent(item_child2.name)" :style="item_child2.css"></a>
                                                                                    <span class="custom-menu-text" v-if="item_child2.type==='popup' || !item_child2.href || item_child2.href==='#'" v-html="handleHtmlContent(item_child2.name)" :style="item_child2.css"></span>
                                                                                    <span v-if="item_child2.href==''">&nbsp;(<span>Root</span>)</span><span v-if="item_child2.badge" :style="'background-color:'+item_child2.badge.color" class="tw-ml-2 badge bg-info">{{item_child2.badge.value}}</span></span>
                                                                            </span>

                                                                            <span class="poly-cursor poly-menu-item-disabled relative pull-right" :data-id="item_child2.id || item_child2.db_id">
                                                                                <div class="onoffswitch">
                                                                            ...
                                                                            <span @click.stop="handleDelete(item_child2)" :data-id="item_child2.id || item_child2.db_id" v-if="item_child2.is_custom=='true'" class="poly-cursor tw-mr-1 text-muted pull-right"><i class="fas fa-trash"></i></span>
                                    ...
                                    <div class="col-md-6">
                                        <h4 class="col-12"><?php echo _l('poly_utilities_controls_header_custom_sidebar_menu')?></h4>
                                        <div id="poly-custom-menu" class="list-group col">
                                            ...
                                            <div v-for="item in custom_menu_items" class="list-group-item">
                                                ...
                                                <div class="tw-mt-1"><i class="fa-solid fa-unlock fa-fw"></i> <?php echo _l('poly_utilities_controls_custom_menu_label_roles')?>: <span class="poly-label label label-danger tw-ml-1 tw-mr-1" v-if="item.roles && item.roles.length==0"><?php echo _l('poly_utilities_custom_menu_admin_allow_all_access') ?></span><span v-for="role in item.roles"><span class="poly-label label label-danger tw-ml-1 tw-mr-1" @click.stop="handleRoleInfo(role)">{{role.text}}</span></span></div>
                                                <div class="tw-mt-1"><i class="fa-solid fa-unlock fa-fw"></i> <?php echo _l('poly_utilities_controls_custom_menu_label_users')?>: <span class="poly-label label label-info tw-ml-1 tw-mr-1" v-if="item.users && item.users.length==0"><?php echo _l('poly_utilities_custom_menu_admin_allow_all_access') ?></span>
