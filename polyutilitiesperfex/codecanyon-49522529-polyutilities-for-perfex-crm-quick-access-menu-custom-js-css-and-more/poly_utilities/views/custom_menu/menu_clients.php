                            ...
                            <div class="panel-body tw-pb-0">
                                <div id="shared-lists" class="row poly-clients-menu">
                                    <div class="col-md-6 poly-menu">
                                        <h4 class="col-12"><?php echo _l('poly_utilities_controls_header_active_clients_menu')?></h4>
                                            ...
                                            <template v-for="(item, parent_index) in menu_items" :key="item.slug">
                                                <div :style="handleDividerStyles(item)" v-if="item.slug && !item.slug.includes('root')" :class="['list-group-item', `nested-${parent_index}`]" :data-id="item.slug" :data-icon="item.svg ? item.svg : item.icon" :data-badge="JSON.stringify(item.badge)" :data-css="item.css" :data-href_original="item.href_original" :data-href="item.href" :data-target="item.target" :data-rel="item.rel" :data-type="item.type" :data-roles="item.roles" :data-clients="item.clients" :data-require_login="item.require_login" :data-is_custom="item.is_custom" :data-name="item.name" :data-slug="item.slug" :data-parent_slug="item.parent_slug">
                                                    ...
                                                    <span class="poly-menu-block">
                                                        ...
                                                        <span>
                                                            <a class="custom-menu-text" v-if="item.type!=='divider' && item.type!=='popup' && item.href && item.href!=='#'" :href="item.href" :slug="item.href" v-html="item.name" :style="item.css"></a>
                                                            <span class="custom-menu-text" v-if="item.type==='divider' || item.type==='popup' || !item.href || item.href==='#'" v-html="item.name" :style="item.css"></span>
                                                            <span v-if="item.badge" :style="'background-color:'+item.badge.color" class="tw-ml-2 badge bg-info">{{item.badge.value}}</span>
                                                        </span>
                                                    </span>

                                                    <span class="poly-cursor poly-menu-item-disabled relative pull-right" :data-id="item.id || item.db_id">
                                                        <div class="onoffswitch">
                                                            <input type="checkbox" :id="'parent_checkbox-' + parent_index" class="onoffswitch-checkbox" :checked="item.disabled === 'true'">
                                                    ...
                                                    <!-- Submenu container area (Level 2) -->
                                                        ...
                                                        <template v-for="(item_child, sub_index) in item.children" :key="item_child.slug">
                                                            <div :style="handleDividerStyles(item_child)" v-if="item_child.slug && !item_child.slug.includes('_add')" :class="['list-group-item sub2',`nested-${parent_index}`]" :data-id="item_child.slug" :data-type="item_child.type" :data-css="item_child.css" :data-roles="item_child.roles" :data-clients="item_child.clients" :data-is_custom="item_child.is_custom" :data-name="item_child.name" :data-href_original="item_child.href_original" :data-href="item_child.href" :data-target="item_child.target" :data-rel="item_child.rel" :data-icon="item_child.svg ? item_child.svg : item_child.icon" :data-badge="JSON.stringify(item_child.badge)" :data-slug="item_child.slug" :data-parent_slug="item_child.parent_slug" :data-require_login="item_child.require_login">
                                                                ...
                                                                <span class="poly-menu-block">
                                                                    ...
                                                                    <span>
                                                                        <a v-if="item_child.type!=='popup' && item_child.href && item_child.href!=='#'" :href="item_child.href" :slug="item_child.href" v-html="item_child.name" :style="item_child.css"></a>
                                                                        <span v-if="item_child.type==='popup' || !item_child.href || item_child.href==='#'" v-html="item_child.name" :style="item_child.css"></span>
                                                                        <span v-if="item_child.badge" :style="'background-color:'+item_child.badge.color" class="tw-ml-2 badge bg-info">{{item_child.badge.value}}</span>
                                                                    </span>
                                                                </span>

                                                                <span class="poly-cursor poly-menu-item-disabled relative pull-right" :data-id="item_child.id || item_child.db_id">
                                                                    <div class="onoffswitch">
                                                                        <input type="checkbox" :id="'children_checkbox-'+ parent_index + '_' + item_child.slug" class="onoffswitch-checkbox" :checked="item_child.disabled === 'true'">
                                                                        <label class="onoffswitch-label" @click.stop="handleDisabled($event, item_child)" :for="'children_checkbox-'+ parent_index + '_' + item_child.slug"></label>
                                                                    </div>
                                                                </span>
                                                                ...
                                                                <!-- Level 3 container area -->
                                                                <div v-if="item_child.children && item_child.children.length" :class="['tw-mt-2 list-group nested-sortable poly-hide']">
                                                                    <template v-for="item_child_l3 in item_child.children" :key="item_child_l3.slug">
                                                                        <div :style="handleDividerStyles(item_child_l3)" v-if="item_child_l3.slug && !item_child_l3.slug.includes('_add')" :class="['list-group-item sub3 sub-l3',`nested-${parent_index}-${sub_index}`]" :data-id="item_child_l3.slug" :data-type="item_child_l3.type" :data-css="item_child_l3.css" :data-clients="item_child_l3.clients" :data-is_custom="item_child_l3.is_custom" :data-name="item_child_l3.name" :data-href_original="item_child_l3.href_original" :data-href="item_child_l3.href" :data-target="item_child_l3.target" :data-rel="item_child_l3.rel" :data-icon="item_child_l3.svg ? item_child_l3.svg : item_child_l3.icon" :data-badge="JSON.stringify(item_child_l3.badge)" :data-slug="item_child_l3.slug" :data-parent_slug="item_child_l3.parent_slug" :data-require_login="item_child_l3.require_login">

                                                                            <span class="poly-menu-block">
                                                                                <span class="poly-menu-icon" v-if="item_child_l3.svg" v-html="decodeHtml(item_child_l3.svg)"></span>
                                                                                <i v-if="!item_child_l3.svg" :class="item_child_l3.icon || ''"></i>&nbsp;
                                                                                <span>
                                                                                    <a v-if="item_child_l3.type!=='popup' && item_child_l3.href && item_child_l3.href!=='#'" :href="item_child_l3.href" :slug="item_child_l3.href" v-html="item_child_l3.name" :style="item_child_l3.css"></a>
                                                                                    <span v-if="item_child_l3.type==='popup' || !item_child_l3.href || item_child_l3.href==='#'" v-html="item_child_l3.name" :style="item_child_l3.css"></span>
                                                                                    <span v-if="item_child_l3.badge" :style="'background-color:'+item_child_l3.badge.color" class="tw-ml-2 badge bg-info">{{item_child_l3.badge.value}}</span>
                                                                                </span>
                                                                            </span>

                                                                            <span class="poly-cursor poly-menu-item-disabled relative pull-right" :data-id="item_child_l3.id || item_child_l3.db_id">
                                                                                <div class="onoffswitch">
                                                                                    <input type="checkbox" :id="'children_l3_checkbox-'+ parent_index + '_' + sub_index + '_' + item_child_l3.slug" class="onoffswitch-checkbox" :checked="item_child_l3.disabled === 'true'">
                                                                                    <label class="onoffswitch-label" @click.stop="handleDisabled($event, item_child_l3)" :for="'children_l3_checkbox-'+ parent_index + '_' + sub_index + '_' + item_child_l3.slug"></label>
                                                                                </div>
                                                                            </span>

                                                                            <span @click.stop="handleDelete(item_child_l3)" :data-id="item.slug" v-if="item_child_l3.is_custom=='true'" class="poly-cursor tw-mr-1 text-muted pull-right"><i class="fas fa-trash"></i></span><span @click.stop="handleEdit(item_child_l3, $event)" v-if="item_child_l3.is_custom=='true'" class="poly-cursor poly-menu-item-edit tw-mr-1 text-muted pull-right"><i class="fas fa-pencil"></i></span><span @click.stop="handleClone(item_child_l3)" class="poly-cursor poly-menu-item-clone relative pull-right"><i class="fa-solid fa-clone fa-fw"></i></span>
                                                                        </div>
                                                                    </template>
                                                                </div>
                                                                <!-- END: Level 3 container area -->
                                    ...
                                    <div class="col-md-6">
                                        <h4 class="col-12"><?php echo _l('poly_utilities_controls_header_custom_clients_menu')?></h4>
                                        <div id="poly-custom-menu" class="list-group col">
                                            ...
                                            <div v-for="item in custom_menu_items" class="list-group-item">
                                                ...
                                                <div class="tw-mt-1"><i class="fa-solid fa-unlock fa-fw"></i> Clients: <span class="poly-label label label-info tw-ml-1 tw-mr-1" v-if="item.clients && item.clients.length==0"><?php echo _l('poly_utilities_custom_menu_clients_allow_all_access') ?></span>
                                                    <span v-for="client in item.clients"><span class="poly-label label label-info tw-ml-1 tw-mr-1 poly-block-clients" @click.stop="handleClientInfo(client)">{{client.text}}</span></span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
