...
$has_permission_delete = staff_can('delete', 'lead_manager');
...
if (!is_admin()) {
    if (staff_can('view_own', 'lead_manager')) {
        array_push($where, 'AND (assigned =' . get_staff_user_id() . ')');
    }
}
array_push($where, 'AND ' . db_prefix() . 'leads.status!=1');
$aColumns = hooks()->apply_filters('lead_manager_table_sql_columns', $aColumns);
if (count($custom_fields) > 4) {
    @$this->ci->db->query('SET SQL_BIG_SELECTS=1');
}
$additionalColumns = hooks()->apply_filters('lead_manager_table_additional_columns_sql', [
    'junk',
    'lost',
    'color',
    'assigned',
...
foreach ($rResult as $aRow) {
    ...
    $allow_email = '';
    $no_permission = '';
    if (staff_can('can_audio_call', 'lead_manager') && get_option('call_twilio_active')) {
    ...
    if (staff_can('can_video_call', 'lead_manager') && get_option('call_zoom_active')) {
    ...
    if (staff_can('can_sms', 'lead_manager')) {
    ...
    $row = hooks()->apply_filters('lead_manager_table_row_data', $row, $aRow);
    $output['aaData'][] = $row;
}
