...
use app\services\Imap\Exception\MessageStructureException as Lmimapstrucexc;
                    ...
                    if ($message->getNumber() > $last_sequence_id) {
                        ...
                        while ($retries > 0) {
                            try {
                                ...
                                // Log or handle cases where $structure is null or missing expected properties
                                if (!$structure || !property_exists($structure, 'type')) {
                                    log_activity('Invalid or incomplete message structure for message ' . $message->getNumber(), $mail->staff_id);
                                    continue 2;  // Skip this message
                                }

                                log_activity('Message structure: ' . print_r($structure, true), $mail->staff_id);

                                $body = $message->getBodyHtml() ?? $message->getBodyText();

                                if (empty($body)) {
                                    throw new Exception('Body content is empty.');
                                }
                                break;
                            } catch (Exception $e) {
                                $retries--;
                                if ($retries == 0) {
                                    log_activity('Failed after retries: ' . $e->getMessage() . ' for message ' . $message->getNumber(), $mail->staff_id);
                                    continue 2;
                                }
                            }
                        }
                        ...
                        try {
                            ...
                            log_activity('Raw email content for message ' . $message->getNumber() . ': ' . $rawMessage, $mail->staff_id);
                        } catch (Exception $e) {
                            log_activity('Failed to fetch raw email content: ' . $e->getMessage(), $mail->staff_id);
                        }

                        if (count($message->getAttachments()) > 0 && empty($body)) {
                            log_activity('Attachment-only email skipped: Message ' . $message->getNumber(), $mail->staff_id);
                            continue;
                        }
                        ...
                        if (is_null($fromAddress)) {
                            log_activity('No valid sender address for message ' . $message->getNumber(), $mail->staff_id);
                            continue;
                        }
...
function create_gpt_assistant($api_key, $data)
{
    $curl = curl_init();
    curl_setopt_array($curl, array(
        ...
        CURLOPT_POSTFIELDS => json_encode([
            "instructions" => $data['settings']['lm_asistant_prompt'],
            "name" => $data['settings']['lm_asistant_name'],
...
function update_gpt_assistant($api_key, $data)
{
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://api.openai.com/v1/assistants/' . $data['settings']['lm_asistant_id'],
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
