...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->load->library('Format', NULL, 'lmLibraryFormat');
        if(!class_exists('lmLibraryFormat'))
        {   
            $this->format = new Format();
        }
        else
        {
            $this->format = $this->lmLibraryFormat;
            // $this->format =$this->load->library('Format');
        }
        ...
        $this->lang->load('lead_manager', $language, FALSE, TRUE, __DIR__.'/../');
        ...
        if(!$this->app_modules->is_active('lead_manager')){
            $this->response([
                'status' => FALSE,
                'message' => "Lead Manager Module is not active"
        ...
        if(!class_exists('lead_manager_api_model')){
            $this->load->model('lead_manager_api_model');
        }
        
        $check_token = $this->lead_manager_api_model->check_token($token);
      
        if($is_valid_token['status'] == false || $check_token === false){
            $message = array(
                'status' => FALSE,
                'message' => $is_valid_token['message']??'Invalid authToken'
            );
            $this->response($message, REST_Controller::HTTP_UNAUTHORIZED);
        }else{
            $this->load->library('session');
            $token_staff_id = $is_valid_token['data']->staff_id;
            $user= get_staff($token_staff_id);
            $is_staff_logged_in=$this ->session->has_userdata('staff_logged_in'); 
            ...
            }elseif($this->session->userdata('staff_user_id') !=$token_staff_id){
     ...
     */
    public function _lm_allow_methods(array $methods)
    {
        ...
        if (in_array(strtolower($REQUEST_METHOD), $this->allowed_http_methods))
        {
            ...
            } else
            {
                ...
                $this->response(['status' => FALSE, 'error' => 'Unknown method'], self::HTTP_METHOD_NOT_ALLOWED);
            }
        } else {
            $this->response(['status' => FALSE, 'error' => 'Unknown method'], self::HTTP_METHOD_NOT_ALLOWED);
        }
