...
<?php if (count($purchase_orders_years) > 1 || isset($currencies)) { ?>
    <div class="tw-flex tw-space-x-3 tw-mb-2">
        <?php if (isset($currencies)) { ?>
            <div class="!tw-w-28 simple-bootstrap-select">
                <select class="selectpicker !tw-w-28" data-width="auto" name="total_currency" onchange="init_purchase_orders_total();">
        ...
        <?php } ?>
        <?php
        if (count($purchase_orders_years) > 1) { ?>
            <div class="simple-bootstrap-select !tw-max-w-xs">
                <select data-none-selected-text="<?php echo date('Y'); ?>" data-width="auto" class="selectpicker tw-w-full" multiple name="purchase_orders_total_years" onchange="init_purchase_orders_total();">
...
<dl class="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 lg:tw-grid-cols-5 tw-gap-3 sm:tw-gap-5 tw-mb-0">
    ...
    foreach ($totals as $key => $data) {
        $class = purchase_order_status_color_class($data['status']);
        $name  = purchase_order_status_by_id($data['status']); ?>
