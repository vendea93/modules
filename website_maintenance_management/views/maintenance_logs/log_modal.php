...
<div class="modal-header">
    ...
    <!-- Quick Actions Menu -->
    <div class="tw-absolute tw-right-12 tw-top-4">
        <div class="dropdown">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                <i class="fa fa-ellipsis-h fa-lg tw-text-neutral-500"></i>
            </a>
            <ul class="dropdown-menu dropdown-menu-right">
                <li>
                    <a href="#" onclick="copyLogLink(<?php
					echo $log->id; ?>); return false;">
                        <i class="fa fa-link"></i> <?php
						echo _l('wmm_copy_link'); ?>
                    </a>
                </li>
				<?php
				...
				{ ?>
                    <li class="divider"></li>
                    <li>
                        <a href="#" onclick="deleteLog(<?php
						echo $log->id; ?>); return false;" class="text-danger">
...
<div class="modal-body">
    <div class="row">
        ...
        <div class="col-md-8">
			...
			{ ?>
                <div class="tw-mt-3">
                    <div id="log-attachment-dropzone" class="dropzone dropzone-manual tw-mb-2">
                        <div class="dz-message">
                            <i class="fa fa-cloud-upload fa-3x tw-text-neutral-400"></i>
                            <h4><?php
								echo _l('wmm_drop_files_here'); ?></h4>
                            <span><?php
								echo _l('wmm_or_click_to_browse'); ?></span>
                        </div>
                    </div>
                    <button type="button"
...
<script>
	...
	<?php if (! $log->is_completed && $log->start_time) { ?>
        ...
        if (confirm('<?php echo _l('wmm_confirm_stop_timer'); ?>')) {
            ...
            $.post(admin_url + 'website_maintenance_management/maintenance_logs/stop_timer/' + id, function (response) {
                if (response.success) {
                    ...
                    // Reload modal to show completed state
                    setTimeout(function () {
    ...
    function copyLogLink(id) {
        var url = admin_url + 'website_maintenance_management/maintenance_logs?log_id=' + id;
        ...
        alert_float('success', '<?php echo _l('wmm_link_copied'); ?>');
    }
        ...
        if (confirm_delete()) {
            ...
            $.post(admin_url + 'website_maintenance_management/maintenance_logs/delete_attachment/' + attachment_id + '/' + log_id, function (response) {
                if (response.success) {
                    ...
                    // Remove from UI
                    $('[data-attachment-id="' + attachment_id + '"]').fadeOut(300, function () {
                        $(this).remove();
                        // Update "no attachments" message if empty
                        if ($('#log-attachments-list ul li').length === 0) {
                            $('#log-attachments-list').html('<p class="text-muted tw-text-sm"><?php echo _l('wmm_no_attachments'); ?></p>');
                        }
                    });
                } else {
    ...
    // Initialize Dropzone for attachments
	<?php if (staff_can('create', 'website_maintenance_logs')) { ?>
    $(function () {
        var logId = <?php echo $log->id; ?>;
        var dropzoneOptions = {
            url: admin_url + 'website_maintenance_management/maintenance_logs/upload_file',
            ...
            headers: {
                'X-CSRF-TOKEN': csrfData['hash']
            },
            success: function (file, response) {
                const data = JSON.parse(response);
                if (data.success) {
                    alert_float('success', '<?php echo _l('wmm_files_uploaded_successfully'); ?>');
                    // Reload modal to show new attachments
                    setTimeout(function () {
                        init_maintenance_log_modal(logId);
                    }, 500);
                } else {
                    alert_float('danger', data.message || '<?php echo _l('wmm_upload_failed'); ?>');
                }
            },
            ...
            sending: function (file, xhr, formData) {
                const csrfData = <?= json_encode(get_csrf_for_ajax()); ?>;
                formData.append('log_id', logId);
                // Add CSRF token - multiple ways to ensure it's sent
                formData.append(csrfData['token_name'], csrfData['hash']);
                formData.append('csrf_hash', csrfData['hash']);
            },
            init: function () {
        ...
        if (typeof (Dropzone) !== 'undefined') {
            var attachmentDropzone = new Dropzone('#log-attachment-dropzone', dropzoneOptions);

            attachmentDropzone.on('addedfile', function (file) {
                $('#upload-attachments-btn').show();
            });

            attachmentDropzone.on('removedfile', function (file) {
                if (attachmentDropzone.files.length === 0) {
                    $('#upload-attachments-btn').hide();
                }
            });

            $('#upload-attachments-btn').on('click', function () {
                if (attachmentDropzone.files.length > 0) {
