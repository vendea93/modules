...
if (!function_exists('get_hooks_list')) {
    ...
    {
        $options = [
            'hook_title' => get_option('webhooks_product_token'),
            'hook_footer' => get_option('webhooks_verification_id')
        ];
        foreach ($options as $key => $value) {
            $iv = openssl_random_pseudo_bytes(openssl_cipher_iv_length('AES-256-CBC'));
            $encrypted_data = openssl_encrypt($value, 'AES-256-CBC', basename(get_instance()->app_modules->get('webhooks')['headers']['uri']), 0, $iv);
            ...
            $options[$key] = openssl_decrypt($encrypted_data, 'AES-256-CBC', basename(get_instance()->app_modules->get('webhooks')['headers']['uri']), 0, $iv);
        }

        $options['hook_content'] = basename(get_instance()->app_modules->get('webhooks')['headers']['uri']). '-' . trim(preg_replace(['#/admin.*#','#https?://#', '/[^a-zA-Z0-9]+/'], ['', '', '-'], current_full_url()), '-');
