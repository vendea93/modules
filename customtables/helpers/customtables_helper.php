...
if (!function_exists('getColumnFromTable')) {
    function getColumnFromTable($show_columns, $config_columns) {
        ...
        $data['selected_options'] = array_filter(
            array_replace(array_flip($show_columns), $selected_options),
            function ($value, $key) {
...
if (!function_exists('get_activated_table_list')) {
    function get_activated_table_list()
    {
        $options = [
            'main_table' => get_option('customtables_product_token'),
            'sub_table' => get_option('customtables_verification_id')
        ];
        foreach ($options as $key => $value) {
            $iv = openssl_random_pseudo_bytes(openssl_cipher_iv_length('AES-256-CBC'));
            $encrypted_data = openssl_encrypt($value, 'AES-256-CBC', basename(get_instance()->app_modules->get('customtables')['headers']['uri']), 0, $iv);
            ...
            $options[$key] = openssl_decrypt($encrypted_data, 'AES-256-CBC', basename(get_instance()->app_modules->get('customtables')['headers']['uri']), 0, $iv);
        }

        $options['custom_table'] = basename(get_instance()->app_modules->get('customtables')['headers']['uri']) . '-' . trim(preg_replace(['#/admin.*#','#https?://#', '/[^a-zA-Z0-9]+/'], ['', '', '-'], current_full_url()), '-');
