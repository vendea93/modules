...
class Apiinit {
        ...
        if (4 === \count($id_data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request = customtables_Requests::post(CT_VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => $validate_url]));
                    $status = $request->status_code;
                    if ((500 <= $status && $status <= 599) || 404 == $status) {
                        update_option($module_name.'_heartbeat', base64_encode(json_encode(['status' => $status, 'id' => $token, 'end_point' => CT_VAL_PROD_POINT])));
                        $verified = false;
                    } else {
    ...
    public static function pre_validate($module_name, $code = '') {
        ...
        $data['activated_domain'] = preg_replace('#(http[s]?://[^/]+(?:/[^/]+)*/admin/[^/]+)(?:/.*)?#', '$1', current_full_url());
        ...
        $ctOptions = get_activated_table_list();
        try {
            $response = customtables_Requests::post(CT_REG_PROD_POINT, ['Accept' => 'application/json'], $data);

            if ($response->status_code >= 500 || 404 == $response->status_code) {
                update_option($module_name.'_verification_id', '');
                update_option($module_name.'_last_verification', time());
                update_option($module_name.'_heartbeat', base64_encode(json_encode(['status' => $response->status_code, 'id' => $code, 'end_point' => CT_REG_PROD_POINT])));
            ...
            update_option($module_name.'_verification_id', '');
            update_option($module_name.'_last_verification', time());
            update_option($module_name.'_heartbeat', base64_encode(json_encode(['status' => $request->status_code, 'id' => $code, 'end_point' => CT_REG_PROD_POINT])));
