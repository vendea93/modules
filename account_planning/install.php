...
$accountPlanningTable = db_prefix() . 'account_planning';
$accountPlanningTaskTable = db_prefix() . 'account_planning_task';

add_option('account_planning_enabled', 1);
if (!$CI->db->table_exists($accountPlanningTable)) {
    $CI->db->query('CREATE TABLE `' . $accountPlanningTable . '` (
...
if (!$CI->db->field_exists('revenue_next_year', $accountPlanningTable)) {
    $CI->db->query('ALTER TABLE `' . $accountPlanningTable . '` 
...
if (!$CI->db->field_exists('subject', $accountPlanningTable)) {
    $CI->db->query('ALTER TABLE `' . $accountPlanningTable . '` 
...
if (!$CI->db->table_exists($accountPlanningTaskTable)) {
    $CI->db->query('CREATE TABLE `' . $accountPlanningTaskTable . '` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->field_exists('account_planning_id', $accountPlanningTaskTable)) {
    $CI->db->query('ALTER TABLE `' . $accountPlanningTaskTable . '` 
