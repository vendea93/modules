...
/*
Module Name: FlowQuest - Account Planning
...
Version: 1.0.0
FlowQuest Source: account_planning
FlowQuest Variant: Integral Fork
Requires at least: 2.3.*
Author: Themesic Interactive
Author URI: https://codecanyon.net/user/themesic/portfolio
*/

defined('ACCOUNT_PLANNING_MODULE_NAME') or define('ACCOUNT_PLANNING_MODULE_NAME', 'account_planning');
defined('ACCOUNT_PLANNING_ATTACHMENTS_FOLDER') or define('ACCOUNT_PLANNING_ATTACHMENTS_FOLDER', module_dir_path(ACCOUNT_PLANNING_MODULE_NAME, 'uploads/'));
...
function account_planning_add_head_components(){
    if (get_option('account_planning_enabled') == '1') {
        if (account_planning_should_load_handsontable()) {
...
function account_planning_add_footer_components(){
    ...
    if (account_planning_should_load_handsontable_helpers()) {
...
function account_planning_request_path()
{
    $CI = &get_instance();
    $path = trim((string) $CI->uri->uri_string(), '/');
    if ($path !== '') {
        return $path;
    }
...
function account_planning_current_group()
{
    $CI = &get_instance();

    return (string) $CI->input->get('group');
}

function account_planning_is_view_page()
{
    return preg_match('~(^|/)admin/account_planning/view/[^/]+$~', account_planning_request_path()) === 1;
}

function account_planning_should_load_handsontable()
{
    $path = account_planning_request_path();

    if (preg_match('~(^|/)admin/account_planning/new_account$~', $path) === 1) {
        return true;
    }

    if (!account_planning_is_view_page()) {
        return false;
    }

    $group = account_planning_current_group();

    return ($group === '' || in_array($group, ['due_diligence', 'service_ability_offering', 'planning'], true));
}

function account_planning_should_load_handsontable_helpers()
{
    if (!account_planning_is_view_page()) {
        return false;
    }
    ...
    return ($group === '' || in_array($group, ['service_ability_offering', 'planning'], true));
}
