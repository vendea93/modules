...
class WorkflowAutomationLic{
	...
	public function activate_license($license, $client, $create_lic = true){
		...
		if (empty($license) || empty($client)) {
			return array(
				'status' => false,
				'message' => 'License code and client name are required.'
			);
		}
		...
		$response = array(
			...
			'lic_response' => base64_encode(json_encode(array(
		...
		if ($create_lic) {
			if ($response['status']) {
				...
				$license_dir = dirname($this->license_file);
				if (!is_dir($license_dir)) {
					mkdir($license_dir, 0755, true);
				}
				
				if (!file_put_contents($this->license_file, $licfile, LOCK_EX)) {
					return array(
						'status' => false,
						'message' => 'Failed to write license file.'
					);
				}
	...
	public function deactivate_license($license = false, $client = false){
		...
		$response = array('status' => true, 'message' => 'License deactivated successfully.');
		if (is_file($this->license_file)) {
		...
		if ($filesize){
			switch ($filesize){
				...
				case $filesize < 1073741824:
					$size = round($filesize / 1073741824, 2) . ' MB'; break;
