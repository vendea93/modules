	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				$staffid = '';
				if(isset($member)){
					$title .= _l('wa_update_permissions');
					$staffid    = $member->staffid;
				}else{
					$title .= _l('wa_add_permissions');
				}
				?>
				<h4 class="modal-title"><?php echo wa_html_entity_decode($title); ?></h4>
			</div>
			<?php echo form_open(admin_url('workflow_automation/workflow_automation_update_permissions/'.$staffid), array('id' => 'update_permissions')); ?>
			<div class="modal-body">
					...
					<div class="hide">
						...
						?>
						<input type="checkbox" name="administrator" id="administrator" <?php echo wa_html_entity_decode($isadmin); ?>>
					</div>
					...
					<?php if(isset($member)){ 
						...
						<div class="lable-display-name">
							<?php echo render_input('staff_name', 'wa_staff_name', get_staff_full_name($member->staffid), '', $staff_attr); ?>
					...
					<div class="class <?php  echo wa_html_entity_decode($display_staff); ?>">
						<?php echo render_select('staff_id',$staffs,array('staffid',array('firstname', 'lastname')),'wa_staff_name',$staff_selected); ?>
					</div>
					...
					<table class="table table-bordered roles no-margin">
						...
						<tbody>
							...
							$hr_profile_permissions = list_workflow_automation_permisstion();

							foreach(get_available_staff_permissions($funcData) as $feature => $permission) { ?>
								<tr data-name="<?php echo wa_html_entity_decode($feature); ?>" class="<?php if(!in_array($feature, $hr_profile_permissions)){echo "hide";} ?>">
									<td>
										<b><?php echo wa_html_entity_decode($permission['name']); ?></b>
									</td>
									<td>
										<?php
										if(isset($permission['before'])){
											echo wa_html_entity_decode($permission['before']);
										}
										?>
										...
										<div class="checkbox">
											...
											type="checkbox"
											<?php echo wa_html_entity_decode($checked);?>
											class="capability"
											id="<?php echo wa_html_entity_decode($feature .'_'.$capability); ?>"
											name="permissions[<?php echo wa_html_entity_decode($feature); ?>][]"
											value="<?php echo wa_html_entity_decode($capability); ?>"
											<?php echo wa_html_entity_decode($disabled); ?>>
											<label for="<?php echo wa_html_entity_decode($feature .'_'.$capability); ?>">
									...
									if(isset($permission['after'])){
										echo wa_html_entity_decode($permission['after']);
									}
									?>
								</td>
							</tr>
...
<?php require('modules/workflow_automation/assets/js/settings/permissions_modal_js.php'); ?>