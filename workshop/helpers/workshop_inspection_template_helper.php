 ...
 * @return mixed
 */
function wshop_render_inspection_template_form_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [])
{
    ...
    $fields = $CI->db->get(db_prefix() . 'wshop_inspection_template_form_details')->result_array();
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            $fields_html .= '<div class="form-question dragger" id="form_question_'.$field['inspection_template_form_id'].'_'.$field['id'].'">';
            $fields_html .= '<input type="hidden" name="field_order" data-question_id="'.$field['id'].'" value="'.$field['field_order'].'">';
            
            $field['name'] = _wshop_maybe_translate_inspection_template_form_field_name($field['name'], $field['slug']);
            ...
            if ($rel_id !== false) {
                if (!is_array($rel_id)) {
                    $value = wshop_get_inspection_template_form_field_value($rel_id, $field['id'], ($items_pr ? 'items_pr' : $belongs_to), false);
                } else {
                    if (wshop_is_custom_fields_smart_transfer_enabled()) {
                        ...
                        if (isset($tmpSlug[1])) {
                            ...
                            $CI->db->where('active', 1);
                            $CI->db->group_end();
                            $cfTransfer = $CI->db->get(db_prefix() . 'wshop_inspection_template_form_details')->result_array();
                            ...
                            if (count($cfTransfer) == 1 && ((similarity($field['name'], $cfTransfer[0]['name']) * 100) >= CUSTOM_FIELD_TRANSFER_SIMILARITY)) {
                                $value = wshop_get_inspection_template_form_field_value($transfer_rel_id, $cfTransfer[0]['id'], $transfer_belongs_to, false);
                            }
                        }
                    }
                }
            ...
            if ((has_permission('workshop_setting', '', 'create') || has_permission('workshop_setting', '', 'edit'))
                ...
                <input type="checkbox" id="required_'.$field['inspection_template_form_id'].'_'.$field['id'].'" name="required_'.$field['inspection_template_form_id'].'_'.$field['id'].'" value="1" '.$required_checked.'  data-id="'.$field['id'].'">
                <label for="required_'.$field['inspection_template_form_id'].'_'.$field['id'].'">'._l('inspection_template_form_detail_required').'</label>
                </div>
                </div>
                </div>';
            ...
            if ((has_permission('workshop_setting', '', 'create') || has_permission('workshop_setting', '', 'edit'))
                && ($items_add_edit_preview == false && $items_applied == false)
                && (!defined('CLIENTS_AREA') || hooks()->apply_filters('show_custom_fields_edit_link_on_clients_area', false))) {
                $fields_html .= '<div class="col-md-2"><a href="#" tabindex="-1" onclick="delete_inspection_template_form_detail(this, '.$field['id'].', '.$field['inspection_template_form_id'].'); return false;" class="pull-right btn-danger btn btn-icon"><i class="fa-regular fa-trash-can"></i></a><a href="#" tabindex="-1" onclick="inspection_template_form_detail_modal('.$field['id'].', '.$field['inspection_template_form_id'].'); return false;" class="custom-field-inline-edit-link pull-right btn-primary btn btn-icon"><i class="fa-regular fa-pen-to-square"></i></a></div>';
            }
 ...
 * @return array
 */
function wshop_get_inspection_template_form_fields($field_to, $where = [], $exclude_only_admin = false)
{
    $is_admin = is_admin();
    $CI       = & get_instance();
    ...
    $results = $CI->db->get(db_prefix() . 'wshop_inspection_template_form_details')->result_array();

    foreach ($results as $key => $result) {
        $results[$key]['name'] = _wshop_maybe_translate_inspection_template_form_field_name(e($result['name']), $result['slug']);
    }

    return $results;
}

function _wshop_maybe_translate_inspection_template_form_field_name($name, $slug)
{
 ...
 * @return array
 */
function wshop_get_table_inspection_template_form_fields($field_to)
{
    return wshop_get_inspection_template_form_fields($field_to, ['show_on_table' => 1]);
}
/**
 ...
 * @return string
 */
function wshop_get_inspection_template_form_field_value($rel_id, $field_id_or_slug, $field_to, $format = true)
{
    $CI = & get_instance();

    $CI->db->select(db_prefix() . 'wshop_inspection_template_values.value,' . db_prefix() . 'wshop_inspection_template_form_details.type');
    $CI->db->join(db_prefix() . 'wshop_inspection_template_form_details', db_prefix() . 'wshop_inspection_template_form_details.id=' . db_prefix() . 'wshop_inspection_template_values.fieldid');
    $CI->db->where(db_prefix() . 'wshop_inspection_template_values.relid', $rel_id);
    if (is_numeric($field_id_or_slug)) {
        $CI->db->where(db_prefix() . 'wshop_inspection_template_values.fieldid', $field_id_or_slug);
    } else {
        $CI->db->where(db_prefix() . 'wshop_inspection_template_form_details.slug', $field_id_or_slug);
    }
    $CI->db->where(db_prefix() . 'wshop_inspection_template_values.fieldto', $field_to);

    $row = $CI->db->get(db_prefix() . 'wshop_inspection_template_values')->row();
 ...
 * @return boolean
 */
function wshop_handle_inspection_template_form_fields_post($rel_id, $custom_fields, $is_cf_items = false)
{
    $affectedRows = 0;
    $CI           = & get_instance();
    ...
    foreach ($custom_fields as $key => $fields) {
        foreach ($fields as $field_id => $field_value) {
            ...
            if (!defined('COPY_CUSTOM_FIELDS_LIKE_HANDLE_POST')) {
                ...
                $field_checker = $CI->db->get(db_prefix() . 'wshop_inspection_template_form_details')->row();
            ...
            if ($row) {
                ...
                $CI->db->update(db_prefix() . 'wshop_inspection_template_values', [
                    'value' => $field_value,
                ]);
                ...
                if ($field_value != '') {
                    $CI->db->insert(db_prefix() . 'wshop_inspection_template_values', [
                        'relid'   => $rel_id,
                        'fieldid' => $field_id,
 ...
 * @return array
 */
function wshop_get_items_inspection_template_form_fields_for_table_html($rel_id, $rel_type)
{
    $CI       = &get_instance();
    $whereSQL = 'id IN (SELECT fieldid FROM ' . db_prefix() . 'wshop_inspection_template_values WHERE value != "" AND value IS NOT NULL AND fieldto="items" AND relid IN (SELECT id FROM ' . db_prefix() . 'itemable WHERE rel_type="' . $CI->db->escape_str($rel_type) . '" AND rel_id="' . $CI->db->escape_str($rel_id) . '") GROUP BY id HAVING COUNT(id) > 0)';

    $whereSQL = hooks()->apply_filters('items_custom_fields_for_table_sql', $whereSQL);

    return wshop_get_inspection_template_form_fields('items', $whereSQL);
}
/**
 * Render custom fields for table add/edit preview area
 * @return string
 */
function wshop_render_inspection_template_form_fields_items_table_add_edit_preview()
{
    $where = hooks()->apply_filters('custom_fields_where_items_table_add_edit_preview', []);

    return wshop_render_inspection_template_form_fields('items', false, $where, [
 ...
 * @return string
 */
function wshop_render_inspection_template_form_fields_items_table_in($item, $part_item_name)
{
    $item_id = false;
    ...
    return wshop_render_inspection_template_form_fields('items', $item_id, [], [
        'items_applied'  => true,
 ...
 * @return array
 */
function wshop_get_company_inspection_template_form_fields()
{
    $fields = wshop_get_inspection_template_form_fields('company');
    $i      = 0;
    foreach ($fields as $field) {
        $fields[$i]['label'] = $field['name'];
        $fields[$i]['value'] = wshop_get_inspection_template_form_field_value(0, $field['id'], 'company');
        $i++;
    }
