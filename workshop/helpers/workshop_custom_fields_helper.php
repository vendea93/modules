 ...
 * @return mixed
 */
function wshop_render_custom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [])
{
    ...
    $fields = $CI->db->get(db_prefix() . 'wshop_customfields')->result_array();
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            $field['name'] = _wshop_maybe_translate_custom_field_name($field['name'], $field['slug']);
            ...
            if ($rel_id !== false) {
                if (!is_array($rel_id)) {
                    $value = wshop_get_custom_field_value($rel_id, $field['id'], ($items_pr ? 'items_pr' : $belongs_to), false);
                } else {
                    if (wshop_is_custom_fields_smart_transfer_enabled()) {
                        ...
                        if (isset($tmpSlug[1])) {
                            ...
                            $CI->db->where('active', 1);
                            $CI->db->group_end();
                            $cfTransfer = $CI->db->get(db_prefix() . 'wshop_customfields')->result_array();
                            ...
                            if (count($cfTransfer) == 1 && ((similarity($field['name'], $cfTransfer[0]['name']) * 100) >= CUSTOM_FIELD_TRANSFER_SIMILARITY)) {
                                $value = wshop_get_custom_field_value($transfer_rel_id, $cfTransfer[0]['id'], $transfer_belongs_to, false);
                            }
                        }
                    }
                }
 ...
 * @return array
 */
function wshop_get_custom_fields($field_to, $where = [], $exclude_only_admin = false)
{
    $is_admin = is_admin();
    $CI       = & get_instance();
    ...
    $results = $CI->db->get(db_prefix() . 'wshop_customfields')->result_array();

    foreach ($results as $key => $result) {
        $results[$key]['name'] = _wshop_maybe_translate_custom_field_name(e($result['name']), $result['slug']);
    }

    return $results;
}

function _wshop_maybe_translate_custom_field_name($name, $slug)
{
 ...
 * @return array
 */
function wshop_get_table_custom_fields($field_to)
{
    return wshop_get_custom_fields($field_to, ['show_on_table' => 1]);
}
/**
 ...
 * @return string
 */
function wshop_get_custom_field_value($rel_id, $field_id_or_slug, $field_to, $format = true)
{
    $CI = & get_instance();

    $CI->db->select(db_prefix() . 'wshop_customfieldsvalues.value,' . db_prefix() . 'wshop_customfields.type');
    $CI->db->join(db_prefix() . 'wshop_customfields', db_prefix() . 'wshop_customfields.id=' . db_prefix() . 'wshop_customfieldsvalues.fieldid');
    $CI->db->where(db_prefix() . 'wshop_customfieldsvalues.relid', $rel_id);
    if (is_numeric($field_id_or_slug)) {
        $CI->db->where(db_prefix() . 'wshop_customfieldsvalues.fieldid', $field_id_or_slug);
    } else {
        $CI->db->where(db_prefix() . 'wshop_customfields.slug', $field_id_or_slug);
    }
    $CI->db->where(db_prefix() . 'wshop_customfieldsvalues.fieldto', $field_to);

    $row = $CI->db->get(db_prefix() . 'wshop_customfieldsvalues')->row();
 ...
 * @return boolean
 */
function wshop_handle_custom_fields_post($rel_id, $custom_fields, $is_cf_items = false)
{
    $affectedRows = 0;
    $CI           = & get_instance();
    ...
    foreach ($custom_fields as $key => $fields) {
        foreach ($fields as $field_id => $field_value) {
            ...
            if (!defined('COPY_CUSTOM_FIELDS_LIKE_HANDLE_POST')) {
                ...
                $field_checker = $CI->db->get(db_prefix() . 'wshop_customfields')->row();
            ...
            if ($row) {
                ...
                $CI->db->update(db_prefix() . 'wshop_customfieldsvalues', [
                    'value' => $field_value,
                ]);
                ...
                if ($field_value != '') {
                    $CI->db->insert(db_prefix() . 'wshop_customfieldsvalues', [
                        'relid'   => $rel_id,
                        'fieldid' => $field_id,
 ...
 * @return array
 */
function wshop_get_items_custom_fields_for_table_html($rel_id, $rel_type)
{
    $CI       = &get_instance();
    $whereSQL = 'id IN (SELECT fieldid FROM ' . db_prefix() . 'wshop_customfieldsvalues WHERE value != "" AND value IS NOT NULL AND fieldto="items" AND relid IN (SELECT id FROM ' . db_prefix() . 'itemable WHERE rel_type="' . $CI->db->escape_str($rel_type) . '" AND rel_id="' . $CI->db->escape_str($rel_id) . '") GROUP BY id HAVING COUNT(id) > 0)';

    $whereSQL = hooks()->apply_filters('items_custom_fields_for_table_sql', $whereSQL);

    return wshop_get_custom_fields('items', $whereSQL);
}
/**
 * Render custom fields for table add/edit preview area
 * @return string
 */
function wshop_render_custom_fields_items_table_add_edit_preview()
{
    $where = hooks()->apply_filters('custom_fields_where_items_table_add_edit_preview', []);

    return wshop_render_custom_fields('items', false, $where, [
 ...
 * @return string
 */
function wshop_render_custom_fields_items_table_in($item, $part_item_name)
{
    $item_id = false;
    ...
    return wshop_render_custom_fields('items', $item_id, [], [
        'items_applied'  => true,
 ...
 * @return array
 */
function wshop_get_company_custom_fields()
{
    $fields = wshop_get_custom_fields('company');
    $i      = 0;
    foreach ($fields as $field) {
        $fields[$i]['label'] = $field['name'];
        $fields[$i]['value'] = wshop_get_custom_field_value(0, $field['id'], 'company');
        $i++;
    }
 ...
 * @return boolean
 */
function wshop_is_cf_date($field)
{
...
*/
function wshop_is_custom_fields_for_customers_portal()
{
 ...
 * @return stirng
 */
function wshop_get_custom_fields_hyperlink_js_function()
{
    ob_start(); ?>
<script>
function wshop_custom_fields_hyperlink() {
        ...
        $.each(cf_hyperlink, function() {
            ...
            if (!cfh_wrapper.hasClass('cfh-initialized')) {
                ...
                textEl.popover({
                    ...
                    $("#custom_fields_" + cfh_field_to + "_" + cfh_field_id + "_btn-save").on('click', 
                    ...
                    $("#custom_fields_" + cfh_field_to + "_" + cfh_field_id + "_btn-cancel").on('click', 
                        function() {
                            if (urlField.val() == '') {
                                hiddenField.val('');
                            }
...
function wshop_is_custom_fields_smart_transfer_enabled()
{
