...
<div id="wrapper">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <?php if(is_numeric($repair_job->invoice_id)){ ?>
                                    <span class="tw-font-semibold tw-text-sm">
                                        <a href="<?php echo admin_url('invoices/list_invoices/'.$repair_job->invoice_id); ?>" target="_blank"><?php echo format_invoice_number($repair_job->invoice_id); ?></a>
                                    </span><br>
                                <?php } ?>
                                ...
                                <?php if(count($_inspection) > 0){ ?>
                                    <?php if(is_numeric($_inspection[0]['invoice_id'])){ ?>
                                        <span class="tw-font-semibold tw-text-sm">
                                            <a href="<?php echo admin_url('invoices/list_invoices/'.$_inspection[0]['invoice_id']); ?>" target="_blank"><?php echo format_invoice_number($_inspection[0]['invoice_id']); ?></a>
                                        </span><br>
                                    <?php } ?>
                                <?php } ?>
                                ...
                                <span class="tw-font-semibold tw-text-xl">
                                    <?php 
                                    $change_status = false;
                                    if(has_permission('workshop_repair_job', '', 'edit')){
                                        $change_status = true;
                                    } ?>
                                ...
                                <?php if($repair_job->status != 'Booked_In' && $repair_job->status != 'In_Progress' && is_null($repair_job->invoice_id)){ ?>
                                <?php if($check_parts_available['status']){ ?>
                                    <a href="<?php echo admin_url('workshop/convert_to_invoice/'.$repair_job->id.'/repair_job'); ?>" class="btn btn-success mright5" data-toggle="tooltip" data-placement="bottom" data-original-title="<?php echo _l('wshop_part_create_invoice_tooltip'); ?>">
                                        <?php echo _l('wshop_create_invoice'); ?>
                                    </a>
                                <?php }elseif(!$check_parts_available['status'] && $repair_job->invoice_id == 0){ ?>
                            ...
                            <div class="col-md-6">
                                <?php if(has_permission('workshop_repair_job', '', 'create') || has_permission('workshop_repair_job', '', 'edit')){ ?>
                                    <div class=" pull-right">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            ...
                                            <li><a href="#" onclick="print_report_preview(); return false;"><?php echo _l('wshop_print_report'); ?></a>
                                            </li>
                                            <?php if($repair_job->status != 'Cancelled'){ ?>
                                                <li><a href="#" onclick="cancel_repair_job(); return false;"><?php echo _l('wshop_cancel'); ?></a>
                                                </li>
                                            <?php } ?>
                                            <li><a href="#" onclick="assign_mechanic(); return false;" ><?php echo _l('wshop_assign_mechanic'); ?></a>
                                            </li>
                                ...
                                <?php if(has_permission('workshop_inspection', '', 'create')){ ?>
                                    <?php if(!isset($_inspection) || count($_inspection) == 0){ ?>
                                        <a href="#" onclick="inspection_modal(0, <?php echo html_entity_decode($repair_job->id); ?>, 'delivery'); return false;" class="btn btn-info pull-right display-block mright5">
                                            <?php echo _l('wshop_create_inspection'); ?>
                                        </a>
                                ...
                                if ($repair_job->sent == 1) {
                                    $_tooltip_already_send = _l('wshop_repair_job_already_send_to_client_tooltip', time_ago($repair_job->datesend));
                                }
                                ?>
                                <?php if (!empty($repair_job->client_id)) { ?>
                                    <a href="#" onclick="repair_job_send_mail_client(); return false;" class="contract-estimate-send-to-client btn btn-default btn-with-tooltip pull-right mright5"
                                ...
                                <?php if (has_permission('workshop_repair_job', '', 'edit')) { ?>
                                    <a href="<?php echo admin_url('workshop/add_edit_repair_job/' . $repair_job->id); ?>"
                                        class="btn btn-default btn-with-tooltip pull-right mright5" data-toggle="tooltip"
                                        title="<?php echo _l('edit'); ?>" data-placement="bottom"><i
                                        class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                <?php } ?>
                                <a href="<?php echo admin_url('workshop/repair_jobs'); ?>" class="btn btn-default pull-right display-block mright5">
                                    <?php echo _l('wshop_back'); ?>
                                </a>
                            </div>
                        </div>
                        <?php if(strlen($check_parts_available['message']) > 0){ ?>
                            <div class="alert alert-warning bold project-due-notice no-mbot mtop15"><?php echo html_entity_decode($check_parts_available['message']); ?></div>
                        <?php } ?>
                                ...
                                <div role="tabpanel" class="tab-pane <?php if(!$this->input->get('tab') || $this->input->get('tab') == 'detail'){echo 'active';} ?>"  id="detail">
                                    ...
                                    <div class="col-md-8">
                                        <table class="table items items-preview estimate-items-preview no-mtop" data-type="estimate">
                                            ...
                                            <tbody class="ui-sortable">
                                                <tr>
                                                    <td ><a href="<?php echo admin_url('workshop/inspection_detail/'.$_inspection[0]['id'].'?tab=detail'); ?>"><?php echo format_inspection_number($_inspection[0]['id']) ?></a></td>
                                                    ...
                                                    if($_inspection[0]['status'] == 'In_Progress'){
                                                        $_inspection_option1 = '<a href="'.admin_url('workshop/inspection_form/'.$_inspection[0]['id']).'" class="btn btn-info mright5 pull-right" data-toggle="tooltip" data-original-title="'._l('wshop_inspection_form').'">
                                                        <i class="fa-solid fa-play"></i>
                                                        </a>';
                                                    }

                                                    $_inspection_option2 = '<a href="'.admin_url('workshop/inspection_form_detail/'.$_inspection[0]['id']).'" class="btn btn-success mright5 pull-right" data-toggle="tooltip" data-original-title="'._l('wshop_inspection_form_detail').'">
                                                    <i class="fa-solid fa-eye"></i>
                                                    </a>';

                                                    $_inspection_option3= '<div class="onoffswitch">
                                                    <input type="checkbox" ' . (((is_admin() || !has_permission('workshop_inspection', '', 'edit')) && !is_admin()) ? 'disabled' : '') . ' data-switch-url="' . admin_url() . 'workshop/change_inspection_visible" name="onoffswitch" class="onoffswitch-checkbox" id="c_' . $_inspection[0]['id'] . '" data-id="' . $_inspection[0]['id'] . '" data-status="' . $_inspection[0]['visible_to_customer'] . '" ' . $checked . '>
                                                    <label class="onoffswitch-label" for="c_' . $_inspection[0]['id'] . '"></label>
                                                    </div>';
    ...
    <?php $this->load->view('repair_jobs/modals/cancel_modal'); ?>
    <?php $this->load->view('repair_jobs/modals/assign_mechanic_modal'); ?>
    <?php $this->load->view('repair_jobs/print_labels/print_label_preview'); ?>
    <?php $this->load->view('repair_jobs/print_reports/print_report_preview'); ?>
    <?php $this->load->view('repair_jobs/send_mail_to_client'); ?>
...
<?php 
require 'modules/workshop/assets/js/repair_jobs/repair_job_detail_js.php';
require 'modules/workshop/assets/js/returns/manage_js.php';
require 'modules/workshop/assets/js/notes/manage_js.php';
