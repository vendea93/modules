	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				$staffid = '';
				if(isset($member)){
					$title .= _l('wshop_update_permissions');
					$staffid    = $member->staffid;
				}else{
					$title .= _l('wshop_add_permissions');
				}
				?>
				<h4 class="modal-title"><?php echo new_html_entity_decode($title); ?></h4>
			</div>
			<?php echo form_open(admin_url('workshop/workshop_update_permissions/'.$staffid), array('id' => 'update_permissions')); ?>
				...
				<div class="table-responsive padding-10">
					...
					<table class="table table-bordered roles no-margin">
						...
						<tbody>
							...
							$workshop_permissions = list_workshop_permisstion();

							foreach(get_available_staff_permissions($funcData) as $feature => $permission) { ?>
								<tr data-name="<?php echo new_html_entity_decode($feature); ?>" class="<?php if(!in_array($feature, $workshop_permissions)){echo "hide";} ?>">
									<td>
...
<?php require('modules/workshop/assets/js/settings/permissions/permission_modal_js.php'); ?>