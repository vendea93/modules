<script type="text/javascript">
    ...
    $(function(){
        ...
        $('#add_edit_note').appFormValidator({
            rules: {
                name: 'required',
        ...
        if($('#dropzoneDragArea').length > 0){
            expenseDropzone = new Dropzone("#add_edit_note", appCreateDropzoneOptions({
                ...
                success:function(file,response){
                    ...
                    if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                        ...
                        if(transaction_type == 'return'){
                            window.location.replace('<?php echo admin_url('workshop/repair_job_detail/'.$repair_job_id.'?tab=return'); ?>');
                        }else{
                            window.location.replace('<?php echo admin_url('workshop/repair_job_detail/'.$repair_job_id.'?tab=delivery'); ?>');
                        }
    ...
    function SubmitHandler(form) {
        ...
        form = $('#add_edit_note');
        var note_id = 0;
        transaction_type = $('#add_edit_note').find('input[name="transaction_type"]').val();
        var formURL = form[0].action;
        var formData = new FormData($(form)[0]);
        formData.append("description", tinyMCE.activeEditor.getContent());

        $('#box-loading').show();
        $('.note_submit_button').attr( "disabled", "disabled" );
            ...
            if (response.note_id) {
                ...
                if(typeof(expenseDropzone) !== 'undefined'){
                    if (expenseDropzone.getQueuedFiles().length > 0) {
                        ...
                        expenseDropzone.options.url = admin_url + 'workshop/add_note_attachment/' + response.note_id;
                        expenseDropzone.processQueue();
