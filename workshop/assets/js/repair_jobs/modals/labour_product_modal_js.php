<script>
    ...
    function add_labour_product(){
        ...
        var hidden_columns = [0,8,9,10,13];
        $('.table-labour_product_table').DataTable().columns(hidden_columns).visible(false, false);
    }

    function add_labour_product_to_table(row, labour_product_id) {
        "use strict";
        var data = labour_product_get_item_preview_values(row, labour_product_id);
      ...
      //show box loading
        ...
        labour_product_get_item_row_template('newlabouritems[' + item_key + ']', 
            data.labour_product_id, data.estimated_hours, item_key, part_item_key).done(function(output){
