<script type="text/javascript">
    ...
    $(function(){
        ...
        $('#add_edit_device').appFormValidator({
            rules: {
                code: 'required',
                name: {
                    required: true,
                    remote: {
                        url: admin_url + "workshop/device_exists",
                        type: 'post',
                        data: {
                            name: function() {
        ...
        if($('#dropzoneDragArea').length > 0){
            expenseDropzone = new Dropzone("#add_edit_device", appCreateDropzoneOptions({
        ...
        $('select[name="model_id"]').on('change', function() {
            ...
            var model_id = $('#add_edit_device select[name="model_id"]').val();
            var device_id = $('#add_edit_device input[name="id"]').val();
            if(device_id == ''){
                device_id = 0;
            }
            if(model_id == ''){
            ...
            requestGet('workshop/get_model_ajax/' + model_id + '/' + device_id).done(function(response) {
                response = JSON.parse(response);
                if (response.success || response.success == true) {
                    $('#add_edit_device #fieldset').html(response.fieldset);
                    init_datepicker();
                    init_selectpicker();
                }
            }).fail(function(error) {
    ...
    function SubmitHandler(form) {
        ...
        $('#box-loading').show();
        $('.device_submit_button').attr( "disabled", "disabled" );
        ...
        $.ajax({
            ...
            $("#deviceModal").modal("hide");
            if($('table.table-device_table').length > 0){
                $('.table-device_table').DataTable().ajax.reload();
            }else{
                location.reload();
            }
    ...
    function readURL(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                $('#deviceModal #wizardPicturePreview').attr('src', e.target.result).fadeIn('slow');
            }
