<?php if(isset($shipment_sign->id)){ ?>
 <div class="col-md-10">  
  ...
  <img src="<?php echo base_url(LOGISTIC_PATH.'consolidation_delivery_shipment/sign/'.$consolidation->id.'/signature_'.$consolidation->id.'.png'); ?>" class="">
  ...
  <?php }elseif(file_exists(LOGISTIC_MODULE_UPLOAD_FOLDER.'/consolidation_delivery_shipment/sign/'.$consolidation->id.'/signature_'.$consolidation->id.'.jpg')){ ?>
     <img src="<?php echo base_url(LOGISTIC_PATH.'consolidation_delivery_shipment/sign/'.$consolidation->id.'/signature_'.$consolidation->id.'.jpg'); ?>" class="">
 ...
 <div class="col-md-2 text-left hide">
   <a href="javascript:void(0);" data-toggle="tooltip" title="<?php echo _l('lg_remove_sign'); ?>" onclick="remove_sign(<?php echo lg_html_entity_decode($consolidation->id); ?>); return false;" class=" text-danger"><i class="fa fa-remove"></i></a>
 </div>
