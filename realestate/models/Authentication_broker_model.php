...
class Authentication_broker_model extends App_Model
{
    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            $table = db_prefix() . 'real_broker_staffs';
            $_id   = 'id';
            ...
            if ($user) {
                ...
                if (!app_hasher()->CheckPassword($password, $user->password ?? '')) {
                    hooks()->do_action('broker_failed_login_attempt', [
                        'user'            => $user,
                        'is_staff_member' => $staff,
                    ]);

                    log_activity('Failed Login Attempt [Email: ' . $email . ', Is Broker: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
                ...
                hooks()->do_action('broker_non_existent_user_login_attempt', [
                        'email'           => $email,
                        'is_staff_member' => $staff,
                ]);

                log_activity('Non Existing User Tried to Login [Email: ' . $email . ', Is Broker: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');

                return false;
            }

            if ($user->active == 0) {
                hooks()->do_action('broker_inactive_user_login_attempt', [
                        'user'            => $user,
                        'is_staff_member' => $staff,
                ]);
                log_activity('Inactive User Tried to Login [Email: ' . $email . ', Is Broker: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
            ...
            hooks()->do_action('broker_before_vendor_login', [
                'email'           => $email,
                'broker_id' => $user->$_id,
                'business_broker_id' => $user->$_company_id,
            ]);
    ...
    public function logout($staff = true)
    {
        ...
        if (is_broker_logged_in()) {
            hooks()->do_action('broker_before_contact_logout', get_broker_id());

            $this->session->unset_userdata('broker_id');
            $this->session->unset_userdata('broker_logged_in');
        } else {
            hooks()->do_action('broker_before_staff_logout', get_staff_user_id());
    ...
    {
        $this->load->helper('cookie');
        if ($cookie = get_cookie('broker_autologin', true)) {
            $data = unserialize($cookie);
            $this->user_autologin->delete($data['user_id'], md5($data['key']), $staff);
            delete_cookie('broker_autologin', 'aal');
        }
    }
    ...
    {
        if (!is_broker_logged_in() && !is_staff_logged_in()) {
            $this->load->helper('cookie');
            if ($cookie = get_cookie('broker_autologin', true)) {
                $data = unserialize($cookie);
                if (isset($data['key']) and isset($data['user_id'])) {
                    if (!is_null($user = $this->user_autologin->get($data['user_id'], md5($data['key'])))) {
                        ...
                        if ($user->staff == 1) {
                            ...
                            $contact = $this->db->get(db_prefix() . 'real_broker_staffs')->row();
    ...
    /**
     * Send set password email for real_broker_staffs
     * @param string $email
     */
    public function set_password_email($email)
    {
        $this->db->where('email', $email);
        $user = $this->db->get(db_prefix() . 'real_broker_staffs')->row();
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                if ($sent) {
                    hooks()->do_action('broker_set_password_email_sent', ['is_staff_member' => false, 'user' => $user]);
    ...
    public function forgot_password($email, $staff = false)
    {
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $html = '';
                $html .= _l('real_Create_New_Password').' <br>';
                $html .= _l('real_Forgot_your_password').' <br>';
                $html .= _l('real_To_create_a_new_password_just_follow_this_link').' <br><br>';
                $html .= '<a href="'.site_url('realestate/authentication_broker/reset_password/0/'.$user->$_id.'/'.$new_pass_key).'">'._l('real_Reset_Password').'</a><br><br>';

                $html .= _l('real_You_received_this_email_because_it_was_requested_by_a').' '.get_option('companyname').'. '._l('real_This_is_part_of_the_procedure_to_create');

                $sent = $this->emails_model->send_simple_email($user->email, _l('real_Create_New_Password'), $html);

                if ($sent) {
                    hooks()->do_action('broker_forgot_password_email_sent', ['is_staff_member' => $staff, 'user' => $user]);
    ...
    public function set_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('User Set Password [User ID: ' . $userid . ', Is Broker: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
    ...
    public function reset_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('User Reseted Password [User ID: ' . $userid . ', Is Broker: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
            $this->db->set('new_pass_key', null);
            $this->db->set('new_pass_key_requested', null);
            $this->db->set('last_password_change', date('Y-m-d H:i:s'));
            $this->db->where($_id, $userid);
            ...
            $sent = send_mail_template('broker_staff_password_resetted', 'realestate', $user->email, $user->$_id);
    ...
    {
        hooks()->do_action('broker_before_staff_login', [
            'email'  => $user->email,
            'userid' => $user->staffid,
        ]);
