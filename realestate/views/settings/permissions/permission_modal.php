	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				$staffid = '';
				if(isset($member)){
					$title .= _l('real_update_permissions');
					$staffid    = $member->staffid;
				}else{
					$title .= _l('real_add_permissions');
				}
				?>
				<h4 class="modal-title"><?php echo new_html_entity_decode($title); ?></h4>
			</div>
			<?php echo form_open(admin_url('realestate/realestate_update_permissions/'.$staffid), array('id' => 'update_permissions')); ?>
			<div class="modal-body">
				...
				<div class="table-responsive padding-10">
					...
					<?php 
					$selected = '';
					if(isset($company_role_id) && $company_role_id != ''){
					...
					?>
					<div class="class <?php echo html_entity_decode($hide_role) ?>">
					...
					<table class="table table-bordered roles no-margin">
						...
						<tbody>
							...
							$realestate_permissions = $list_realestate_permisstion;

							foreach(get_available_staff_permissions($funcData) as $feature => $permission) { ?>
								<tr data-name="<?php echo new_html_entity_decode($feature); ?>" class="<?php if(!in_array($feature, $realestate_permissions)){echo "hide";} ?>">
									<td>
		...
		<div class="modal-footer">
			...
			<?php if(has_permission('real_permission', '', 'create') || has_permission('real_permission', '', 'edit')){ ?>
...
<?php require('modules/realestate/assets/js/settings/permissions/permission_modal_js.php'); ?>