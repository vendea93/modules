	...
	<div class="table-responsive">
		<table class="table table-bordered">
			<thead>
				...
				<th align="right"><?php echo _l('real_rental_price') ?></th>
			</thead>
			<tbody>
				<?php 
				$listing_type = '';
				...
				$index = 0;
				?>
				<?php foreach ($vacant_rentals as $key => $value) { ?>
					...
					<?php 
					$index ++;
					$total_amount += (float)$value['rent_price'];
					...
					<tr>
						...
						<td align="right"><?php echo app_format_money($value['rent_price'], $base_currency_id); ?></td>
					</tr>

					<?php if( (isset($vacant_rentals[$key+1]) && $listing_type != $vacant_rentals[$key+1]['listing_type']) || (count($vacant_rentals) == $key+1)){
