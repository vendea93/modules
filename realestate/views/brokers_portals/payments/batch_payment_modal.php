                    ...
                    <div class="col-sm-4">
                        <div class="form-group select-placeholder">
                            <select id="batch-payment-filter" class="selectpicker" name="client_filter"
                                ...
                                <option value="<?php echo new_html_entity_decode($customer->userid); ?>"><?php echo new_html_entity_decode($customer->company); ?>
                                </option>
                                <?php } ?>
                            </select>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr class="batch_payment_item" data-clientid="<?php echo new_html_entity_decode($invoice->clientid); ?>"
                                        data-invoiceId="<?php echo new_html_entity_decode($invoice->id) ?>">
                                        <td>
                                            ...
                                            <a class="text-dark"
                                                ...
                                                target="_blank">
                                                <?php echo new_html_entity_decode($invoice->company) ?>
                                            </a>

                                            <input type="hidden" name="invoice[<?php echo new_html_entity_decode($index) ?>][invoiceid]"
                                                value="<?php echo new_html_entity_decode($invoice->id) ?>">
                                        </td>
                                        ...
                                        <td>
                                            <div class="form-group">
                                                <select class="selectpicker"
                                                    name="invoice[<?php echo new_html_entity_decode($index) ?>][paymentmode]" data-width="100%"
                                                    data-none-selected-text="-">
                                                    <option></option>
                                                    <?php foreach ($invoice->allowed_payment_modes as $mode) { ?>
                                                    <option value="<?php echo new_html_entity_decode($mode->id); ?>"><?php echo new_html_entity_decode($mode->name); ?>
                                                    </option>
                                                    <?php } ?>
                                                </select>
                                            </div>
