	...
	<nav>
		<div class="tw-flex tw-justify-between">
			<div class="tw-flex tw-flex-1 sm:tw-flex-initial">
				<div id="top_search"
				class="tw-inline-flex tw-relative dropdown sm:tw-ml-1.5 sm:tw-mr-3 tw-max-w-xl tw-flex-auto hide"
				...
				<?php if ($quickActions->isNotEmpty()) { ?>
					...
					<ul class="dropdown-menu dropdown-menu-right animated fadeIn tw-text-base">
						...
						<?php foreach ($quickActions as $key => $item) {
							...
							<li>
								<a href="<?php echo e($url); ?>" <?php echo new_html_entity_decode($href_attributes); ?>
...
<ul class="mobile-icon-menu tw-inline-flex tw-mt-5">
	...
	if (is_mobile()) { ?>
		...
		<?php $this->load->view('brokers_portals/notifications'); ?>
	</li>

<?php } ?>
</ul>
<div class="mobile-navbar collapse" id="mobile-collapse" aria-expanded="false"
role="navigation">
<ul class="nav navbar-nav">
	<li class="header-my-profile"><a href="<?php echo site_url('realestate/broker/add_edit_company'); ?>">
		<?php echo _l('real_broker_prfile'); ?>
	</a>
</li>
<li class="header-my-profile"><a href="<?php echo site_url('realestate/broker/profile'); ?>">
...
<ul class="nav navbar-nav navbar-right">
	...
	<?php hooks()->do_action('broker_navbar_start'); ?>
...
<li class="icon header-user-profile" data-toggle="tooltip" title="<?php echo e(get_broker_name()); ?>"
	data-placement="bottom">
	<a href="#" class="dropdown-toggle profile tw-block rtl:!tw-px-0.5 !tw-py-1" data-toggle="dropdown"
	aria-expanded="false">
	<?php echo broker_profile_image($current_broker->id, ['img', 'img-responsive', 'staff-profile-image-small', 'tw-ring-1 tw-ring-offset-2 tw-ring-primary-500 tw-mx-1 tw-mt-2.5']); ?>
</a>
<ul class="dropdown-menu animated fadeIn">
	<li class="header-my-profile"><a
		href="<?php echo site_url('realestate/broker/add_edit_company'); ?>"><?php echo _l('real_broker_prfile'); ?></a></li>
		<li class="header-my-profile"><a
			href="<?php echo site_url('realestate/broker/profile'); ?>"><?php echo _l('nav_my_profile'); ?></a></li>

			<?php if (!is_language_disabled()) { ?>
				<li class="dropdown-submenu pull-left header-languages">
					<a href="#" tabindex="-1"><?php echo _l('language'); ?></a>
					<ul class="dropdown-menu dropdown-menu">
						<li class="<?php echo new_html_entity_decode($current_broker->default_language == '' ? 'active' : ''); ?>">
							<a href="<?php echo site_url('realestate/broker/change_language'); ?>">
								<?php echo _l('system_default_string'); ?>
							</a>
						</li>
						<?php foreach ($this->app->get_available_languages() as $user_lang) { ?>
							<li
							class="<?php echo new_html_entity_decode($current_broker->default_language == $user_lang ? 'active' : ''); ?>">
							<a href="<?php echo site_url('realestate/broker/change_language/' . $user_lang); ?>">
...
<?php hooks()->do_action('broker_navbar_end'); ?>
</ul>
</div>
</nav>
</div>