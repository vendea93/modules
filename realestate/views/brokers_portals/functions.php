 ...
 */
hooks()->add_action('app_broker_portal_head', 'app_theme_broker_portal_head_hook');
...
register_theme_broker_portal_assets_hook('theme_broker_portal');

function theme_broker_portal()
{
    $CI = &get_instance();
    ...
    $CI->app_scripts->theme('app-v3', 'assets/builds/app.js');
    ...
    if (!is_broker_logged_in()) {
    $CI->app_css->theme('custom-css', 'modules/realestate/assets/css/login.css');
    }
    ...
    $CI->app_css->theme(
        'theme-css',
        base_url($CI->app_scripts->core_file('assets/css', 'style.css')) . '?v=' . $CI->app_css->core_version()
    );
}
