...
<div class="alert alert-info">
	<div class="row">
		<div id="merge" class="col-md-6">
			<?php
			if (isset($invoice)) {
				$this->load->view('brokers_portals/invoices/merge_invoice', ['invoices_to_merge' => $invoices_to_merge]);
			}
			?>
		</div>
		...
		<?php if (!isset($invoice_from_project)) { ?>
			<div id="expenses_to_bill" class="col-md-6">
				<?php if (isset($invoice) && $invoice->status != Invoices_model::STATUS_CANCELLED) {
					$this->load->view('brokers_portals/invoices/bill_expenses', ['expenses_to_bill' => $expenses_to_bill]);
				} ?>
			</div>
		<?php } ?>
	</div>
	...
	<div class="panel-body">
		...
		<div class="row">
			...
			if (!isset($invoice_from_project)) { ?>
				<div class="hide form-group select-placeholder projects-wrapper<?php if ((!isset($invoice)) || (isset($invoice) && !customer_has_projects($invoice->clientid))) {
									...
									<span class="shipping_street">
										...
										<span class="shipping_city">
											...
											<div class="form-group">
												...
												<div class="input-group">
													<span class="input-group-addon">
														<?php if (isset($invoice)) { ?>
															...
															data-content="<label class='control-label'><?php echo _l('settings_sales_invoice_prefix'); ?></label><div class='input-group'><input name='s_prefix' type='text' class='form-control' value='<?php echo new_html_entity_decode($invoice->prefix); ?>'></div><button type='button' onclick='save_sales_number_settings(this); return false;' data-url='<?php echo site_url('realestate/broker/update_number_settings/' . $invoice->id); ?>' class='btn btn-primary btn-block mtop15'><?php echo _l('submit'); ?></button>">
															<i class="fa fa-cog"></i>
														</a>
													<?php }
												...
												data-isedit="<?php echo new_html_entity_decode($isedit); ?>"
												data-original-number="<?php echo new_html_entity_decode($data_original_number); ?>"
												<?php echo ($_is_draft) ? 'disabled' : '' ?>>
												<?php if ($format == 3) { ?>
													<span class="input-group-addon">
														<span id="prefix_year" class="format-n-yy"><?php echo new_html_entity_decode($yy); ?></span>
													</span>
												<?php } elseif ($format == 4) { ?>
													<span class="input-group-addon">
														<span id="prefix_month" class="format-mm-yyyy"><?php echo new_html_entity_decode($mm); ?></span>
														/
														<span id="prefix_year" class="format-mm-yyyy"><?php echo new_html_entity_decode($yyyy); ?></span>
													</span>
												<?php } ?>
											</div>
										</div>
											...
											<div class="form-group hide">
												...
												<div class="form-group mbot15<?= count($payment_modes) > 0 ? ' select-placeholder' : ''; ?>">
													...
													<?php if (count($payment_modes) > 0) { ?>
														<?php 
														$allowed_payment_modes_title =  $this->input->get('allowed_payment_modes');
														 ?>
														<select class="selectpicker"
														data-toggle="<?php echo new_html_entity_decode($allowed_payment_modes_title); ?>"
														...
														<?php foreach ($payment_modes as $mode) {
															...
															<option value="<?php echo new_html_entity_decode($mode['id']); ?>" <?php echo new_html_entity_decode($selected); ?>>
																<?php echo new_html_entity_decode($mode['name']); ?></option>
																<?php
															} ?>
														</select>
													<?php } else { ?>
														...
														<a class="btn btn-primary btn-sm" href="<?php echo site_url('paymentmodes'); ?>">
										...
										<div class="col-md-12">
											...
											<div class="input-group">
												<input type="number" class="form-control" <?php if ($value == 0) {
													echo ' disabled';
												} ?> name="cycles" id="cycles" value="<?php echo new_html_entity_decode($value); ?>" <?php if (isset($invoice) && $invoice->total_cycles > 0) {
						...
						<tbody>
							<tr class="main">
								...
								<td>
									...
									?>
									<button type="button"
									onclick="add_item_to_table('undefined','undefined',<?php echo new_html_entity_decode($new_item); ?>); return false;"
					...
					<div class="row">
						...
						<div class="col-md-5">
							...
							data-title="<?php echo _l('numbers_not_formatted_while_editing'); ?>" value="<?php if (isset($invoice)) {
								echo new_html_entity_decode($invoice->adjustment);
							} else {
								echo 0;
