...
<div class="col-md-12">
	<div class="tw-mb-2 sm:tw-mb-4">
		<div class="_buttons">
			<?php $this->load->view('brokers_portals/invoices/invoices_top_stats'); ?>
			<a href="<?php echo site_url('realestate/broker/invoice'); ?>"
...
<?php foreach ($invoices_statuses as $status) { ?>
	...
	<a href="#" data-cview="invoices_<?php echo new_html_entity_decode($status); ?>"
		onclick="dt_custom_view('invoices_<?php echo new_html_entity_decode($status); ?>','.table-invoices','invoices_<?php echo new_html_entity_decode($status); ?>'); return false;"><?php echo format_invoice_status($status, '', false); ?></a>
	</li>
<?php } ?>
<?php if (count($invoices_years) > 0) { ?>
	...
	<?php foreach ($invoices_years as $year) { ?>
		<li class="active">
			<a href="#" data-cview="year_<?php echo new_html_entity_decode($year['year']); ?>"
				onclick="dt_custom_view(<?php echo new_html_entity_decode($year['year']); ?>,'.table-invoices','year_<?php echo new_html_entity_decode($year['year']); ?>'); return false;"><?php echo new_html_entity_decode($year['year']); ?>
			</a>
		</li>
	<?php } ?>
<?php } ?>
...
<?php foreach ($payment_modes as $mode) {
	...
	<li>
		<a href="#" data-cview="invoice_payments_by_<?php echo new_html_entity_decode($mode['id']); ?>"
			onclick="dt_custom_view('<?php echo new_html_entity_decode($mode['id']); ?>','.table-invoices','invoice_payments_by_<?php echo new_html_entity_decode($mode['id']); ?>'); return false;">
	...
	<div class="col-md-12" id="small-table">
		<div class="panel_s">
			<div class="panel-body panel-table-full">
				...
				<?php $this->load->view('brokers_portals/invoices/table_html'); ?>
			</div>
		</div>
	</div>
