...
if (isset($invoice->scheduled_email) && $invoice->scheduled_email) { ?>
	<div class="alert alert-warning">
		...
		<?php if (staff_can('edit', 'invoices') || $invoice->addedfrom == get_staff_user_id()) { ?>
			<a href="#" onclick="edit_invoice_scheduled_email(<?php echo new_html_entity_decode($invoice->scheduled_email->id); ?>); return false;">
				<?php echo _l('edit'); ?>
			</a>
		<?php } ?>
	</div>
...
<div id="invoice-preview">
		...
		if ($invoice->recurring > 0 || $invoice->is_recurring_from != null) {
			...
			<div class="col-md-12">
				<div class="mbot10">
					<?php if ($invoice->is_recurring_from == null
						...
						<?php } elseif ($show_recurring_invoice_info) { ?>
							<span class="label label-info">
								...
								<b>
									<?php
									echo new_html_entity_decode($recurring_invoice->cycles == 0 ? _l('cycles_infinity') : $recurring_invoice->cycles - $recurring_invoice->total_cycles);
									?>
								</b>
							</span>
							<?php
					...
					<?php if ($invoice->is_recurring_from != null) { ?>
						<?php echo '<p class="text-muted' . ($show_recurring_invoice_info ? ' mtop15': '') . '">' . _l('invoice_recurring_from', '<a href="' . site_url('realestate/broker/list_invoices/' . $invoice->is_recurring_from) . '" onclick="init_invoice(' . $invoice->is_recurring_from . ');return false;">' . format_invoice_number($invoice->is_recurring_from) . '</a></p>'); ?>
					<?php } ?>
				</div>
			...
			<div class="col-md-6 col-sm-6">
				<h4 class="bold">
					...
					?>
					<a href="<?php echo site_url('realestate/broker/invoice/' . $invoice->id); ?>">
				...
				<address>
					<?php echo real_get_company_name($invoice->broker_id, true, false, true); ?>
				</address>
			...
			<div class="col-sm-6 text-right">
				...
				foreach ($pdf_custom_fields as $field) {
					...
					<p class="no-mbot">
						<span class="bold"><?php echo new_html_entity_decode($field['name']); ?>: </span>
						<?php echo new_html_entity_decode($value); ?>
					</p>
					<?php
				} ?>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="table-responsive">
					...
					echo new_html_entity_decode($items->table());
					?>
				</div>
			</div>
			<div class="col-md-5 col-md-offset-7">
				<table class="table text-right">
					...
					<?php if (get_option('show_amount_due_on_invoice') == 1 && $invoice->status != Invoices_model::STATUS_CANCELLED) { ?>
						<tr>
							<td>
								<span class="<?php echo new_html_entity_decode($invoice->total_left_to_pay > 0 ? 'text-danger ': ''); ?> bold">
									<?php echo _l('invoice_amount_due'); ?>
								</span>
							</td>
							<td>
								<span class="<?php echo new_html_entity_decode($invoice->total_left_to_pay > 0 ? 'text-danger ': ''); ?>">
	...
	<?php if (count($invoice->attachments) > 0) { ?>
		...
		<?php foreach ($invoice->attachments as $attachment) {
			...
			<div class="mbot15 row inline-block full-width" data-attachment-id="<?php echo new_html_entity_decode($attachment['id']); ?>">
				<div class="col-md-8">
					<div class="pull-left"><i class="<?php echo get_mime_class($attachment['filetype']); ?>"></i></div>
					<a href="<?php echo new_html_entity_decode($attachment_url); ?>" target="_blank"><?php echo new_html_entity_decode($attachment['file_name']); ?></a>
					<br />
					<small class="text-muted"> <?php echo new_html_entity_decode($attachment['filetype']); ?></small>
				</div>
				<div class="col-md-4 text-right tw-space-x-2">
					...
					onclick="toggle_file_visibility(<?php echo new_html_entity_decode($attachment['id']); ?>,<?php echo new_html_entity_decode($invoice->id); ?>,this); return false;"
					data-title="<?php echo new_html_entity_decode($tooltip); ?>"><i class="fa <?php echo new_html_entity_decode($icon); ?> fa-lg"
						aria-hidden="true"></i></a>
						
						<a href="#" class="text-danger"
						onclick="delete_invoice_attachment(<?php echo new_html_entity_decode($attachment['id']); ?>); return false;"><i
		...
		<?php if ($invoice->clientnote != '') { ?>
			<div class="col-md-12 row mtop15">
				...
				<p><?php echo new_html_entity_decode($invoice->clientnote); ?></p>
			</div>
		<?php } ?>
		<?php if ($invoice->terms != '') { ?>
			<div class="col-md-12 row mtop15">
				<p class="bold text-muted"><?php echo _l('terms_and_conditions'); ?></p>
				<p><?php echo new_html_entity_decode($invoice->terms); ?></p>
			</div>
		<?php } ?>
	</div>