            ...
            <div class="tw-px-3 tw-py-4 sm:tw-p-4">
                ...
                <dd class="tw-mt-1 tw-flex tw-items-baseline tw-justify-between md:tw-block lg:tw-flex">
                    <div class="tw-flex tw-items-baseline tw-text-base tw-font-semibold tw-text-primary-600">
                        <?php echo new_html_entity_decode($total_by_status); ?> / <?php echo new_html_entity_decode($total_invoices); ?>
                        <span class="tw-ml-2 tw-text-sm tw-font-medium tw-text-neutral-500">
                            <a href="#" data-cview="invoices_<?php echo new_html_entity_decode($status); ?>"
                                onclick="dt_custom_view('invoices_<?php echo new_html_entity_decode($status); ?>','.table-invoices','invoices_<?php echo new_html_entity_decode($status); ?>',true); return false;">
                                <?php echo _l('view'); ?>
                            </a>
                        </span>
                    </div>
                    <div class="tw-font-medium md:tw-mt-2 lg:tw-mt-0">
                        <?php echo new_html_entity_decode($percent); ?>%
                    </div>
                </dd>
            </div>
            <?php
