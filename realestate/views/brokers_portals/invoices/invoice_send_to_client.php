...
<div class="modal fade email-template" data-editor-id=".<?php echo 'tinymce-' . $invoice->id; ?>" id="invoice_send_to_client_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php echo form_open('realestate/broker/send_invoice_to_email/' . $invoice->id); ?>
                        ...
                        <?php foreach ($invoice->attachments as $attachment) {
                            ...
                            <div class="checkbox checkbox-primary">
                                <input type="checkbox" <?php if (!empty($attachment['external'])) {
                                    echo 'disabled';
                                }; ?> value="<?php echo new_html_entity_decode($attachment['id']); ?>" name="email_attachments[]">
                                <label for=""><a href="<?php echo site_url('download/file/sales_attachment/' . $attachment['attachment_key']); ?>"><?php echo new_html_entity_decode($attachment['file_name']); ?></a></label>
                            </div>
                        <?php
                            } ?>
                    </div>
...
<?php 
require 'modules/realestate/assets/js/brokers/invoices/invoice_send_to_client_js.php';
?>
