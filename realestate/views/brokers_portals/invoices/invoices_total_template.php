...
<div class="tw-flex tw-space-x-3 tw-mb-2">
    ...
    if (!$this->input->post('currency')) {
        ...
        if ($this->input->post('currency') == $currency['id']) {
            ...
            <option value="<?php echo new_html_entity_decode($currency['id']); ?>" <?php echo new_html_entity_decode($selected); ?>
                data-subtext="<?php echo new_html_entity_decode($currency['name']); ?>">
                <?php echo new_html_entity_decode($currency['symbol']); ?>
            </option>
            <?php
} ?>
        </select>
    ...
    <div class="simple-bootstrap-select !tw-max-w-xs">
        <select data-none-selected-text="<?php echo date('Y'); ?>" data-width="auto" class="selectpicker tw-w-full"
            ...
            <option value="<?php echo new_html_entity_decode($year['year']); ?>" <?php if ($this->input->post('years') && in_array($year['year'], $this->input->post('years')) || !$this->input->post('years') && date('Y') == $year['year']) {
        echo ' selected';
    } ?>>
                <?php echo new_html_entity_decode($year['year']); ?>
            </option>
            <?php } ?>
        </select>
    </div>
