...
if ($clientid != '') {
    ...
    array_push($where, 'AND ' . db_prefix() . 'contracts.broker_id=' . get_business_broker_id());
...
foreach ($rResult as $aRow) {
    ...
    $subjectOutput = '<a href="' . site_url('realestate/broker/contract/' . $aRow['id']) . '"' . ($projectId ? ' target="_blank"' : '') . '>' . $aRow['subject'] . '</a>';
    if ($aRow['trash'] == 1) {
    ...
    $subjectOutput .= '<a href="' . site_url('contract/' . $aRow['id'] . '/' . $aRow['hash']) . '" target="_blank">' . _l('view') . '</a>';
        ...
        $subjectOutput .= ' | <a href="' . site_url('realestate/broker/contract/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    
        $subjectOutput .= ' | <a href="' . site_url('realestate/broker/contract_delete/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
