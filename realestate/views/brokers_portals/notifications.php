...
<?php echo form_open_multipart(site_url('realestate/broker_portal'), array('id' => 'search_job')); ?>
<?php echo form_close(); ?>
...
<a href="#" class="dropdown-toggle notifications-icon" data-toggle="dropdown" aria-expanded="false">
  ...
  <?php if($current_broker->total_unread_notifications > 0){ ?>
    <span class="label icon-total-indicator bg-warning icon-notifications"><?php echo new_html_entity_decode($current_broker->total_unread_notifications); ?></span>
  <?php } ?>
</a>
<ul class="dropdown-menu notifications animated fadeIn width400" data-total-unread="<?php echo new_html_entity_decode($current_broker->total_unread_notifications); ?>">
  ...
  <?php
  $_notifications = $this->realestate_model->get_user_notifications();
      ...
      <?php if(!empty($notification['link'])){ ?>
        ...
        <div class="notification-box<?php if($notification['isread_inline'] == 0){echo ' unread';} ?>">
          ...
          if(($notification['fromcompany'] == NULL && $notification['fromuserid'] != 0) || ($notification['fromcompany'] == NULL && $notification['fromclientid'] != 0)){
            ...
            echo '<img src="'.broker_profile_image_url($notification['fromclientid']).'" class="client-profile-image-small img-circle pull-left notification-image">';
          }
        }
        ?>
...
<li class="text-center">
  <?php if(count($_notifications) > 0){ ?>
    <a href="<?php echo site_url('realestate/broker_portal/notifications_detail'); ?>"><?php echo _l('nav_view_all_notifications'); ?></a>
  <?php } else { ?>
