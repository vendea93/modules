<script>
var all_marker = [],markers = [], drawing_filter = [], zoom_scale = 10, all_overlays = [];
<?php if(isset($map_property_listing)){ ?>
	all_marker = <?php echo json_encode($map_property_listing); ?>;
<?php } ?>
...
function show_map_marker(map, markers){
	...
	for( i = 0; i < markers.length; i++ ) {
		...
		const infowindow = new google.maps.InfoWindow({
			...
			'<h5 class="!tw-text-base tw-max-w-[350px]">'+mk.address+'</h5>'+
			'<a href="#"><span class="no-mleft">'+mk.sqFt_total+' . '+mk.property_style+'</span></a>'+
