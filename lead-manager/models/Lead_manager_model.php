...
class Lead_manager_model extends App_Model
{
    ...
    public function getOld($id = '', $where = [], $order_by = [], $limit = [])
    {
     ...
     public function get($id = '', $where = [], $order_by = [], $limit = [], $is_whatsapp=false)
    {
        $custom_fields = get_table_custom_fields('leads');
        $this->db->select(db_prefix() . 'leads.*, ' . db_prefix() . 'leads.name as lead_name, ' . db_prefix() . 'leads.id as lead_id, ' . db_prefix() . 'leads_status.name as status_name, ' . db_prefix() . 'leads_sources.name as source_name');
        $this->db->from(db_prefix() . 'leads');
        $this->db->join(db_prefix() . 'leads_status', db_prefix() . 'leads_status.id=' . db_prefix() . 'leads.status', 'left');
        $this->db->join(db_prefix() . 'leads_sources', db_prefix() . 'leads_sources.id=' . db_prefix() . 'leads.source', 'left');
        if ($is_whatsapp) {
            $this->db->join(db_prefix() . 'lead_manager_whatsapp as chats',
                '(chats.from_id=' . db_prefix() . 'leads.id OR chats.to_id=' . db_prefix() . 'leads.id) AND chats.is_client=0 AND chats.id = (SELECT MAX(id) FROM ' . db_prefix() . 'lead_manager_whatsapp WHERE from_id = ' . db_prefix() . 'leads.id OR to_id = ' . db_prefix() . 'leads.id)',
                'left'
            );
        }else{
            $this->db->join(db_prefix() . 'lead_manager_conversation as chats',
                '(chats.from_id=' . db_prefix() . 'leads.id OR chats.to_id=' . db_prefix() . 'leads.id) AND chats.is_client=0 AND chats.id = (SELECT MAX(id) FROM ' . db_prefix() . 'lead_manager_conversation WHERE from_id = ' . db_prefix() . 'leads.id OR to_id = ' . db_prefix() . 'leads.id)',
                'left'
            );
        }
        foreach ($custom_fields as $key => $field) {
            $selectAs = (is_cf_date($field) ? 'date_picker_cvalue_' . $key : 'cvalue_' . $key);
            $this->db->select('ctable_' . $key . '.value as ' . $selectAs, FALSE); // Add custom field select
            $this->db->join(db_prefix() . 'customfieldsvalues as ctable_' . $key,
                db_prefix() . 'leads.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id'],
                'left'
            );
            if ($is_whatsapp && $field['slug'] === 'leads_whatsapp_enable') { 
                $this->db->where('ctable_' . $key . '.value', 'Enable');
            }
        }
        if (isset($where) && !empty($where)) {
            $this->db->where($where);
        ...
        if (is_numeric($id)) {
            ...
            if ($lead) {
                ...
                foreach ($custom_fields as $key => $field) {
                    $selectAs = (is_cf_date($field) ? 'date_picker_cvalue_' . $key : 'cvalue_' . $key);
                    $lead->{$field['slug']} = $lead->{$selectAs}; // Access by field slug
                }
            }
            return $lead;
        }
        ...
        foreach ($results as $key => $lead) {
            foreach ($custom_fields as $cf_key => $field) {
                $selectAs = (is_cf_date($field) ? 'date_picker_cvalue_' . $cf_key : 'cvalue_' . $cf_key);
                $results[$key][$field['slug']] = $lead[$selectAs]; // Access by field slug
            }
        }
        return $results;
    }
    ...
    public function save_meeting_remark($data)
    {
        if ($data) {
            $saveData = array(
                ...
                'lm_follow_up_date' => isset($data['lm_follow_up']) ? to_sql_date($data['lm_follow_up'], true) : NULL
            );
...
{
    $variables = [];
    if (isset($data['var_key']) && count($data['var_key']) > 0 && isset($data['var_val']) && count($data['var_val']) > 0) {
        $varKey = $data['var_key'];
        $varVal = $data['var_val'];
        foreach ($varKey as $index => $key) {
            if (isset($varVal[$index])) {
                $variables[$key] = $varVal[$index];
            }
        }
        $data['content_variables'] = json_encode($variables); 
        unset($data['var_key']); 
        unset($data['var_val']);
    }
...
public function save_zoom_access_token($data)
{
    if ($data && isset($data['staffid'])) {
        $existingRecord = $this->db->get_where(
            db_prefix() . 'lead_manager_zoom_meeting_access_token',
            array('staff_id' => $data['staffid'])
        )->row();
        if ($existingRecord) {
            $update_data = array(
...
public function add_or_update_ai_configuration($data)
{
    $gpt_api_key = get_option('lm_api_key');
    if (empty($data) || empty($data['settings'])) {
        return ['success' => false, 'message' => 'Invalid data provided.'];
    }
    if (!empty($data['settings']['lm_asistant_id'])) {
        $assistant_response = update_gpt_assistant($gpt_api_key, $data);

    } else {
        $assistant_response = create_gpt_assistant($gpt_api_key, $data);

    }
    if (isset($assistant_response) && !empty($assistant_response->id)) {
        update_option('lm_asistant_id', $assistant_response->id);
        update_option('lm_asistant_name', $assistant_response->name);
        update_option('lm_asistant_prompt', $assistant_response->instructions);
        return ['success' => true, 'message' => 'Configuration updated successfully.'];

    } else {

        return ['success' => false, 'message' => 'Failed to communicate with the GPT API.'];
    }
}


public function is_customer($customer_email)
{
...
public function add_form($data)
{
    ...
    $this->db->insert(db_prefix() . 'lm_ai_chat_form', $data);
    $insert_id = $this->db->insert_id();
    if ($insert_id) {
        log_activity('New  Form Added [' . $data['name'] . ']');
...
public function get_chat_form($where)
{
    $this->db->where($where);

    return $this->db->get(db_prefix() . 'lm_ai_chat_form')->row();
}
public function get_chat_form_id()
{
    $this->db->select('id');
    return $this->db->get(db_prefix() . 'lm_ai_chat_form')->row();
}
public function get_chat_form_key()
{
...
public function generate_form_key()
{
    ...
    $insert_id = $this->db->insert_id();
    if ($insert_id) {
        log_activity('New  Form  Key Generated  [' . $data['form_key'] . ']');
        return $insert_id;
    }
