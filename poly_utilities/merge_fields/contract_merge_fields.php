...
class Contract_merge_fields extends App_merge_fields
{
    public function build()
    {
        return [
            ...
            [
                'name'      => 'Contract Short Link',
                'key'       => '{contract_short_url}',
                'available' => [
                    'contract',
                ],
            ],
    ...
    public function format($contract_id)
    {
        $fields = [];
        $this->ci->db->select(
            db_prefix() . 'contracts.id as id, subject, description, datestart, dateend, contract_value, hash, project_id, ' .
            db_prefix() . 'contracts_types.name as type_name,' . db_prefix() . 'contracts.dateadded as created_at'
        );
        $this->ci->db->where('contracts.id', $contract_id);
        $this->ci->db->join(
            db_prefix() . 'contracts_types',
            db_prefix() . 'contracts_types.id = ' . db_prefix() . 'contracts.contract_type',
            'left'
        );
        ...
        $fields['{contract_description}']    = nl2br((string) ($contract->description ?? ''));
