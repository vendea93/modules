...
foreach ($rResult as $aRow) {
    ...
    if (has_permission('reputation_vendors', '', 'delete') || rep_is_vendor_admin($aRow['userid']) || is_admin()) {
        if ($aRow['is_primary'] == 0 || ($aRow['is_primary'] == 1 && $total_client_contacts == 1)) {
            $rowName .= ' | <a href="' . admin_url('reputation/delete_vendor_contact/' . $aRow['userid'] . '/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
        }
    }
    ...
    $outputActive = '<div class="onoffswitch">
                <input type="checkbox"' . ((total_rows(db_prefix() . 'pur_vendor', 'registration_confirmed=0 AND userid=' . $aRow['userid']) > 0) || $disabled != '' ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'reputation/change_contact_status" name="onoffswitch" class="onoffswitch-checkbox" id="c_' . $aRow['id'] . '" data-id="' . $aRow['id'] . '"' . ($aRow['active'] == 1 ? ' checked': '') . '>
