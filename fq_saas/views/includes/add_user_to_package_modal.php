    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?php echo _l('fq_saas_add_user_to_package_modal_title'); ?></h4>
            </div>
            <div class="modal-body tw-m-0">


                <!--- add user to package form -->
                <?= form_open(admin_url(FQ_SAAS_ROUTE_NAME . '/packages/add_user_to_package'), ['id' => 'add_user_to_package_form']); ?>
                <!-- contact selection -->
                <div class="form-group">
                    <label for="packageid" class="control-label"><?php echo _l('fq_saas_package'); ?></label>
                    <select id="packageid" name="packageid" class="form-control">
                        <?php foreach (get_instance()->fq_saas_model->packages() as $package) : ?>
                ...
                <div class="form-group select-placeholder">
                    <label for="clientid" class="control-label"><?php echo _l('fq_saas_invoice_select_customer'); ?></label>
                ...
                <div class="form-group">
                    <button class="btn btn-primary"><?= _l('fq_saas_submit'); ?></button>
                </div>
                <?= form_close(); ?>
    ...
    window.addEventListener('load', function() {
        ...
        let modalButton = `
            <button type="button" class="btn btn-danger tw-ml-2" data-target="#add-package-modal" data-toggle="modal">
                <span class="btn-with-tooltip" data-toggle="tooltip" data-title="<?php echo _l('fq_saas_add_user_to_package_btn'); ?>"
                    data-placement="bottom">
                    <i class="fa-regular fa-plus"></i>
                    <?php echo _l('fq_saas_add_user_to_package_btn'); ?>
                </span>
            </button>`;
