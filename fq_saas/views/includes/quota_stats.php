...
<?php
$tenant = !empty($tenant) ? $tenant : fq_saas_tenant();
$package_quota = $tenant->package_invoice->metadata->limitations ?? [];
$usage_limits = fq_saas_get_tenant_quota_usage($tenant);
...
<div class="widget relative tw-mb-4" id="widget-saas_top_stats"
    data-name="<?= _l('fq_saas_tenant_quota_dashboard'); ?>">
    <div class="widget-dragger ui-sortable-handle"></div>

    <div class="tw-flex tw-justify-between tw-items-center">

        <h4><?= _l('fq_saas_tenant_quota_dashboard'); ?></h4>

        <!-- customize package quota accessibility link -->
        <?php if (fq_saas_is_single_package_mode() || ($tenant->package_invoice->metadata->allow_customization ?? 'yes') !== 'no') {
            $target = '';
            $url = base_url('clients/my_account');
            if (fq_saas_is_tenant()) {
                $target = '_blank';
                $url = fq_saas_tenant_is_enabled('client_bridge') ? admin_url('billing/my_account?redirect=clients/my_account') : fq_saas_default_base_url('clients/my_account');
            }
        ?>
        <a href="<?= $url; ?>" class="" target="<?= $target; ?>">
            <?= _l('fq_saas_pricing_customize'); ?> <i class="fa fa-edit"></i>
        </a>
        <?php } ?>
    </div>
    ...
    <div class="row">
        ...
        <?php
            $quota = fq_saas_tenant_resources_quota($tenant, $resources);
        ...
        $unlimited = fq_saas_tenant_storage_is_unlimited($tenant);
        ...
        <?php if ($display_storage) {
            $quota_in_byte = $unlimited ? 0 : fq_saas_convert_formatted_size_to_bytes($quota);
            $usage_in_byte = fq_saas_tenant_used_storage($tenant);
            $usage = fq_saas_format_storage_size($usage_in_byte);
