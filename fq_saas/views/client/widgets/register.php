...
$package_slug = $CI->session->{fq_saas_route_id_prefix('plan')} ?? '';
if (!empty($package_slug)) {
    $CI->db->where('slug', $package_slug);
    $package = $CI->fq_saas_model->packages()[0] ?? [];
} else {
...
$can_use_subdomain = (int)$package->metadata->enable_subdomain && (int)fq_saas_get_options('fq_saas_enable_subdomain_input_on_signup_form');
$can_use_customdomain = (int)$package->metadata->enable_custom_domain && (int)fq_saas_get_options('fq_saas_enable_customdomain_input_on_signup_form');
?>
...
<div class="form-group mtop15 register-saas-info-group" style="display: none;">
    <!-- slug -->
    <?= $can_use_subdomain ? render_input('slug', _l('fq_saas_register_form_subdomain_id') . fq_saas_form_label_hint('fq_saas_create_company_slug_hint', fq_saas_get_saas_default_host()), '', 'text', ['maxlength' => FQ_SAAS_MAX_SLUG_LENGTH], [], "text-left tw-mb-4 ", '') : ''; ?>

    <!-- custom domain -->
    <?= $can_use_customdomain ? render_input('custom_domain', _l('fq_saas_register_form_custom_domain') . fq_saas_form_label_hint('fq_saas_custom_domain_hint'), '', 'text', [], [], "text-left tw-mb-4", '') : ''; ?>

    <!-- package slug -->
    <input type="hidden" value="<?= $package->slug; ?>" name="<?= fq_saas_route_id_prefix('plan');?>" />
</div>
