...
$company_options = [];
$pending_custom_domain_notice = "<span data-toggle='tooltip' data-title='" . _l("fq_saas_domain_pending") . "'><i class='fa fa-warning text-danger'></i></span>";
$next_slug = empty($companies) ? fq_saas_generate_unique_slug(get_client(get_client_user_id())->company, 'companies') : '';
...
$on_trial = $invoice ? fq_saas_invoice_is_on_trial($invoice) : false;
...
$theme_name = $allow_deploy_splash && ($total_companies === 1 && in_array($companies[0]->status, [FQ_SAAS_STATUS_PENDING])) ? '_deploy_progress' : $theme_name;
...
$view_path = module_dir_path(FQ_SAAS_MODULE_NAME, 'views');
?>

<div class="ps-container <?= $theme_name; ?>-theme">

    <?php if (fq_saas_contact_can_manage_subscription()) : ?>
    ...
    <?php if (fq_saas_contact_can_manage_instances()) : ?>
