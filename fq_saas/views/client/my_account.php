...
$billing_cycle = strtolower(($interval > 1 ? $interval : '') . _l($interval > 1 ? "fq_saas_" . $interval_type : "fq_saas_" . $interval_type . '_singular'));
if (!empty($package->metadata->is_liftetime_deal))
    $billing_cycle = _l('fq_saas_interval_lifetime');

$subscribed = !empty($invoice->{fq_saas_column('packageid')}) && $invoice->{fq_saas_column('packageid')} == $package->id;
...
if (isset($package->metadata->storage_limit->unit)) {
    ...
    $limitations = array_merge(
        ...
        ['storage' => $storage === -1 ? _l('fq_saas_unlimited') : $storage . ' ' . ($package->metadata->storage_limit->unit ?? 'B')],
    );
}
...
$modules = $CI->fq_saas_model->modules();
$purchased_modules = $invoice->purchased_modules;
$allow_module_request = get_option('fq_saas_enable_custom_module_request') == "1";
$module_request_url = get_option('fq_saas_custom_module_request_form');
$module_request_url = empty($module_request_url) ? site_url('clients/open_ticket?request_custom_module&title=' . _l('fq_saas_custom_module_request')) : $module_request_url;
$allow_module_marketplace = ($package->metadata->disable_module_marketplace ?? '') !== 'yes';

$services = $CI->fq_saas_model->services();
$allow_service_request = get_option('fq_saas_enable_custom_service_request') == "1";
$service_request_url = get_option('fq_saas_custom_service_request_form');
$service_request_url = empty($service_request_url) ? site_url('clients/open_ticket?request_custom_service&title=' . _l('fq_saas_custom_service_request')) : $service_request_url;
...
<div class="panel ps">
    <div class="panel-body min-h-50-vh">
        <div class="tw-flex tw-flex-col tw-justify-center tw-items-center tw-mb-3">
            <h1 class="tw-mt-0">
                <?php if ($subscribed) : ?>
                    <i class="fa fa-check-circle text-success <?= fq_saas_is_single_package_mode() ? 'fa-2x' : ''; ?>"></i>
                <?php endif; ?>
                <?php if (!fq_saas_is_single_package_mode()) echo $package->name; ?>
            </h1>
            <div>
            ...
            <div class="col-md-8 col-lg-9">
                <div class="table-responsive">
                    <table class="table table-bordered table-condensed tw-mb-1">
                        <thead>
                            <tr>
                                <th><?= _l('fq_saas_feature_or_module'); ?></th>
                                <th><?= _l('fq_saas_current_limit'); ?></th>
                                <th><?= _l('fq_saas_extra_limit'); ?></th>
                                <th><?= _l('fq_saas_unit_price'); ?>/<?= $billing_cycle; ?></th>
                                <th><?= _l('fq_saas_price_addition'); ?>/<?= $billing_cycle; ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    <td><?= _l('fq_saas_limit_' . $resources); ?></td>
                                    <td><?= $limit; ?></td>
                                    <td>
                ...
                <?php if (!empty((array)$purchased_services) || $allow_service_marketplace) : ?>
                    <p><?= _l('fq_saas_package_custom_table_notice'); ?></p>
                <?php endif ?>
            </div>
            <div class="col-md-3">
                <div class="summary-card">
                    <div class="tw-bg-neutral-200 tw-p-1 tw-mb-4">
                        <table class="table table-condensed">
                            <tfoot>
                                ...
                                <tr>
                                    <th colspan="4"><?= _l('fq_saas_total'); ?></th>
                    ...
                    <div class="tw-flex tw-flex-col tw-gap-2 tw-justify-end tw-w-full">
                        <?php if ($can_customize) : ?>
                            <button class="btn btn-success tw-w-full" onclick="return confirm('<?= fq_saas_ecape_js_attr(_l('fq_saas_confirm_customize_package')); ?>');"><?= _l($on_trial ? 'fq_saas_subscribe' : 'fq_saas_update_subscription'); ?></button>
                        <?php endif; ?>
                        <?php if ((int)get_option('fq_saas_allow_customer_cancel_subscription') && !$on_trial) : ?>
