...
if ($clientid)
    $invoice =  get_instance()->fq_saas_model->get_company_invoice($clientid);

$alternative_host = fq_saas_get_saas_alternative_host();
$fq_saas_default_host = empty($alternative_host) ? fq_saas_get_saas_default_host() : $alternative_host;
?>
...
const SAAS_DEFAULT_HOST = '<?= $fq_saas_default_host; ?>';
...
const SAAS_CONTROL_CLIENT_MENU = <?= (int)get_option('fq_saas_control_client_menu'); ?>;
const SAAS_MAX_SLUG_LENGTH = <?= FQ_SAAS_MAX_SLUG_LENGTH; ?>;
</script>

<!-- Load client panel script and style -->
<script src="<?= fq_saas_asset_url('js/client.js'); ?>"></script>
<script>
...
<link rel="stylesheet" type="text/css" href="<?= fq_saas_asset_url('css/client.css'); ?>" />

<!-- style control for client menu visibility -->
<?php if ((int)get_option('fq_saas_control_client_menu')) : ?>
<style>
...
// NOTE: input is base64 + urlencoded then decoded here, so CI's XSS filter on the raw string
// is meaningless. We json_encode the final decoded value when injecting into JS below.
$portal_message_value = $CI->input->get('portal-message-value', true);
if (!empty($portal_message_value))
    $portal_message_value = urldecode(base64_decode($portal_message_value));
if (!is_string($portal_message_value)) $portal_message_value = '';
...
const SINGLE_PORTAL_TARGET_ORIGIN = <?= json_encode((string) $portal_origin, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES); ?>;
const SINGLE_PORTAL_MESSAGE = <?= json_encode((string) $portal_message, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES); ?>;
const SINGLE_PORTAL_MESSAGE_VALUE = <?= json_encode((string) $portal_message_value, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES); ?>;
const SAAS_SINGLE_PORTAL_ORIGIN = <?= json_encode((string) (parse_url(fq_saas_default_base_url())['host'] ?? ''), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES); ?>;

const SAAS_SINGLE_PORTAL_TARGET_IS_CUSTOM_DOMAIN = !SINGLE_PORTAL_TARGET_ORIGIN.includes(SAAS_SINGLE_PORTAL_ORIGIN);
const SAAS_CUSTOM_DOMAIN_CAN_USE_SINGLE_PORTAL = "<?= get_option('fq_saas_enable_cross_domain_bridge'); ?>" == "1";
</script>
