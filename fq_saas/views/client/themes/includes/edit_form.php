...
<div class="edit-form tw-w-full text-left <?= $theme_name == 'single' ? 'panel panel-body' : 'tw-mt-4'; ?>"
    ...
    <?= render_input('name', 'fq_saas_company_name', $company->name, 'text', [], [], "text-left tw-mb-4 $centered_size", $input_class); ?>

    <!-- disabled modules selection -->
    <?php $selected = (!empty($company->metadata->disabled_modules) ? (array)$company->metadata->disabled_modules : []); ?>
    <?php
    $modules = fq_saas_tenant_modules($company, false, true);
    $_modules = $CI->fq_saas_model->modules();
    ?>
    <?php $label = _l('fq_saas_disabled_modules') . fq_saas_form_label_hint('fq_saas_disabled_modules_hint'); ?>
    ...
    <div class="text-center">
        ...
        <button type="submit" data-loading-text="<?= _l('fq_saas_saving...'); ?>" data-form="#company_edit_form"
            class="btn btn-primary mtop15 mbot15"><?= _l('fq_saas_submit'); ?></button>
    </div>
    <?= form_close(); ?>
</div>
<!-- end edit form -->