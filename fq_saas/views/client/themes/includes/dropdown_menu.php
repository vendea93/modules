...
<div class="dropdown tw-pr-2">
    <a href="#" class="dropdown-toggle tw-pr-2" data-toggle="dropdown" role="button" aria-haspopup="true"
        aria-expanded="false">
        <i class="fa fa-ellipsis-v fa-2x" data-toggle="tooltip" data-title="<?= _l('fq_saas_more_option'); ?>"
            data-placement="bottom" data-original-title="" title=""></i>
    </a>
    <ul class="dropdown-menu animated fadeIn">
        <?php if ($can_use_custom_domain) : ?>
        <li class="edit-custom-domain-nav">
            <a href="#custom-domain" onclick="return false;"><?= _l('fq_saas_client_add_custom_domain'); ?></a>
        </li>
        <?php endif; ?>
        <li class="edit-company-nav">
            <a href="#company_edit_form" onclick="return false;"><?= _l('fq_saas_client_edit_company'); ?></a>
        </li>
        <li class="customers-nav-item-logout">
            <?php if ($company->status != FQ_SAAS_STATUS_PENDING && $company->status !== FQ_SAAS_STATUS_PENDING_DELETE) : ?>
