...
<div class="custom-domain-form tw-w-full text-left <?= $theme_name == 'single' ? 'panel panel-body' : 'tw-mt-4'; ?>" style="display:none">
    ...
    <?= render_input('custom_domain', _l('fq_saas_custom_domain') . fq_saas_form_label_hint('fq_saas_custom_domain_hint'), $company->custom_domain, 'text', [], [], "text-left tw-mb-4 $centered_size", $input_class); ?>

    <div class="text-center">
        <button type="button" class="btn btn-default mtop15 mbot15 close-btn"><?= _l('fq_saas_cancel'); ?></button>
        <button type="submit" data-loading-text="<?= _l('fq_saas_saving...'); ?>" data-form="#company_custom_domain" class="btn btn-primary mtop15 mbot15"><?= _l('fq_saas_submit'); ?></button>
    </div>
    <?= form_close(); ?>

    <?php
    $_key = 'fq_saas_custom_domain_guide';
    $_alt_key = 'fq_saas_custom_domain_guide_option';
    ...
    $urls = fq_saas_tenant_base_url($company, '', 'all');
    ...
    ?>
                    ...
                    <div class="panel-heading">
                        <h4 class="panel-title">
                            <a class="accordion-toggle tw-w-full tw-inline-block collapsed" data-toggle="collapse" data-parent="#accordion" href="#<?= $company->slug; ?>-dns-guide">
                                <?= _l('fq_saas_custom_domain_guide'); ?>
                            </a>
                        </h4>
                    </div>
        ...
        <?php else : ?>
            <button type="button" class="tw-w-full tw-inline-block btn btn-info tw-bg-transparent hover:tw-bg-transparent tw-truncate tw-mt-9 tw-py-1 tw-px-1" data-toggle="modal" data-target="#<?= $company->slug; ?>-dns-guide" class="" data-dismiss="modal">
                <span class="text-info"><?= _l('fq_saas_custom_domain_guide'); ?> <i class="fa fa-question-circle"></i></span>
            </button>
