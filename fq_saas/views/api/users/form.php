...
<div id="wrapper">
    <div class="content">
        <?= form_open(admin_url(FQ_SAAS_ROUTE_NAME . '/api/' . (isset($api_user->id) ? 'edit_user/' . $api_user->id : 'create_user'))); ?>
        <input type="hidden" name="id" value="<?= $api_user->id ?? '' ?>" />
        <div class="row">
            <div class="col-md-6">
                <div class="panel_s">
                    <div class="panel-body">
                        <?= render_input('name', _l('fq_saas_api_key_name'), $api_user->name ?? '', 'text', ['max' => 50, 'min' => '2', 'required' => 'required']); ?>

                        <div class="tw-mt-4 tw-mb-4">
                            <hr />
                        </div>

                        <?php $key = 'fq_saas_api_key'; ?>
                        <div class="form-group tw-mb-2">
                            <label><?= fq_saas_input_label_with_hint($key); ?></label>
                            <div class="tw-flex">
                                <div class="input-group col-md-12 <?= $key; ?>">
                                    ...
                                    <span class="input-group-addon tw-px-2 tw-border-l-0 copy-to-clipboard"
                                        data-toggle="tooltip" data-title="<?= _l('fq_saas_copy_to_clipboard'); ?>"
                                        data-text="<?= $api_user->token ?? ''; ?>"
                                        data-success-text="<?= _l('fq_saas_copied'); ?>">
                                        <a href="javascript:;"><i class="fa fa-copy"></i></a>
                                    </span>
                                    <span class="input-group-addon tw-px-2 tw-border-l-0 _delete api-key-generator"
                                        data-toggle="tooltip" data-title="<?= _l('fq_saas_generate_api_key'); ?>">
                        ...
                        <p class="alert alert-warning mtop20"><?= _l('fq_saas_api_key_warning'); ?></p>
            ...
            <div class="col-md-6">
                <div class="panel_s">
                    <div class="panel-body">
                        <p> <?= _l('fq_saas_api_key_permissions'); ?></p>
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <tbody>
                                    <?php
                                    foreach (fq_saas_api_endpoints_specs() as $feature => $endpoint) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let $apiKeyWrapper = $(".fq_saas_api_key");
