...
$CI = &get_instance();
$packages = isset($packages) ? $packages : $CI->fq_saas_model->packages();
$invoice_package_id = $invoice->{fq_saas_column('packageid')} ?? '';

if (is_client_logged_in()) {
    // Filter by assigned customers
    $packages = $CI->fq_saas_model->packages_filter_by_assigned_client($packages, get_client_user_id(), $invoice_package_id);
}
...
$_can_trial = $is_client ? fq_saas_client_can_trial_package(get_client_user_id()) : true;

$enable_grouping = $showing_subscribed_card || $single_pricing_mode ? false : get_option('fq_saas_enable_package_grouping') != '0';

// Group and sort the data by interval alias
$grouped_packages = $enable_grouping ? fq_saas_group_pricing_by_interval_alias($packages) : ['0' => $packages];
