...
if (!empty($invoice))
    $disabled_default_modules = fq_saas_tenant_disabled_default_modules((object)['package_invoice' => $invoice]);
else
    $disabled_default_modules = fq_saas_alias_disabled_default_modules((array)($package->metadata->disabled_default_modules ?? []));
...
if (!empty($package->metadata->is_liftetime_deal))
    $interval_title = _l('fq_saas_interval_lifetime');
else
    $interval_title .= ' ' . _l($interval > 1 ? "fq_saas_" . $interval_type : "fq_saas_" . $interval_type . '_singular');
...
<div class=" panel_s tw-p-4 tw-py-2
        ...
        <div class="tw-flex tw-justify-center tw-w-full">
            <ul class="tw-grid tw-grid-cols-2 tw-gap-2">
                ...
                <li class="text-left text-capitalize <?= ($key + 1) % 2 ? 'tw-mr-2' : 'tw-ml-2'; ?>">
                    ...
                    <?= $limit_display_option === "yes_2" ||  $limit_display_option === "yes_3" ? ((int)$limit === -1 ? _l('fq_saas_unlimited') : $limit) : ''; ?>
                    <?= _l($feature, '', false); ?>
                </li>
                <?php $key++;
                        endforeach;
    ...
    <div class="panel_footer tw-flex tw-justify-center tw-mt-4">
        ...
        <a href="<?= base_url('clients/packages/' . $package->slug . '/select'); ?>"
            ...
            <?= $package->trial_period > 0 ? _l('fq_saas_start_trial', $package->trial_period) : _l('fq_saas_subscribe'); ?>
        </a>
        ...
        <div class="tw-flex tw-flex-col">
            ...
            <a href="<?= base_url('clients/packages/' . $invoice->slug . '/select'); ?>"
                ...
                <?php if ($on_trial) echo 'onclick="return confirm(\'' . fq_saas_ecape_js_attr(_l('fq_saas_upgrade_confirm_text')) . '\')"'; ?>>
                <i class="fa fa-check"></i>
                <?= $days_left > 0 ? _l('fq_saas_view_subscription_invoice_trial', $days_left) : _l('fq_saas_view_subscription_trial_over'); ?>
            </a>
            <?php else : ?>
        ...
        <a onclick="return confirm('<?= fq_saas_ecape_js_attr(_l('fq_saas_upgrade_confirm_text')); ?>')"
            href="<?= base_url('clients/packages/' . $package->slug . '/select'); ?>" class="btn btn-primary">
            <?= empty($invoice) ? _l('fq_saas_subscribe') : ($package->price >= ($invoice->price ?? 0) ? _l('fq_saas_upgrade') : _l('fq_saas_downgrade')); ?>
        </a>
    ...
    <div class="panel_footer tw-flex tw-justify-between tw-mt-4">
        <div class="tw-flex  tw-space-x-2">
            <?php $stat = $CI->fq_saas_model->package_stats((int)$package->id); ?>
            <?php if (in_array($package->db_scheme, ['single_pool', 'shard'])) : ?>
            <span data-title="<?= _l('fq_saas_total_db_pools'); ?>" data-toggle="tooltip">
                <i class="fa fa-database"></i>
                <?= count($package->db_pools); ?>
            </span>

            <span data-title="<?= _l('fq_saas_total_instances_on_pool'); ?>" data-toggle="tooltip">
                <i class="fa fa-users"></i>
                <?= $stat->total_pool_population; ?>
            </span>
            <?php endif; ?>

            <span data-title="<?= _l('fq_saas_package_total_attached_invoices'); ?>" data-toggle="tooltip">
        ...
        <div class="tw-flex tw-space-x-2">
            ...
            <!-- copy to clipboad -->
            <a href="#" data-success-text="<?= _l('fq_saas_copied'); ?>"
                data-text="<?= site_url('authentication/register') . '?' . fq_saas_route_id_prefix('plan') . '=' . $package->slug; ?>"
                onclick="return false;" data-toggle="tooltip"
                data-title="<?= _l('fq_saas_package_copy_to_clipboard'); ?>"
                class="btn btn-secondary btn-xs copy-to-clipboard">
                <i class="fa fa-share-alt"></i>
            </a>
            <!-- clone -->
            <a href="<?= admin_url(FQ_SAAS_ROUTE_NAME . '/packages/clone/' . $package->id); ?>"
                data-toggle="tooltip" data-title="<?= _l('fq_saas_clone'); ?>" class="btn btn-secondary btn-xs"><i
                    class="fa fa-copy"></i></a>
            <?php endif ?>

            <?php if (staff_can('edit', 'fq_saas_packages')) : ?>
            <a href="<?= admin_url(FQ_SAAS_ROUTE_NAME . '/packages/edit/' . $package->id); ?>" data-toggle="tooltip"
                data-title="<?= _l('fq_saas_edit'); ?>" class="btn btn-primary btn-xs"><i class="fa fa-pen"></i></a>
            <?php endif ?>

            <?php if (staff_can('delete', 'fq_saas_packages')) : ?>
            <?= form_open(admin_url(FQ_SAAS_ROUTE_NAME . '/packages/delete')); ?>
            <?= form_hidden('id', $package->id); ?>
            <button class="btn btn-danger btn-xs  _delete" data-toggle="tooltip"
                data-title="<?= _l('fq_saas_delete'); ?>"><i class="fa fa-trash"></i></button>
            <?= form_close(); ?>
            <?php endif ?>
