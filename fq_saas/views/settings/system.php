                ...
                <div class="panel">
                    <div class="panel-body">
                        <div class="input-group">
                            <input type="text" placeholder="<?= _l('fq_saas_purchase_code'); ?>" class="form-control" id="purchase-code" data-value="<?= $purchase_code; ?>" data-action="<?= admin_url(FQ_SAAS_ROUTE_NAME . '/system/save_purchase_code'); ?>" value="<?= $purchase_code; ?>" readonly>
                        ...
                        <?php if (!empty($remote_modules->{FQ_SAAS_MODULE_NAME})) :
                            $installed_version = $saas_module['headers']['version'];
                            $latest_version = $remote_modules->{FQ_SAAS_MODULE_NAME}->version;
                        ?>
                            <div class="text-center tw-mt-5">
                                ...
                                <div class="tw-text-2xl tw-w-full tw-mt-4">
                                    ...
                                    <a onclick="javascript: return confirm('<?= fq_saas_ecape_js_attr(_l('fq_saas_backup_warning')); ?>');" href="<?= admin_url(FQ_SAAS_ROUTE_NAME . '/system/get_module/' . FQ_SAAS_MODULE_NAME); ?>" class="btn btn-<?= $installed_version != $latest_version ? 'danger' : 'primary'; ?> btn-block"><?= $installed_version != $latest_version ? _l('update_now') : _l('download'); ?></a>
        ...
        <?php if (!empty($remote_modules)) : ?>
            ...
            <div class="panel">
                <div class="panel-body">
                    ...
                    <h3 class="tw-mt-0"><?= _l('fq_saas_extensions'); ?></h3>
                    <p><?= _l('fq_saas_extensions_hint'); ?></p>

                    <?php $this->load->view('authentication/includes/alerts'); ?>

                    <?php if (empty($remote_modules)) : ?>
                        <p class="text-center"><?= _l('fq_saas_empty_data'); ?></p>
                    <?php endif; ?>
                    ...
                    <div class="tw-grid tw-gap-3 tw-grid-cols-1 sm:tw-grid-cols-2 md:tw-grid-cols-3 tw-mt-4">
                        <?php foreach ($remote_modules as $module_name => $module) :
                            if ($module_name === FQ_SAAS_MODULE_NAME) continue;
                            ...
                            <div class="panel_s tw-p-4 tw-py-2 tw-bg-neutral-100">
                                <div class="panel_body tw-flex tw-flex-col tw-items-center tw-justify-center text-center tw-gap-3 tw-h-full tw-relative">
                                    ...
                                    <div class="tw-text-xl tw-w-full tw-mt-4 tw-grid tw-gap-3 tw-grid-cols-<?= $installed_module ? '2' : '1'; ?>">
                                        ...
                                        <a onclick="javascript: return confirm('<?= fq_saas_ecape_js_attr(_l('fq_saas_backup_warning')); ?>');" href="<?= !empty($module->link) ? $module->link : admin_url(FQ_SAAS_ROUTE_NAME . '/system/get_module/' . $module_name); ?>" class="btn btn-<?= $installed_module && $installed_version != $module->version ? 'danger' : 'primary'; ?> btn-block"><?= $installed_module && $installed_version != $module->version ? _l('update_now') : _l('download'); ?></a>

                                        <?php if ($installed_module) : ?>
                                            <?php if ($installed_module['activated']) : ?>
                                                <a href="<?= admin_url(FQ_SAAS_ROUTE_NAME . '/system/deactivate/' . $module_name); ?>" class="btn btn-danger"><?= _l('module_deactivate'); ?></a>
                                            <?php else : ?>
                                                <a href="<?= admin_url(FQ_SAAS_ROUTE_NAME . '/system/activate/' . $module_name); ?>" class="btn btn-success"><?= _l('module_activate'); ?></a>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                    </div>
                                </div>
