...
$services = array_merge($default_service, $CI->fq_saas_model->services());
$currency = get_base_currency();
?>
<div class="tw-flex tw-flex-col">
    <div class="row">
        <div class="col-xs-12">
            <?php render_yes_no_option('fq_saas_enable_custom_service_request', _l('fq_saas_enable_custom_service_request'), _l('fq_saas_enable_custom_service_request_hint')); ?>
        </div>
        <div class="col-xs-12">
            <?= $label = _l('fq_saas_custom_service_request_form') . fq_saas_form_label_hint('fq_saas_custom_service_request_form_hint'); ?>
            <a href="<?= admin_url('leads/form'); ?>" target="_blank"
                class="tw-ml-3"><?= _l('fq_saas_create_marketplace_form'); ?></a>
            <?= render_input('settings[fq_saas_custom_service_request_form]', '', get_option('fq_saas_custom_service_request_form')); ?>
        </div>
    </div>
        ...
        <div class="tw-mt-4 tw-flex tw-flex-col tw-gap-6 services">
            ...
            <div class="service" data-id="<?= $service_id; ?>">
                <div class="tw-mb-4 tw-flex tw-flex-wrap tw-items-start tw-justify-between">
                    ...
                    <div class="col-sm-8">
                        <input data-toggle="tooltip" data-title="<?= _l('fq_saas_service_name'); ?>"
                            name="settings[fq_saas_custom_services][<?= $service_id; ?>][name]"
                            value="<?= $value['name']; ?>" class="form-control"
                            placeholder="<?= _l('fq_saas_service_name'); ?>" required />
                        <textarea data-toggle="tooltip" data-title="<?= _l('fq_saas_description'); ?>"
                            name="settings[fq_saas_custom_services][<?= $service_id; ?>][description]"
                            class="form-control"
                            placeholder="<?= _l('fq_saas_description'); ?>"><?= $value['description'] ?? ''; ?></textarea>
                        <div class="input-group" data-toggle="tooltip" data-title="<?= _l('fq_saas_price'); ?>">
                            <input min="0" step="0.01" type="number"
                                name="settings[fq_saas_custom_services][<?= $service_id; ?>][price]"
                                value="<?= $value['price'] ?? ''; ?>" class="form-control"
                                placeholder="<?= _l('fq_saas_price'); ?>" required />
                            <span class="input-group-addon tw-px-2 tw-border-l-0">
                                ...
                                <select class="input-control tw-border-neutral-300 tw-border tw-rounded-md"
                                    name="settings[fq_saas_custom_services][<?= $service_id; ?>][billing_mode]"
                                    required>
                        ...
                        <div>
                            <input data-toggle="tooltip" data-title="<?= _l('fq_saas_image_url'); ?>"
                                ...
                                name="settings[fq_saas_custom_services][<?= $service_id; ?>][image]"
