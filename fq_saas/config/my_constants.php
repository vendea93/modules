 ...
 */
$upload_folder = FQ_SAAS_UPLOAD_BASE_DIR;
...
if (fq_saas_is_tenant()) {
    ...
    fq_saas_handle_tenant_404_static_file_request($tenant, $upload_folder, $tenants_upload_folder);

    $storage_base_path = FCPATH . $tenants_upload_folder;
    if (!file_exists($storage_base_path))
        fq_saas_create_dir($storage_base_path);
...
define('FQ_SAAS_TENANT_UPLOAD_BASE_FOLDER', $storage_base_path);
...
foreach ($attachment_folders as $key => $folder_name) {
    ...
    define($key, $path);
    if (!file_exists($path)) {
        fq_saas_create_dir($path, $blueprint = $master_base_path . $folder_name);
    }
}
