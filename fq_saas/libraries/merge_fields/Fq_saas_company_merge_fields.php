...
class Fq_saas_company_merge_fields extends App_merge_fields
{
    /**
    ...
    {
        $fields['{instance_url}'] = fq_saas_tenant_base_url($company);
        $fields['{instance_admin_url}']   = fq_saas_tenant_admin_url($company);
        ...
        if (!empty($company->clientid) && !fq_saas_is_tenant()) {
            $CI = &get_instance();
            if ($CI->db->table_exists(fq_saas_extensions_table('affiliates'))) {
                $aff = $CI->db->get_where(fq_saas_extensions_table('affiliates'), ['clientid' => (int) $company->clientid])->row();
                if ($aff) {
                    $fields['{affiliate_code}']    = $aff->code;
                    $fields['{affiliate_balance}'] = app_format_money((float) $aff->balance, '');
                }
            }
        }
