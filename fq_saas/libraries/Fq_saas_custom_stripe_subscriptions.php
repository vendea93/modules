...
class Fq_saas_custom_stripe_subscriptions extends Stripe_subscriptions
{
    public function update_subscription($subscription_id, $update_values, $db_subscription, $prorate = false)
    {
        ...
        $metadata = fq_saas_search_client_metadata('subscription_id', (int)$subscription_id);
        if (!empty($metadata)) {
            throw new \Exception(_l('fq_saas_stripe_admin_update_denied'), 1);
        }
