...
require_once 'Fq_saas_subscription_trait.php';

class Fq_saas_model extends App_Model
{
    use Fq_saas_subscription_trait;
    ...
    {
        $this->db->select();
        $this->db->from(fq_saas_table($entity));
    ...
    {
        return $this->add_or_update_raw(fq_saas_table($entity), $data);
    }
    ...
    {
        $this->db->where('id', $id);
        return $this->db->delete(fq_saas_table($entity));
    }
    ...
    public function clone(string $entity, $id)
    {
        ...
        if (isset($entity_data->slug)) {
            $entity_data->slug = fq_saas_generate_unique_slug($entity_data->name, $entity);
        }
        ...
        $filter =  hooks()->apply_filters('fq_saas_package_clone_filter', ['entity' => $entity, 'entity_data' => (array)$entity_data]);
        ...
        foreach ($packages as $p) {
            ...
            if (!empty($pools)) {
                foreach ($pools as $pool) {
                    $pool = (array)$pool;
                    $key = fq_saas_dsn_to_string($pool, false);

                    if (!isset($pools_map[$key])) {
                        try {

                            $population = count(fq_saas_raw_query($query, $pool, true));
    ...
    public function get_db_pools_population($pools)
    {
        ...
        foreach ($pools as $pool) {
            ...
            if (!isset($pools_map[$key])) {
                ...
                $total = count(fq_saas_raw_query($query, $pool, true));
                $population = $total;
                $population_map[$key] = (int)$population;
                $pools_map[$key] = $pool;
            }
    ...
    function package_stats($packageid)
    {
        ...
        $packageid_col = fq_saas_column('packageid');
        $query = 'SELECT COUNT(DISTINCT(' .  $packageid_col . ')) as total FROM `' . fq_saas_master_db_prefix() . 'invoices` WHERE `' . $packageid_col . "`=$packageid";
        $resp = fq_saas_raw_query_row($query, [], true);
    ...
    public function db_schemes()
    {
        ...
        $schemes = [
            ['key' => 'multitenancy', 'label' => _l('fq_saas_use_the_current_active_database_for_all_tenants')], // Option for using the current active database for all tenants
            ['key' => 'single', 'label' => _l('fq_saas_use_single_database_for_each_company_instance.')], // Option for using a single database for each company instance
            ['key' => 'single_pool', 'label' => _l('fq_saas_single_pool_db_scheme')], // Option for using a single pool database scheme
            ['key' => 'shard', 'label' => _l('fq_saas_distribute_companies_data_among_the_provided_databases_in_the_pool')], // Option for distributing companies' data among provided databases in the pool
        ];
        ...
        $schemes = hooks()->apply_filters('fq_saas_module_db_schemes', $schemes);
    ...
    public function db_schemes_alt()
    {
        ...
        $schemes = [
            ['key' => 'package', 'label' => _l('fq_saas_auto_detect_from_client_subcribed_package')], // Option for auto-detecting database scheme from client subscribed package
            ['key' => 'multitenancy', 'label' => _l('fq_saas_use_the_current_active_database')], // Option for using the current active database
            ['key' => 'single', 'label' => _l('fq_saas_create_a_separate_database')], // Option for creating a separate database
            ['key' => 'shard', 'label' => _l('fq_saas_i_will_provide_database_credential')], // Option for providing database credentials
        ];
        ...
        $schemes = hooks()->apply_filters('fq_saas_module_db_schemes_alt', $schemes);
    ...
    public function company_status_list()
    {
        ...
        $editable_statuses = [FQ_SAAS_STATUS_ACTIVE, FQ_SAAS_STATUS_INACTIVE, FQ_SAAS_STATUS_BANNED];
        foreach ($editable_statuses as $key => $value) {
            $statuses[] = ['key' => $value, 'label' => _l('fq_saas_' . $value)];
        }
        return $statuses;
    }
    ...
    public function yes_no_options()
    {
        ...
        return [
            ['key' => 'no', 'label' => _l('fq_saas_no')], // Option for "No"
            ['key' => 'yes', 'label' => _l('fq_saas_yes')] // Option for "Yes"
        ];
    }
    ...
    {
        $options_table = fq_saas_master_db_prefix() . 'options';
        // Retrieve the options from the database
        $this->db->select("`name` as 'key', REPLACE(`name`,'_',' ') as 'name'");
        $this->db->where("`name` NOT LIKE 'perfex_saas%' AND `name` NOT LIKE 'fq_saas%'");
    ...
    public function sensitive_shared_options()
    {
        ...
        $sql = "SELECT `name` as 'key', REPLACE(`name`,'_',' ') as 'name' FROM `" . $options_table . "` WHERE
                ...
                `name` LIKE 'perfex_saas%' OR `name` LIKE 'fq_saas%'
                ";
    ...
    public function modules($exclude_self = true)
    {
        ...
        $option_getter_method = fq_saas_is_tenant() ? 'fq_saas_get_options' : 'get_option';

        // Retrieve the custom module names from the options
        $custom_modules_name = $option_getter_method('fq_saas_custom_modules_name');
        $custom_modules_name = empty($custom_modules_name) ? [] : json_decode($custom_modules_name, true);

        $modules_market_settings = json_decode($option_getter_method('fq_saas_modules_marketplace') ?? '', true);

        foreach ($_modules as $value) {
            $module_id = $value['system_name'];

            // Check if the module is the self module and exclude it if necessary
            if ($module_id == FQ_SAAS_MODULE_NAME && $exclude_self) {
                continue;
            }
        ...
        $modules  = hooks()->apply_filters('fq_saas_module_list_filter', $modules);
    ...
    {
        $option = get_option('fq_saas_custom_services');
    ...
    {
        $packages = $this->get(fq_saas_table('packages'), $id);
    ...
    public function companies($id = '', $skip_parsing = false)
    {
        ...
        $companies = $this->get(fq_saas_table('companies'), $id);
        if (!empty($id)) {
            $companies = [$companies];
        }
    ...
    public function create_or_update_company($data, $invoice)
    {
        ...
        if ($creating_new || empty($data['id'])) {
            if (empty($data['clientid']) || empty($data['name'])) {
                throw new \Exception(_l('fq_saas_malformed_company_payload'), 1);
            }
        }


        $creating_as_admin = !is_client_logged_in() && (
            staff_can('create', 'fq_saas_companies') &&
            staff_can('edit', 'fq_saas_companies')
        );
        ...
        if (!empty($custom_domain)) {
            ...
            if ($custom_domain !== ($company->custom_domain ?? '')) {
                if (!fq_saas_is_valid_custom_domain($custom_domain))
                    throw new \Exception(_l('fq_saas_invalid_custom_domain', $custom_domain));
                ...
                $count = count($this->get(fq_saas_table('companies')));
                if ($count) throw new \Exception(_l('fq_saas_custom_domain_exist'), 1);
        ...
        if ($creating_new) {
            ...
            $max_instance_limit = fq_saas_get_tenant_instance_limit($invoice);
            $this->db->where('clientid', $data['clientid']);
            $count = count($this->companies());
            if ($max_instance_limit > 0 && $count >= $max_instance_limit) {
                throw new \Exception(_l('fq_saas_max_instance_reached' . ($creating_as_admin ? '_admin' : ''), $max_instance_limit), 1);
            }
            ...
            $slug = fq_saas_generate_unique_slug($slug, 'companies', $data['id'] ?? '');
            ...
            if (empty($data['dsn'])) {
                ...
                if ($invoice->db_scheme == 'single') {
                    ...
                    if (!$this->db_user_has_create_privilege()) {
                        throw new \Exception(_l('fq_saas_db_scheme_single_not_supported'), 1);
                    }

                    $dbname = fq_saas_db($data['slug']);
                    $create_db = $this->create_database($dbname);
                    if ($create_db !== true) {
                        throw new \Exception(_l('Error creating database: ' . $create_db), 1);
                    }

                    $data['dsn'] = fq_saas_dsn_to_string([
                        'dbname' => $dbname
                    ]);
                }
        ...
        $filter = hooks()->apply_filters('fq_saas_module_tenant_data_payload', ['data' => $data, 'tenant' => $company, 'invoice' => $invoice, 'error' => '']);
        $data = $filter['data'];
        ...
        if (!$creating_new) {
            if (!$creating_as_admin) {
                if (
                    isset($data['status']) &&
                    $data['status'] !== FQ_SAAS_STATUS_PENDING_DELETE
                ) {
                    unset($data['status']);
                }
            }
        ...
        if (!defined('CRON'))
            fq_saas_trigger_module_install('', $company->slug ?? $data['slug']);

        hooks()->do_action('fq_saas_module_tenant_created_or_updated', $_id);

        fq_saas_log('tenant_created_or_updated', [
            'id'           => $_id,
    ...
    public function delete_company(int $id)
    {
        ...
        if (ENVIRONMENT == 'development') {
            ...
            if (in_array($company->status, [FQ_SAAS_STATUS_DEPLOYING, FQ_SAAS_STATUS_PENDING])) {
                $delay_tenant_removal = false;
            }
        }

        if ($delay_tenant_removal) {
            $data = ['id' => $id, 'status' => FQ_SAAS_STATUS_PENDING_DELETE, 'metadata' => ['deleted_at' => date('Y-m-d H:i:s')]];
        ...
        $removed = false;
        try {
            $removed = fq_saas_remove_company($company);
        } catch (\Throwable $th) {
    ...
    public function get_invoices_total($data)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            // Must have packageid
            $this->db->where(fq_saas_column('packageid') . ' IS NOT NULL');
            $this->db->where(fq_saas_column('packageid') . " != ''");
