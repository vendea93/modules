...
trait Fq_saas_subscription_trait
{
    /**
    ...
    public function generate_company_invoice($clientid, $packageid, $extra_custom_data = [])
    {
        ...
        $client_metadata = (object)fq_saas_get_or_save_client_metadata($clientid);
        ...
        if ($can_trial) {
            fq_saas_get_or_save_client_metadata(
                $clientid,
                [
        ...
        if ($old_invoice) {
            ...
            $max_instance_limit = fq_saas_get_tenant_instance_limit($package);
            ...
            if ($max_instance_limit > 0 && $total_instances > 0) {
                ...
                if ($total_instances > $max_instance_limit)
                    throw new \Exception(_l('fq_saas_plan_upgrade_unfit_number_of_instances'), 1);
                ...
                foreach ($companies as $company) {
                    // Current usage limit
                    $usage_limits = fq_saas_get_tenant_quota_usage($company, $limited_resources, $old_invoice);
                    ...
                    foreach ($usage_limits as $resources => $usage) {
                        $quota = fq_saas_tenant_resources_quota($company, $resources);

                        if ($quota !== -1 && $usage > $quota) {
                            throw new \Exception(_l('fq_saas_plan_upgrade_unfit_quota', [$company->name, $resources]), 1);
                        }
                    }
                }
            }
            ...
            if ($old_invoice->{fq_saas_column('packageid')} == $packageid) {
                ...
                if ($on_trial) {
                    ...
                    throw new \Exception(_l("fq_saas_no_change_detected"), 1);
                }
            }
        ...
        if (($metadata->stripe->enabled ?? '') == '1') {
            return $this->fq_saas_stripe_model->generate_company_invoice($clientid, $packageid, $old_customization);
        }
        ...
        $new_items = [
            [
                "order" => "1",
                "description" => _l('fq_saas_invoice_desc_subscription', $package->name),
                "long_description" => "",
                "qty" => "1",
                "unit" => "",
                "rate" => $package->price,
        ...
        // Important
        $data[fq_saas_column('packageid')] = $packageid;
        ...
        fq_saas_get_or_save_client_metadata($clientid, ['last_cancelled_invoice' => '', 'subscription_id' => '']);

        $invoice = $this->get_company_invoice($clientid);
        if (!$invoice) throw new \Exception(_l('fq_saas_client_invoice_lookup_error'), 1);
     ...
     * @param int $clientid
     * @param Fq_saas_custom_limit $custom_limitations
     * @return true
     * @throws Exception
     */
    public function update_company_invoice($invoice, $clientid, $custom_limitations)
    {
        $package = $this->packages($invoice->{fq_saas_column('packageid')});
        $metadata = $package->metadata;
        $client = $this->clients_model->get($clientid);
        ...
        $max_instance_limit = fq_saas_get_tenant_instance_limit($_package);
        ...
        foreach ($companies as $company) {
            ...
            $_storage = fq_saas_tenant_storage_is_unlimited($company) ? null : $_storage;

            // Check for storage
            if (!is_null($_storage)) {
                $company->package_invoice->custom_limits->storage = $_storage;
                $usage = fq_saas_tenant_used_storage($company, false);
                $quota = fq_saas_convert_formatted_size_to_bytes(fq_saas_tenant_storage_limit($company));
                if ($quota !== -1 && $usage > $quota)
                    throw new \Exception(_l('fq_saas_plan_upgrade_unfit_quota', [$company->name, 'storage']), 1);
            }
        ...
        // Important
        $data[fq_saas_column('packageid')] = $package->id;
        ...
        if ($has_subscription) {
            ...
            $update = $this->fq_saas_stripe_model->update_company_invoice($invoice, $clientid, $custom_limitations);
    ...
    public function update_company_onetime_invoices($clientid, $package, $parent_invoice_data, $group, $items = [])
    {
        ...
        try {
            ...
            $client_metadata = fq_saas_get_or_save_client_metadata($clientid);
            ...
            foreach ($items as $item_id => $item) {
                ...
                if (!$new_invoice_id) {
                    ...
                    unset($new_invoice_data[fq_saas_column('packageid')]);
                ...
                if ($new_invoice_id) {
                    ...
                    $client_metadata = fq_saas_get_or_save_client_metadata($clientid, $metadata);
    ...
    public function get_company_invoice($clientid, $options = [])
    {
        ...
        return fq_saas_get_client_package_invoice($clientid, $options);
    }
    ...
    public function cancel_company_invoice(int $clientid, $type = 'immediately', $invoice = null)
    {
        ...
        if (empty($packageid)) {
            throw new \Exception(_l('fq_saas_no_invoice_client_for_client'), 1);
        }


        if (!empty($invoice->subscription_id)) {
            $cancelled_invoice = $this->fq_saas_stripe_model->cancel_subscription($invoice->subscription_id, $type);
            if ($cancelled_invoice && $type == 'at_period_end')
                fq_saas_get_or_save_client_metadata($clientid, ['last_cancelled_invoice' => $cancelled_invoice->subscription_id]);
            return $cancelled_invoice;
        }

        // ---- Non-Stripe recurring invoice path ----

        // Cancel till end of period (added in 0.3.7): do NOT mark current invoice as
        // cancelled -- keep it active so the client retains access until `duedate`.
        // Only break the recurrence so that cron will not auto-generate the next invoice.
        if ($type === 'at_period_end') {
            if ($invoice->recurring != "0" && !isset($invoice->is_mock)) {
                $this->invoices_model->db->update(
                    fq_saas_master_db_prefix() . 'invoices',
                    ['recurring' => '0'],
                    ['id' => $invoice->id]
                );
            }
        ...
        $stale_invoices = fq_saas_get_company_invoice_child_invoices($invoice, false);
        ...
        foreach ($stale_invoices as $_invoice) {
            ...
            if (
                ...
                if (!$this->invoices_model->mark_as_cancelled($_invoice->id))
                    throw new \Exception(_l('fq_saas_invoice_cancel_error'), 1);
            }

            // Mark as non recurring (so the child invoice will not be recreated by cron)
            if ($_invoice->recurring != "0")
                $this->invoices_model->db->update(fq_saas_master_db_prefix() . 'invoices', ["recurring" => "0"], ['id' => $_invoice->id]);
        }

        // Remove recurring
        fq_saas_get_or_save_client_metadata($clientid, ['last_cancelled_invoice' => $invoice->id, 'last_cancelled_invoice_recurring' => $invoice->recurring, 'subscription_id' => '']);
    ...
    public function get_company_cancelled_invoice($clientid, $resumable = true)
    {
        ...
        if (!$invoice || empty($package_id = $invoice->{fq_saas_column('packageid')} ?? ''))
            return false;
    ...
    public function resume_company_invoice(int $clientid)
    {
        ...
        if ($invoice && !empty($package_id = $invoice->{fq_saas_column('packageid')})) {
            ...
            if (!empty($invoice->subscription_id)) {
                $resumed_invoice = $this->fq_saas_stripe_model->resume_subscription($invoice->subscription_id);
            } else {
                ...
                $this->invoices_model->db->update(fq_saas_master_db_prefix() . 'invoices', $update_data, ['id' => $invoice->id]);
                $resumed_invoice = $this->get_company_invoice($clientid);
            }
            if ($resumed_invoice) {
                fq_saas_get_or_save_client_metadata($clientid, ['last_cancelled_invoice' => '', 'last_cancelled_invoice_recurring' => '']);
    ...
    public function prepare_invoice_customization_items($package, $data_source, $old_package = null)
    {
        ...
        return Fq_saas_custom_limit::create($package, $data_source, $modules, $services, $old_package);
    }
}
...
class Fq_saas_custom_limit
{
    ...
    public static function create(object $package, $data_source, $modules, $services, ?object $old_package = null)
    {
        ...
        if (!empty($custom_limits)) {
            foreach ($custom_limits as $resources => $quantity) {
                ...
                $custom_limitations[] = [
                    'resources' => $resources,
                    'quantity' => $quantity,
                    'description' => _l('fq_saas_invoice_addon_item_desc', _l('fq_saas_limit_' . $resources)),
        ...
        if (!empty($purchased_modules)) {
            foreach ($purchased_modules as $index => $module) {
                ...
                $item = [
                    'resources' => $module,
                    'quantity' => 1,
                    'description' => _l('fq_saas_invoice_addon_module_item_desc', $modules[$module]['custom_name']),
        ...
        if (!empty($purchased_services)) {
            foreach ($purchased_services as $index => $service_id) {
                ...
                $item = [
                    ...
                    'quantity' => 1,
                    'description' => _l('fq_saas_invoice_addon_service_item_desc', $service['name']),
                    'unit_price' => $price,
                    'skip_limit_validation' => true,
                ];
        ...
        $result = new Fq_saas_custom_limit();
