...
class Fq_saas_stripe_model extends App_Model
{
     ...
     * @param Fq_saas_custom_limit $custom_limitations Extra invoice custom data
    ...
    public function generate_company_invoice($clientid, $packageid, $custom_limitations)
    {
        ...
        $package = $this->fq_saas_model->packages($packageid);
        ...
        if (($package_stripe_settings->enabled ?? '') != '1' || empty($default_stripe_price_id)) {
            throw new \Exception(_l('fq_saas_package_can_not_use_strip_model'), 1);
        }

        $old_invoice = $this->fq_saas_model->get_company_invoice($clientid);
        ...
        try {
            ...
            fq_saas_log('subscription_activated', [
                'clientid'               => $clientid,
                'packageid'              => $packageid,
                'stripe_subscription_id' => $stripeSubscription->id ?? '',
            ]);
            $this->session->unset_userdata('fq_saas_checkout_coupon');

            return $this->fq_saas_model->get_company_invoice($clientid);
        } catch (\Throwable $th) {
    ...
    public function update_company_invoice($invoice, $clientid, $custom_limitations)
    {
        ...
        $package = $this->fq_saas_model->packages($invoice->{fq_saas_column('packageid')});
        $package_stripe_settings = $this->get_package_stripe_settings($package);
    ...
    public function cancel_subscription($subscription_id, $type = 'immediately')
    {
        ...
        hooks()->do_action('fq_saas_before_cancel_stripe_subscription', ['subscription_id' => $subscription_id, 'type' => $type]);
        ...
        $invoice = $this->fq_saas_model->get_company_invoice($subscription->clientid, ['include_cancelled' => true]);

        hooks()->do_action('fq_saas_after_cancel_stripe_subscription', ['subscription_id' => $subscription_id, 'type' => $type, 'invoice' => $invoice]);
    ...
    public function resume_subscription($subscription_id)
    {
        ...
        hooks()->do_action('fq_saas_before_resume_stripe_subscription', ['subscription_id' => $subscription_id]);
        ...
        $invoice = $this->fq_saas_model->get_company_invoice($subscription->clientid);

        hooks()->do_action('fq_saas_after_resume_stripe_subscription', ['subscription_id' => $subscription_id, 'invoice' => $invoice]);
    ...
    protected function get_custom_limit_items($package, $custom_limitations, $force_sync_package = false)
    {
        ...
        foreach ($custom_limitations->customization_items as $key => $limit) {
            ...
            if (empty($resources_stripe_price_id)) {
                throw new \Exception(_l('fq_saas_stripe_missing_price_id', _l('fq_saas_limit_' . $resources)), 1);
            }
    ...
    {
        $package = is_object($package) ? $package : $this->fq_saas_model->packages((int)$package);
    ...
    {
        // Stripe-facing free-form text fields have a practical ~500 char limit.
        // Truncate server-side so we never attempt to push a too-long value to Stripe API.
        if (isset($data['terms']) && is_string($data['terms'])) {
            $data['terms'] = mb_substr($data['terms'], 0, 500);
        }

        $package = $this->fq_saas_model->packages($package_id);
        ...
        $this->fq_saas_model->add_or_update('packages', $data);
    ...
    public function setup_package_on_stripe($package)
    {
        ...
        hooks()->do_action('fq_saas_before_setup_package_on_stripe', $package);
        ...
        hooks()->do_action('fq_saas_after_setup_package_on_stripe', $package, $package_stripe_settings);

        return $package_stripe_settings;
    }

    /**
     * Synchronize all Stripe-enabled packages with Stripe in one pass.
     * Introduced in 0.3.8 (mass sync support).
     *
     * Iterates over every local package and pushes its Stripe settings (product,
     * prices, taxes) to Stripe. Each package is wrapped in its own try/catch so
     * a single bad package cannot abort the whole batch.
     *
     * @return array{synced:int,skipped:int,failed:int,errors:array<int,array{package_id:int,package_name:string,error:string}>}
     */
    public function mass_sync_packages_on_stripe()
    {
        $report = [
            'synced'  => 0,
            'skipped' => 0,
        ...
        $packages = (array) $this->fq_saas_model->packages();
        $packages = hooks()->apply_filters('fq_saas_mass_sync_stripe_packages', $packages);

        foreach ($packages as $package) {
            if (empty($package->id)) continue;

            $package_stripe_settings = $this->get_package_stripe_settings($package);
            if (($package_stripe_settings->enabled ?? '') != '1' || ($package_stripe_settings->sync ?? '1') == '0') {
                $report['skipped']++;
                continue;
            }

            try {
                $this->setup_package_on_stripe($package);
                $report['synced']++;
            } catch (\Throwable $th) {
                $report['failed']++;
                $report['errors'][] = [
                ...
                log_message('error', 'fq_saas mass_sync_packages_on_stripe failed for package ' . $package->id . ': ' . $th->getMessage());
            }
        }
    ...
    protected function prepare_package_stripe_prices($package, $stripe_product)
    {
        ...
        foreach ($modules as $module) {
            ...
            $limitations_description->{$key} = _l('fq_saas_invoice_addon_module_item_desc', $name);
        }
        ...
        foreach ($services as $service_id => $service) {
            ...
            $limitations_description->{$service_id} = _l('fq_saas_invoice_addon_service_item_desc', $name);
        }
        ...
        foreach ($discounts as $resources => $levels) {
            ...
            foreach ($levels as $level) {
                ...
                $limitations_description->{$key} = _l('fq_saas_invoice_addon_item_desc', _l('fq_saas_limit_' . $resources)) . " ($_percent% off)";
            }
        }
        ...
        foreach ($limitations as $resources => $limit) {
            ...
            $res_name = _l('fq_saas_limit_' . $resources, '', false);
            $res_name = str_starts_with($res_name, 'fq_saas') ? ($limitations_name->{$resources} ?? $resources) : $res_name;
            ...
            $prices[] = [
                ...
                'metadata' => [
                    "description" => $limitations_description->{$resources} ?? _l('fq_saas_invoice_addon_item_desc', $res_name),
                    "package_id" => $package->id,
                    "resources" => $resources,
                    "group" => "pcrm_saas"
                ]
    ...
    protected function package_stripe_settings_to_subscription_info($package)
    {
        ...
        return [
            ...
            'description' => nl2br(_l('fq_saas_invoice_desc_subscription', $package->name)),
    ...
    protected function get_stripe_customer($clientid)
    {
        ...
        $contact = fq_saas_get_primary_contact($clientid);
    ...
    protected function create_stripe_subscription($subscription, $items, $stripe_customer, $proration_behavior, $taxes = [], array $context = [])
    {
        $params = [];
        ...
        $params = hooks()->apply_filters('fq_saas_stripe_subscription_params', $params, array_merge($context, [
