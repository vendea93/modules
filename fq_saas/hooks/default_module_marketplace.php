 ...
 * @return bool
 */
function fq_saas_default_modules_marketplace_enabled()
{
    static $option = null;
    if ($option === null) {

        $option_getter_method = fq_saas_is_tenant() ? 'fq_saas_get_options' : 'get_option';
        $option = $option_getter_method('fq_saas_allow_default_modules_on_marketplace');
    }

    return $option == '1';
}

if (!fq_saas_default_modules_marketplace_enabled()) return;
...
hooks()->add_filter('fq_saas_module_list_filter', function ($modules) use ($default_modules_marketplace_prefix) {

    $default_modules = get_instance()->fq_saas_model->default_modules();
...
hooks()->add_filter('fq_saas_tenant_disabled_default_modules_list_filter', function ($tenant_default_disabled_modules, $tenant, $mode) use ($default_modules_marketplace_prefix) {

    $purchased_modules = (array)($tenant->package_invoice->purchased_modules ?? []);

    $disabled_modules = [];

    if (fq_saas_is_tenant() && function_exists('get_option')) {
