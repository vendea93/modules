 ...
 */
function fq_saas_filter_tenant_tabs($group, $hidden_tabs, $tabs, $lookup_tabs = [])
{
    $CI = &get_instance();
 ...
 */
function fq_saas_filter_tenant_settings_tabs($group, $hidden_tabs)
{
    $CI = &get_instance();
    ...
    if (empty($settings_sections)) {
        ...
        $lookup_tabs = fq_saas_app_settings_tabs();
        ...
        foreach ($section_keys as $index => $slug) {
            ...
            $_tabs_filtered = array_values(fq_saas_filter_tenant_tabs($group, $hidden_tabs, $tabs, $lookup_tabs));
 ...
 */
$tenant_hidden_tabs = fq_saas_tenant()->package_invoice->metadata->hidden_app_tabs ?? (object)[];
foreach (fq_saas_app_tabs_group() as $group) {
    ...
    if ($group === 'settings' && method_exists($CI->app, 'get_settings_sections') && $CI->router->fetch_class() === 'settings') {
        ...
        hooks()->add_action("admin_init", function () use ($group, $hidden_tabs) {
            fq_saas_filter_tenant_settings_tabs($group, $hidden_tabs);
        }, PHP_INT_MAX);
        continue;
    }
    ...
    hooks()->add_filter("{$group}_tabs", function ($tabs) use ($group, $hidden_tabs) {
        return fq_saas_filter_tenant_tabs($group, $hidden_tabs, $tabs);
    }, PHP_INT_MAX);
}