...
if (get_option('fq_saas_plesk_enabled') != '1') return;

/** Load library */
$CI->load->library(FQ_SAAS_MODULE_NAME . '/integrations/plesk_api');


/******** HELPERS *******/
if (!function_exists('fq_saas_plesk_enable_aliasdomain')) {
    /**
     * Check if addon domain is enabled for cpanel
     *
     * @return bool
     */
    function fq_saas_plesk_enable_aliasdomain()
    {
        return get_option('fq_saas_plesk_enable_aliasdomain') == '1';
    }
}

if (!function_exists('fq_saas_plesk_allowed_for_tenant')) {
    /**
     ...
     * @return bool
     */
    function fq_saas_plesk_allowed_for_tenant($tenant)
    {
...
if (!function_exists('fq_saas_get_plesk')) {
    /**
     * Get plesk api library instance
     *
     * @return Plesk_api $plesk
     */
    function fq_saas_get_plesk()
    {
        $CI = &get_instance();
        $host = get_option('fq_saas_plesk_host');
        $domain = get_option('fq_saas_plesk_primary_domain');
        $user = get_option('fq_saas_plesk_username');
        $password = $CI->encryption->decrypt(get_option('fq_saas_plesk_password'));
        ...
        $CI->plesk_api->init(
            ...
            $user,
            $password,
            FQ_SAAS_MODULE_NAME_SHORT . '_'
        );
        return $CI->plesk_api;
    }
}

if (!function_exists('fq_saas_plesk_setup_tenant_db')) {
    /**
     ...
     * @return array
     */
    function fq_saas_plesk_setup_tenant_db($slug)
    {

        $CI = &get_instance();
        $plesk = fq_saas_get_plesk();
...
if (!function_exists('fq_saas_plesk_rollback')) {
    /**
     ...
     * @return void
     */
    function fq_saas_plesk_rollback($tenant)
    {
        if (!fq_saas_plesk_allowed_for_tenant($tenant)) return;

        $plesk = fq_saas_get_plesk();

        $slug = $tenant->slug;
        $db_name = $plesk->addPrefix($slug);
        $subdomain = $slug . '.' . fq_saas_get_saas_default_host();
        ...
        if (!fq_saas_plesk_enable_aliasdomain()) return;
...
hooks()->add_filter('fq_saas_module_db_schemes', function ($schemes) {
    $schemes[] = ['key' => 'plesk', 'label' => _l('fq_saas_plesk_scheme')];
    return $schemes;
});
...
hooks()->add_filter('fq_saas_module_maybe_create_tenant_dsn', function ($data) {
    ...
    if ($invoice->db_scheme === 'plesk') {
        ...
        $data['dsn'] = fq_saas_plesk_setup_tenant_db($tenant->slug);
    }
    return $data;
});
...
hooks()->add_filter('fq_saas_module_tenant_data_payload', function ($payload) {
    ...
    $addondomain_mode = get_option('fq_saas_plesk_addondomain_mode');
    ...
    if (!fq_saas_plesk_enable_aliasdomain() || !$tenant || !$can_use_custom_domain) return $payload;

    if (!fq_saas_plesk_allowed_for_tenant($tenant)) return $payload;
...
hooks()->add_action('fq_saas_module_tenant_removed', 'fq_saas_plesk_rollback');
hooks()->add_action('fq_saas_module_tenant_removal_failed', 'fq_saas_plesk_rollback');
hooks()->add_action('fq_saas_module_tenant_deploy_failed', 'fq_saas_plesk_rollback');