...
if (get_option('fq_saas_cpanel_enabled') != '1') return;

/** Load library */
$CI->load->library(FQ_SAAS_MODULE_NAME . '/integrations/cpanel_api');


/******** HELPERS *******/
if (!function_exists('fq_saas_cpanel_enable_addondomain')) {
    /**
     * Check if addon domain is enabled for cpanel
     *
     * @return bool
     */
    function fq_saas_cpanel_enable_addondomain()
    {
        return get_option('fq_saas_cpanel_enable_addondomain') == '1';
    }
}

if (!function_exists('fq_saas_cpanel_allowed_for_tenant')) {
    /**
     ...
     * @return bool
     */
    function fq_saas_cpanel_allowed_for_tenant($tenant)
    {
...
if (!function_exists('fq_saas_get_cpanel')) {
    /**
     * Get cpanel api library instance
     *
     * @return Cpanel_api $cpanel
     */
    function fq_saas_get_cpanel()
    {
        $CI = &get_instance();
        $user = get_option('fq_saas_cpanel_username');

        // Set appropriately if user have added custom cpanel db_prefix
        $db_prefix = get_option('fq_saas_cpanel_db_prefix') ?? '';
        $db_prefix = empty($db_prefix) ? $user : $db_prefix;
        $prefix = (empty($db_prefix) ? '' : $db_prefix . '_') . FQ_SAAS_MODULE_NAME_SHORT . '_';

        $CI->cpanel_api->init(
            $user,
            $CI->encryption->decrypt(get_option('fq_saas_cpanel_password')),
            get_option('fq_saas_cpanel_login_domain'),
            get_option('fq_saas_cpanel_port'),
            $prefix
        );
        $CI->cpanel_api->mainDomain = get_option('fq_saas_cpanel_primary_domain');
        return $CI->cpanel_api;
    }
}

if (!function_exists('fq_saas_cpanel_setup_tenant_db')) {
    /**
     ...
     * @return array
     */
    function fq_saas_cpanel_setup_tenant_db($slug)
    {

        $CI = &get_instance();
        $cpanel = fq_saas_get_cpanel();
...
if (!function_exists('fq_saas_cpanel_rollback')) {
    /**
     ...
     * @return void
     */
    function fq_saas_cpanel_rollback($tenant)
    {
        if (!fq_saas_cpanel_allowed_for_tenant($tenant)) return;
        ...
        if (!fq_saas_cpanel_enable_addondomain()) return;

        /// Remove subdomain
        try {
            $cpanel->deleteSubdomain($slug, fq_saas_get_saas_default_host());
        } catch (\Throwable $th) {
            log_message('error', $th->getMessage());
        }
        try {
            fq_saas_cpanel_delete_domain($cpanel, $slug);
        } catch (\Throwable $th) {
            log_message('error', $th->getMessage());
        }
        ...
        if (!empty($tenant->custom_domain)) {
            try {
                fq_saas_cpanel_delete_domain($cpanel, $slug . 'addon', $tenant->custom_domain);
            } catch (\Throwable $th) {
                log_message('error', $th->getMessage());
            }
        }
...
if (!function_exists('fq_saas_cpanel_delete_domain')) {
    /**
     ...
     * @return mixed
     */
    function fq_saas_cpanel_delete_domain($cpanel, $slug, $domain = null)
    {
        $root_domain = fq_saas_get_saas_default_host();
...
hooks()->add_filter('fq_saas_module_db_schemes', function ($schemes) {
    $schemes[] = ['key' => 'cpanel', 'label' => _l('fq_saas_cpanel_scheme')];
    return $schemes;
});
...
hooks()->add_filter('fq_saas_module_maybe_create_tenant_dsn', function ($data) {
    ...
    if ($invoice->db_scheme === 'cpanel') {
        ...
        $data['dsn'] = fq_saas_cpanel_setup_tenant_db($tenant->slug);
    }
    return $data;
});

// Detect change in custom domain and add/delete in cpanel accordingly if there is valid change
hooks()->add_filter('fq_saas_module_tenant_data_payload', function ($payload) {

    $data = $payload['data'];
    $tenant = $payload['tenant'];
    $invoice = $payload['invoice'];

    $addondomain_mode = get_option('fq_saas_cpanel_addondomain_mode');
    ...
    if (!$tenant || !fq_saas_cpanel_enable_addondomain() || !$can_use_custom_domain) return $payload;

    if (!fq_saas_cpanel_allowed_for_tenant($tenant)) return $payload;
    ...
    if ($new_domain !== $old_domain) {
        ...
        if (!empty($old_domain)) {
            ...
            try {
                fq_saas_cpanel_delete_domain($cpanel, $slug . 'addon', $old_domain);
            } catch (\Throwable $th) {
                log_message('error', $th->getMessage());
...
hooks()->add_action('fq_saas_module_tenant_deployed', function ($data) {
    $tenant = $data['tenant'];
    $invoice = $data['invoice'];
...
hooks()->add_action('fq_saas_module_tenant_removed', 'fq_saas_cpanel_rollback');
hooks()->add_action('fq_saas_module_tenant_removal_failed', 'fq_saas_cpanel_rollback');
hooks()->add_action('fq_saas_module_tenant_deploy_failed', 'fq_saas_cpanel_rollback');
