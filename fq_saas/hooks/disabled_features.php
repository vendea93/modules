...
if ($is_tenant) {
     ...
     */
    $disabled_features = fq_saas_tenant_disabled_default_modules(null, "menu");
    $GLOBALS['disabled_features'] = $disabled_features;
    hooks()->add_filter("sidebar_menu_items", "fq_saas_remove_disabled_modules_menu_filter");
    hooks()->add_filter("setup_menu_items", "fq_saas_remove_disabled_modules_menu_filter");
    hooks()->add_filter('theme_menu_items', 'fq_saas_remove_disabled_modules_menu_filter');
    function fq_saas_remove_disabled_modules_menu_filter($items)
    {
    ...
    hooks()->add_action('app_admin_footer', function () use ($disabled_features, $CI) {
        if (in_array($CI->router->fetch_class(), ['dashboard', 'staff', 'profile'])) {
            ...
            echo '<script src=' . fq_saas_asset_url('js/disabled_features.js') . '></script>';
        }
    });
}