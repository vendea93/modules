...
if (!function_exists('fq_saas_rename_tenant_favicon')) {
    function fq_saas_rename_tenant_favicon($hook_data = null)
    {
        $CI = &get_instance();
        ...
        if (!empty($favicon) && str_starts_with($favicon, 'favicon')) {
            ...
            $filename = md5('favicon_' . fq_saas_tenant_slug() . '_' . time()) . '.' . $extension;
 ...
 */
hooks()->add_filter('before_settings_updated', 'fq_saas_rename_tenant_favicon');
 ...
 * 
 */
hooks()->add_action('before_cron_run', 'fq_saas_rename_tenant_favicon');