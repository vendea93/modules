...
$use_cache_file_enabled = (int)get_option('fq_saas_enable_tenant_seeding_caching');
if (!$use_cache_file_enabled) return;

// Check if the function fq_saas_tenant_seed_cache_get_folder does not already exist
if (!function_exists('fq_saas_tenant_seed_cache_get_folder')) {
    /**
     ...
     */
    function fq_saas_tenant_seed_cache_get_folder($hash_key = '')
    {
...
// Check if the function fq_saas_tenant_seed_cache_clear_cache does not already exist
if (!function_exists('fq_saas_tenant_seed_cache_clear_cache')) {
    /**
     ...
     */
    function fq_saas_tenant_seed_cache_clear_cache()
    {
        // Get the path to the seeding_cache directory
        $folder_path = fq_saas_tenant_seed_cache_get_folder();

        // Remove the directory and return the result
        return fq_saas_remove_dir($folder_path);
    }
}

// Check if the function fq_saas_tenant_seed_cache_get_cache_file_name does not already exist
if (!function_exists('fq_saas_tenant_seed_cache_get_cache_file_name')) {
    /**
     ...
     */
    function fq_saas_tenant_seed_cache_get_cache_file_name($purpose)
    {
...
// Check if the function fq_saas_tenant_seed_cache_generate_cache does not already exist
if (!function_exists('fq_saas_tenant_seed_cache_generate_cache')) {
    /**
     * Generate the seed cache for the tenant.
     */
    function fq_saas_tenant_seed_cache_generate_cache()
    {
        ...
        $seed_source = get_option('fq_saas_tenant_seeding_source');
        if ($seed_source == FQ_SAAS_SEED_SOURCE_TENANT && !empty($source_tenant_slug = get_option('fq_saas_seeding_tenant'))) {
            // If seeding from a tenant, get the tenant's database prefix and DSN
            $dbprefix = fq_saas_tenant_db_prefix($source_tenant_slug);
            $source_tenant = get_instance()->fq_saas_model->get_company_by_slug($source_tenant_slug);
        ...
        $folder_path = fq_saas_tenant_seed_cache_get_folder($hash_key);
        $setup_file = $folder_path . fq_saas_tenant_seed_cache_get_cache_file_name('setup');
        // If the setup file does not exist, trigger the setup cache
        if (!file_exists($setup_file)) {
            fq_saas_setup_dsn($dsn, $slug, [], true);
        }

        // Determine if the source is the super admin
        $hash_key = empty($source_dsn) ? 'super_admin' : (is_string($source_dsn) ? $source_dsn : json_encode($source_dsn));
        $folder_path = fq_saas_tenant_seed_cache_get_folder($hash_key);
        $setup_file = $folder_path . fq_saas_tenant_seed_cache_get_cache_file_name('seed');
        ...
        if (!file_exists($setup_file)) {
            if ($seed_source != FQ_SAAS_SEED_SOURCE_FILE) {
                fq_saas_seed_tenant_from_dsn((object)['slug' => $slug], $dsn, $source_dsn, $dbprefix, true);
            }
        }
    }
}
...
hooks()->add_filter('fq_saas_dsn_setup_source_dsn_queries', function ($data) {
    $dsn = $data['dsn'] ?? '';
    ...
    $hash_key = empty($source_dsn) ? 'super_admin' : (is_string($source_dsn) ? $source_dsn : json_encode($source_dsn));
    $folder_path = fq_saas_tenant_seed_cache_get_folder($hash_key);
    $cache_file = $folder_path . fq_saas_tenant_seed_cache_get_cache_file_name('setup');
    ...
    hooks()->add_filter('fq_saas_after_dsn_setup_source_dsn_queries', function ($data) use ($cache_file, $placeholder) {
...
hooks()->add_filter('fq_saas_before_tenant_seeding_from_dsn', function ($data) use ($seed_source) {

    if ($seed_source == FQ_SAAS_SEED_SOURCE_FILE) return $data;
    ...
    $hash_key = empty($source_dsn) ? 'super_admin' : (is_string($source_dsn) ? $source_dsn : json_encode($source_dsn));
    $folder_path = fq_saas_tenant_seed_cache_get_folder($hash_key);
    $cache_file = $folder_path . fq_saas_tenant_seed_cache_get_cache_file_name('seed');

    if (!empty($dsn) && file_exists($cache_file) && !$return_queries_only) {
        $total_queries_ran = fq_saas_seed_tenant_from_sql_files($company, $dsn, $cache_file);
        if ($total_queries_ran > 0)
            return []; // Return empty array to ensure the proccess is continued as we already seed the tenant
    }

    hooks()->add_filter('fq_saas_tenant_seeding_queries', function ($data) use ($cache_file) {
        $queries = $data['queries'] ?? [];
        if (!empty($queries)) {
            ...
            if (!empty($cache_file)) {
                $CI = &get_instance();
                $CI->load->helper('file');
                write_file($cache_file, str_replace($tenant_dbprefix, fq_saas_master_db_prefix(), implode("\n", $queries)));
            }
        }
...
// Trigger caching
hooks()->add_action('fq_saas_after_cron', 'fq_saas_tenant_seed_cache_generate_cache');

// Clear cache on installer run
hooks()->add_action('fq_saas_before_installer_run', 'fq_saas_tenant_seed_cache_clear_cache');

// Clear cache when seeding option changes
hooks()->add_filter('before_settings_updated', function ($data) {

    $should_clear_cache = false;
    $fields = ['fq_saas_tenant_seeding_source', 'fq_saas_seeding_tenant', 'fq_saas_tenants_seed_tables'];

    if (isset($data['clear-seed-cache'])) {
        unset($data['clear-seed-cache']);
        //fq_saas_tenant_seed_cache_clear_cache();
        fq_saas_tenant_seed_cache_generate_cache();
        redirect(uri_string());
        exit;
    }
    ...
    if ($should_clear_cache) fq_saas_tenant_seed_cache_clear_cache();
