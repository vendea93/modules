...
if ($is_tenant) {
    ...
    if (empty($theme_name) || !is_dir(VIEWPATH . 'themes/' . $theme_name)) {
        $new_theme_name = fq_saas_get_options($settings_key);
...
hooks()->add_action('fq_saas_after_tenant_seeding', function ($data) {

    $settings_key = 'clients_default_theme';
    if ($data['seed_source'] == FQ_SAAS_SEED_SOURCE_FILE && !empty($data['dsn'])) {
        $theme_name = fq_saas_get_options($settings_key);
        $options_table = fq_saas_tenant_db_prefix($data['company']->slug) . "options";
        $query = "UPDATE `" . $options_table . "` SET `value`= '$theme_name' WHERE `name` = '$settings_key'";
        fq_saas_raw_query($query, $data['dsn']);
    }
});
...
hooks()->add_filter('before_settings_updated', function ($data) {
    ...
    $field = 'fq_saas_clients_default_theme_whitelabel_name';
    if (isset($data['settings'][$field])) {
        ...
        fq_saas_whitelabel_clients_default_theme($new_theme_name, $old_theme_name);
    }
 ...
 */
hooks()->add_action('fq_saas_before_installer_run', function () {
    $field = 'fq_saas_clients_default_theme_whitelabel_name';
    $old_theme_name = get_option($field);
    fq_saas_whitelabel_clients_default_theme($old_theme_name, $old_theme_name);
});

/**
 * Ensure the whitelabelling is reversed when uninstalling or deactivating the SaaS
 */
hooks()->add_action('fq_saas_before_uninstaller_run', function ($clean) {
    $field = 'fq_saas_clients_default_theme_whitelabel_name';
    $old_theme_name = get_option($field);
    fq_saas_whitelabel_clients_default_theme('', $old_theme_name);
});
 ...
 * @return void
 */
function fq_saas_whitelabel_clients_default_theme($new_theme_name, $old_theme_name)
{
    ...
    $move_theme_folder = function ($base_path, $update_option = false) use ($default_theme_name, $old_theme_name, $new_theme_name) {
        ...
        if (is_dir($base_path . $default_theme_name) && $new_theme_name !== $default_theme_name && $old_theme_name !== $default_theme_name) {
            if (xcopy($base_path . $default_theme_name, $to)) {
                fq_saas_remove_dir($base_path . $default_theme_name);
            }
        }
