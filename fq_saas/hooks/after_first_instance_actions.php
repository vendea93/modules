...
if (!$is_tenant && $is_client) {
    ...
    if (!function_exists('fq_saas_after_first_instance')) {
        /**
         ...
         * @return void
         */
        function fq_saas_after_first_instance($company)
        {
            $CI = &get_instance();

            $flag = 'first_instance_redirection';

            $redirect_url = get_option('fq_saas_after_first_instance_redirect_url');
    ...
    hooks()->add_action('fq_saas_after_client_create_instance', function ($id) use ($CI) {
        if ($id) {
            $companies = $CI->fq_saas_model->companies();
            if (count($companies) === 1 && is_contact_email_verified()) {

                $new_company = $companies[0];
                fq_saas_after_first_instance($new_company);
            }
        }
    });

    // Handle other cases
    $client_companies = $CI->fq_saas_model->companies();
    if (count($client_companies) === 1) {
        $new_company = $client_companies[0];
        if ($new_company->status === FQ_SAAS_STATUS_PENDING && is_contact_email_verified()) {
            fq_saas_after_first_instance($new_company);
        }
    }
}
