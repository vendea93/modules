...
$client_bridge = $is_tenant ? fq_saas_tenant_is_enabled('client_bridge') : (int)fq_saas_get_options('fq_saas_enable_client_bridge');
if ($client_bridge) {
    ...
    if ($is_tenant) {
        ...
        hooks()->add_action('admin_init', function () use ($CI) {
            if (is_admin()) {
                ...
                if (($tenant->saas_options['fq_saas_client_bridge_account_menu_position'] ?? '') === 'sidebar') {
                ...
                $child_menu_items = [
                    [
                        ...
                        'name'     => _l('fq_saas_client_menu_overview'),
                    ...
                    [
                        ...
                        'name'     => _l('fq_saas_client_menu_invoices'),
                ...
                if (fq_saas_tenant_get_super_option('show_subscriptions_in_customers_area') == '1' && !empty($tenant->package_invoice->stripe_subscription_id)) {
                    $child_menu_items[] = [
                        'slug'     => 'saas_client_invoices',
                ...
                $child_menu_items = hooks()->apply_filters('fq_saas_tenant_account_menu', $child_menu_items);

                if (!empty($child_menu_items)) {
                    $CI->app_menu->$method('my_account', [
                        'collapse' => true,
                        'name'     => _l('fq_saas_my_account'),
        ...
        hooks()->add_action('app_admin_footer', function () {
            ...
            if (fq_saas_tenant()->http_identification_type === FQ_SAAS_TENANT_MODE_DOMAIN && !$support_custom_domain_magic_login) {
                echo '
                        <script>
    ...
    if (!$is_tenant && $is_client) {
        ...
        $invoice = $CI->fq_saas_model->get_company_invoice(get_client_user_id());
        $on_trial_and_ended = isset($invoice->on_trial) && $invoice->on_trial && fq_saas_get_days_until($invoice->duedate) <= 0;
        $invoice_overdue = $invoice && !$invoice->is_private && fq_saas_is_invoice_overdue_for_payment($invoice);
        ...
        hooks()->add_action('clients_init', function () use ($has_outstanding) {
            ...
            if ($is_magic_auth) {
                ...
                if (!$cross_domain && !$has_outstanding && (int)get_option('fq_saas_enable_client_menu_in_bridge') !== 1) {
        ...
        if (
            ...
            foreach ($companies as $key => $company) {
                if ($company->status === FQ_SAAS_STATUS_ACTIVE) {
                    $primary_active_instance = $company;
                }
            }

            // Auto signin and redirect
            if ($primary_active_instance && fq_saas_contact_can_magic_auth($primary_active_instance)) {
                $urlmode = 'auto';
    ...
    if ($is_tenant) {
        ...
        hooks()->add_action('after_user_logout', function () use (&$is_admin, &$is_magic_auth) {
            if ($is_admin) {
                ...
                redirect(fq_saas_default_base_url('authentication/logout' . $query), []);
            }
        });
    } else {
