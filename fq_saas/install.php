...
add_option('fq_saas_enable_auto_trial', '1');
add_option('fq_saas_autocreate_first_company', '1');
add_option('fq_saas_reserved_slugs', 'www,app,deal,controller,master,ww3,hack');
add_option('fq_saas_control_client_menu', '1');
add_option('fq_saas_cron_cache', '');
add_option('fq_saas_tenants_seed_tables', '');
add_option('fq_saas_sensitive_options', '');
add_option('fq_saas_enable_single_package_mode', '0');
add_option('fq_saas_enable_client_menu_in_bridge', '0');
add_option('fq_saas_enable_custom_module_request', '1');
add_option('fq_saas_enable_deploy_splash_screen', '1');
add_option('fq_saas_deploy_splash_screen_theme', 'verbose');
add_option('fq_saas_enable_client_menu_in_interim_pages', '0');
add_option('fq_saas_client_bridge_account_menu_position', 'setup');
add_option('fq_saas_registered_global_active_modules', '[]');
add_option('fq_saas_masked_settings_pages', '');
add_option('fq_saas_autolaunch_instance', 'new');
add_option('fq_saas_restricted_clients_id', '', 1);
add_option('fq_saas_tenant_seeding_source', 'master');
add_option('fq_saas_instance_delete_pending_days', 0);
add_option('fq_saas_allow_customer_cancel_subscription', 1);
add_option('fq_saas_enable_api', 1);
add_option('fq_saas_api_allow_public_access_to_doc', 1);
add_option('fq_saas_landing_builtin_slug', '');

// From v0.1.1
add_option('fq_saas_enable_client_bridge', '1');
add_option('fq_saas_enable_cross_domain_bridge', '0');
add_option('fq_saas_enable_instance_switch', '1');

add_option('fq_saas_enable_package_grouping', '1');
...
add_option('fq_saas_custom_domain_guide', $custom_domain_guide_for_client);
...
if (!$CI->db->table_exists(fq_saas_table('packages'))) {
    $CI->db->query(
        "CREATE TABLE IF NOT EXISTS `" . fq_saas_table('packages') . "` (
...
if (!$CI->db->table_exists(fq_saas_table('companies'))) {
    $CI->db->query(
        "CREATE TABLE IF NOT EXISTS `" . fq_saas_table('companies') . "` (
            `id` int NOT NULL AUTO_INCREMENT,
...
// Add status note
if (!$CI->db->field_exists('status_note', fq_saas_table('companies'))) {
    $CI->db->query(
        "ALTER TABLE `" . fq_saas_table('companies') . "` ADD `status_note` TEXT AFTER `status`;"
    );
}
...
if (!$CI->db->field_exists(fq_saas_column('packageid'), db_prefix() . 'invoices')) {
    $CI->db->query(
        "ALTER TABLE `" . db_prefix() . "invoices` ADD `" . fq_saas_column('packageid') . "` INT NULL DEFAULT NULL AFTER `subscription_id`;"
    );
}
...
if (!$CI->db->table_exists(fq_saas_table('client_metadata'))) {
    $CI->db->query(
        "CREATE TABLE IF NOT EXISTS `" . fq_saas_table('client_metadata') . "` (
            `id` int NOT NULL AUTO_INCREMENT,
            `clientid` int NOT NULL,
            `metadata` text COMMENT 'Extra data',
            PRIMARY KEY (`id`),
...
if (!$CI->db->table_exists(fq_saas_table('api_users'))) {
    $CI->db->query(
        "CREATE TABLE IF NOT EXISTS `" . fq_saas_table('api_users') . "` (
...
// FQ SAAS extension tables (prefix fq_saas_ — not legacy perfex_saas core tables)
