...
function fq_saas_api_classes()
{
    return [module_dir_path(FQ_SAAS_MODULE_NAME, 'controllers/api/Main.php')];
}
 ...
 */
function fq_saas_api_openapi_instance($paths_to_scan = [])
{
    $paths_to_scan = !empty($paths_to_scan) ? $paths_to_scan : fq_saas_api_classes();
    $openapi = Generator::scan($paths_to_scan);
    return $openapi;
}

function fq_saas_api_endpoints_specs()
{
    $openapi = fq_saas_api_openapi_instance();
    $CI = &get_instance();

    $session_key = 'fq_saas_api_endpoints_specs';
    ...
    foreach ($openapi->paths as $path => $pathItem) {
        ...
        foreach (['get', 'post', 'put', 'delete', 'patch', 'options', 'head'] as $method) {
            if (isset($pathItem->$method)) {
                ...
                foreach (fq_saas_api_classes() as $class_file) {
 ...
 * @return array|object
 */
function fq_saas_api_users($id = '')
{
    $CI = get_instance();
    $table = fq_saas_table('api_users');
    $users = $CI->fq_saas_model->get($table, $id);
    if ($id && $users) {
 ...
 */
function fq_saas_api_users_by_token($token = '')
{
    $CI = get_instance();
    $table = fq_saas_table('api_users');
    $CI->fq_saas_model->db->where('token', $token);
    $user = $CI->fq_saas_model->get($table);
