 ...
 */
function fq_saas_get_company_dsn($company = null, $invoice = null)
{
    $default_dsn = fq_saas_master_dsn();
    $CI = &get_instance();

    if (!empty($company->dsn)) {
        $dsn = fq_saas_parse_dsn($company->dsn);
        ...
        if (fq_saas_is_valid_dsn($dsn) === true) {
            return $dsn;
        }
        ...
        if (isset($dsn['dbname']) && $dsn['dbname'] == fq_saas_db($company->slug)) {
    ...
    if (empty($company->dsn) && !empty($invoice)) {
        $invoice = is_null($invoice) ? $CI->fq_saas_model->get_company_invoice($company->clientid) : $invoice;
        ...
        if (isset($invoice->db_scheme) && !empty($invoice->db_scheme)) {
            ...
            if ($db_scheme == 'single') {
                $default_dsn['dbname'] = fq_saas_db($company->slug);
                return $default_dsn;
            }

            $packageid = $invoice->{fq_saas_column('packageid')};

            list($populations, $pools) = !empty($invoice->db_pools) && !is_string($invoice->db_pools) ?  $CI->fq_saas_model->get_db_pools_population((array)$invoice->db_pools) :
                $CI->fq_saas_model->get_db_pools_population_by_packgeid($packageid);
            ...
            if ($db_scheme == 'single_pool') {
                if (((int)array_values($populations)[0]) != 0) {
                    ...
                    if (add_notification([
                        'touserid' => $staffid,
                        'description' => 'fq_saas_not_package_db_list_exhausted',
                        'link' => FQ_SAAS_MODULE_NAME . '/packages/edit/' . $packageid,
            ...
            $filter = hooks()->apply_filters('fq_saas_module_maybe_create_tenant_dsn', ['dsn' => $selected_pool, 'tenant' => $company, 'invoice' => $invoice]);
    ...
    throw new \Exception(_l('fq_saas_error_finding_valid_datacenter'), 1);
}
 ...
 * each company using the `fq_saas_deploy_company()` function. If the deployment is successful, the
 ...
 */
function fq_saas_deployer($company_id = '', $clientid = '', $limit = 5)
{
    if (fq_saas_is_tenant()) return;

    $CI = &get_instance();
    $CI->db->where('status', FQ_SAAS_STATUS_PENDING);
    $CI->db->limit($limit);
    ...
    $pending_companies = $CI->fq_saas_model->companies($company_id);
    ...
    foreach ($pending_companies as $company) {
        try {
            ...
            if (!$contact || ($contact && is_null($contact->email_verified_at))) {
                $_status_note = _l('fq_saas_deployment_requires_active_contact');
                if ($company->status_note != $_status_note) {
                    ...
                    $CI->fq_saas_model->add_or_update('companies', ['id' => $company->id, 'status_note' => $_status_note]);
                }
                continue;
            }

            // Set to deploying
            $CI->fq_saas_model->add_or_update('companies', ['id' => $company->id, 'status' => FQ_SAAS_STATUS_DEPLOYING]);

            // Attempt deploy
            $deploy = fq_saas_deploy_company($company);
            ...
            $CI->fq_saas_model->add_or_update('companies', ['id' => $company->id, 'status' => FQ_SAAS_STATUS_ACTIVE, 'status_note' => '']);

            $total_deployed += 1;

            $url = fq_saas_tenant_base_url($company, 'cron/index?setup=1', 'path');

            try {
                // Install modules by triggering cron
                fq_saas_trigger_module_install('*', $company->slug);
                $cron_req = fq_saas_http_request($url, ['timeout' => 10]);
            } catch (\Throwable $th) {
            }
            ...
            try {
                ...
                $company = $CI->fq_saas_model->companies($company->id);
                fq_saas_remove_company($company);
            } catch (\Throwable $th) {
            }

            try {
                $CI->fq_saas_model->delete('companies', $company->id);
            } catch (\Throwable $th) {
            }
        }
    }
 ...
 */
function fq_saas_deploy_company($company, $silent_mode = false)
{
    try {

        //Clear
        fq_saas_deploy_step($company->slug, '');

        $CI = &get_instance();
        $invoice = $CI->fq_saas_model->get_company_invoice($company->clientid);


        // Seeding source
        $seed_source = get_option('fq_saas_tenant_seeding_source');
        $seed_source_reference = '';
        if ($seed_source == FQ_SAAS_SEED_SOURCE_TENANT)
            $seed_source_reference = get_option('fq_saas_seeding_tenant');

        if (!empty($invoice->metadata->seeding_tenant)) {
            $seed_source = FQ_SAAS_SEED_SOURCE_TENANT;
            $seed_source_reference = $invoice->metadata->seeding_tenant;
        }

        // Get data center
        fq_saas_deploy_step($company->slug, _l('fq_saas_detecting_appropriate_datacenter'));
        $dsn = fq_saas_get_company_dsn($company, $invoice);

        fq_saas_deploy_step($company->slug, _l('fq_saas_validating_datacenter'));
        if (!fq_saas_is_valid_dsn($dsn, true))
            throw new \Exception(_l('fq_saas_invalid_datacenter'), 1);
        ...
        if (isset($dsn['source']) && $dsn['source'] == 'pool') {
            ...
            $data = ['id' => $company->id, 'dsn' => $CI->encryption->encrypt(fq_saas_dsn_to_string($dsn))];
            $CI->fq_saas_model->add_or_update('companies', $data);
        }

        // Setup the db structure (i.e tables)
        fq_saas_deploy_step($company->slug, _l('fq_saas_preparing_datacenter_for_installation'));
        fq_saas_setup_dsn($dsn, $company->slug);

        fq_saas_deploy_step($company->slug, _l('fq_saas_deploying_seed_to_datacenter'));
        fq_saas_setup_seed($company, $dsn, $seed_source, $seed_source_reference);
        hooks()->do_action(
            'fq_saas_after_tenant_seeding',
            [
        ...
        fq_saas_deploy_step($company->slug, _l('fq_saas_securing_installation_settings'));
        fq_saas_clear_sensitive_data($company, $dsn, $invoice, $seed_source);


        fq_saas_deploy_step($company->slug, _l('fq_saas_registering_first_administrative_user'));
        fq_saas_setup_tenant_admin($company, $dsn);

        fq_saas_deploy_step($company->slug, _l('fq_saas_preparing_push_notifications'));
        ...
        if (!$silent_mode) {
            ...
            if (add_notification([
                'touserid' => $staffid,
                'description' => 'fq_saas_not_customer_create_instance',
            ...
            fq_saas_deploy_step($company->slug, _l('fq_saas_sending_push_notification_to_the_company_and_superadmin'));
            pusher_trigger_notification($notifiedUsers);

            fq_saas_deploy_step($company->slug, _l('fq_saas_sending_email_notification_to_the_company_contact_and_superadmin'));

            // Send email to customer about deployment
            $contact = fq_saas_get_primary_contact($company->clientid);

            if (!empty($contact->email)) {
                send_mail_template('customer_deployed_instance', FQ_SAAS_MODULE_NAME, $contact->email, $company->clientid, $contact->id, $company);
            }

            // Send email to admin about the deployment
            if (!empty($admin->email)) {
                send_mail_template('customer_deployed_instance_for_admin', FQ_SAAS_MODULE_NAME, $admin->email, $company->clientid, $contact->id, $company);
            }

            fq_saas_deploy_step($company->slug, _l('fq_saas_complete'));


            hooks()->do_action('fq_saas_module_tenant_deployed', ['tenant' => $company, 'invoice' => $invoice]);
            fq_saas_log('tenant_deployed', ['tenant_id' => $company->id, 'slug' => $company->slug]);
        }
        ...
        if (!$silent_mode) {
            try {
                ...
                if (add_notification([
                    'touserid' => $staffid,
                    'description' => 'fq_saas_not_customer_create_instance_failed',
                    'link' => 'clients/client/' . $company->clientid,
            ...
            hooks()->do_action('fq_saas_module_tenant_deploy_failed', $company);
            fq_saas_log('tenant_deploy_failed', ['tenant_id' => $company->id ?? null, 'slug' => $company->slug ?? '']);
        }
 ...
 * @return void
 */
function fq_saas_deploy_step(string $slug, string $step = '')
{
 ...
 * @return array
 */
function fq_saas_get_deploy_step(string $slug): array
{
 ...
 */
function fq_saas_remove_company($company, $silent_mode = false)
{
    ...
    try {
        ...
        if (!isset($company->slug) || empty($company->slug))
            throw new \Exception(_l('fq_saas_company_slug_is_missing_!'), 1);
        ...
        fq_saas_deploy_step($company->slug, _l('fq_saas_removing_data_from_datacenter'));
        ...
        foreach ($tables as $table) {
            if (str_starts_with($table, $tenant_dbprefix)) {
                fq_saas_raw_query("DROP TABLE $table", $dsn, false, false, null, true);
                $deleted_tables++;
            }
        }
        ...
        if (
            $dsn['dbname'] === fq_saas_db($slug) &&
            count($tables) === $deleted_tables
        ) {
            try {
                // Drop database if using single and not other tenant on the db
                fq_saas_raw_query("DROP DATABASE `" . $dsn['dbname'] . '`', $dsn, false, false, null, true);
            } catch (\Throwable $th) {
                log_message("error", $th->getMessage());
            }
        }
        ...
        $tenant_upload_folder = fq_saas_tenant_upload_base_path($company);
        fq_saas_remove_dir($tenant_upload_folder);

        // Storage clean: Clear tenants media upload
        $tenant_media_folder =  fq_saas_tenant_media_base_path($company);
        fq_saas_remove_dir($tenant_media_folder);
        ...
        if (!$silent_mode) {
            ...
            if (add_notification([
                'touserid' => $staffid,
                'description' => 'fq_saas_not_customer_instance_removed',
                'link' => 'clients/client/' . $company->clientid,
                'additional_data' => serialize([$company->name])
            ])) {
                array_push($notifiedUsers, $staffid);
            ...
            $contact = fq_saas_get_primary_contact($company->clientid);
            if (!empty($contact->email)) {
                send_mail_template('customer_removed_instance', FQ_SAAS_MODULE_NAME, $contact->email, $company->clientid, $contact->id, $company);
            }

            // Send email to admin about the removal
            if (!empty($contact->id) && !empty($admin->email)) {
                send_mail_template('customer_removed_instance_for_admin', FQ_SAAS_MODULE_NAME, $admin->email, $company->clientid, $contact->id, $company);
            }

            fq_saas_deploy_step($company->slug, _l('fq_saas_complete'));

            hooks()->do_action('fq_saas_module_tenant_removed', $company);
            fq_saas_log('tenant_removed', ['tenant_id' => $company->id, 'slug' => $company->slug]);
        }
        ...
        if (!$silent_mode) {
            ...
            hooks()->do_action('fq_saas_module_tenant_removal_failed', $company);
        }
 ...
 */
function fq_saas_master_default_tables($folder_path = '')
{
    // If the folder path is not provided, use the default path for the Perfex Saas module's default seeds.
    $folder_path = empty($folder_path) ? module_dir_path(FQ_SAAS_MODULE_NAME, 'migrations/default_seeds') : $folder_path;
 ...
 * @return void
 */
function fq_saas_setup_dsn($dsn, $slug, $source_dsn = [], $return_queries_only = false)
{
    $dbprefix = fq_saas_master_db_prefix();

    // List all tables from master 
    $master_tables = fq_saas_master_default_tables();

    // Get tables with seed and join with master tables
    $master_tables = array_unique(array_merge($master_tables, fq_saas_seed_tables()));

    $show_create_queries = [];

    $hooks_payload = ['dsn' => $dsn, 'source_dsn' => $source_dsn, 'slug' => $slug, 'tables' => $master_tables, 'queries' => $show_create_queries];
    $filter = hooks()->apply_filters('fq_saas_dsn_setup_source_dsn_queries', $hooks_payload);
    ...
    if (empty($show_create_queries)) {
        foreach ($master_tables as $table) {
            ...
            if (str_starts_with($table, fq_saas_table('')) || !str_starts_with($table, $dbprefix)) continue;
            ...
            try {
                ...
                $row = fq_saas_raw_query_row($query, $source_dsn, true);
                $new_table =  fq_saas_tenant_db_prefix($slug, $table);
        ...
        $filter = hooks()->apply_filters('fq_saas_after_dsn_setup_source_dsn_queries', $hooks_payload);
        $show_create_queries = (array)($filter['queries'] ?? []);
    ...
    return fq_saas_raw_query($sql_commands_to_run, $dsn, false, false, null, true, true);
}
 ...
 * @return array
 */
function fq_saas_default_seed_tables($dbprefix = '')
{
    $dbprefix = !empty($dbprefix) ? $dbprefix : fq_saas_master_db_prefix();
 ...
 * @return void
 */
function fq_saas_setup_seed($company, $dsn, $seed_source, $seed_source_reference = '')
{
    if ($seed_source == FQ_SAAS_SEED_SOURCE_FILE) {

        // Only seed from file
        return fq_saas_seed_tenant_from_sql_files($company, $dsn);
    }

    if ($seed_source == FQ_SAAS_SEED_SOURCE_TENANT && !empty($seed_source_reference)) {

        // Seed from a tenant
        $dbprefix = fq_saas_tenant_db_prefix($seed_source_reference);
        $source_tenant = get_instance()->fq_saas_model->get_company_by_slug($seed_source_reference);
        $source_dsn = fq_saas_get_company_dsn($source_tenant);
        return fq_saas_seed_tenant_from_dsn($company, $dsn, $source_dsn, $dbprefix);
    }

    // Seed from master
    $dbprefix = fq_saas_master_db_prefix();
    return fq_saas_seed_tenant_from_dsn($company, $dsn, [], $dbprefix);
}
 ...
 */
function fq_saas_seed_tenant_from_dsn($company, $dsn, $source_dsn, $dbprefix, $return_queries_only = false)
{
    $CI = &get_instance();

    $default_senstive_fields = fq_saas_sensitive_options_fields();

    $slug = $company->slug;
    $tenant_dbprefix = fq_saas_tenant_db_prefix($slug);
    $source_is_super_admin = $dbprefix === fq_saas_master_db_prefix() || empty($source_dsn);

    $hooks_payload = ['company' => $company, 'dsn' => $dsn, 'source_dsn' => $source_dsn, 'dbprefix' => $dbprefix, 'return_queries_only' => $return_queries_only];
    $filter = hooks()->apply_filters('fq_saas_before_tenant_seeding_from_dsn', $hooks_payload);
    ...
    $default_table_selectors = fq_saas_default_seed_tables($dbprefix);
    $seed_tables = fq_saas_seed_tables($dbprefix);
    ...
    foreach ($seed_tables as $table) {
        ...
        $rows = fq_saas_raw_query($q, $source_dsn, true, false);
        ...
        foreach ($rows as $row) {
            ...
            if ($table === $dbprefix . 'options') {
                ...
                if (str_starts_with($row['name'], 'fq_saas'))
                    continue;
            ...
            /*    $primary_col = array_keys($row)[0];
                ...
                $result = fq_saas_raw_query("SELECT * FROM $tenant_table WHERE " . implode(' AND ', $where) . ' LIMIT 1', $dsn, true);
                if (!empty($result)) {
                    continue;
                }
            */
    ...
    $filter = hooks()->apply_filters('fq_saas_tenant_seeding_queries', $hooks_payload);
    ...
    fq_saas_raw_query($queries, $dsn, false, false, null, true, false);
}
 ...
 */
function fq_saas_seed_tenant_from_sql_files($company, $dsn, $folder_path = '')
{
    $CI = &get_instance();
 ...
 */
function fq_saas_get_super_admin($slug = '', $dsn = [])
{
    $dbprefix = empty($slug) ? fq_saas_master_db_prefix() : fq_saas_tenant_db_prefix($slug);
    $table = $dbprefix . "staff";

    // Retrieve the super admin from the database
    return fq_saas_raw_query_row("SELECT * FROM `$table` WHERE `admin`='1' AND `active`='1' ORDER BY `staffid` ASC LIMIT 1", $dsn, true);
}
 ...
 * @return void
 */
function fq_saas_setup_tenant_admin($tenant, $dsn)
{
    $CI = &get_instance();

    $tenant_dbprefix = fq_saas_tenant_db_prefix($tenant->slug);
    $table = $tenant_dbprefix . "staff";

    $result = fq_saas_get_super_admin($tenant->slug, $dsn);
    ...
    if (!$contact) throw new \Exception(_l('fq_saas_error_getting_contact_to_be_used_as_administrator_on_the_new_instance'), 1);
    ...
    return fq_saas_raw_query($admin_insert_query, $dsn);
}
 ...
 * @return void
 */
function fq_saas_clear_sensitive_data($company, $dsn_array, $invoice = null, $seed_source = null)
{
    $slug = $company->slug;
    $tenant_dbprefix = fq_saas_tenant_db_prefix($slug);
    ...
    if (fq_saas_is_tenant_installation_secured($slug, $dsn_array)) return;
    ...
    if ($seed_source == FQ_SAAS_SEED_SOURCE_MASTER) {
        ...
        $sensitive_fields = fq_saas_sensitive_options_fields();
    ...
    $queries[] = "DELETE FROM `" . $options_table . "` WHERE `name` LIKE 'perfex_saas%' OR `name` LIKE 'fq_saas%'";
    ...
    fq_saas_raw_query($queries, $dsn_array, false, true);

    fq_saas_secure_tenant_installation($slug, $dsn_array);
}
 ...
 * @return array
 */
function fq_saas_sensitive_options_fields()
{
    $sensitive_fields = get_option('fq_saas_sensitive_options');
    if (empty($sensitive_fields)) {
        $sensitive_options = get_instance()->fq_saas_model->sensitive_shared_options();
 ...
 * @return void
 */
function fq_saas_secure_tenant_installation($slug, $dsn_array)
{
    $tenant_dbprefix = fq_saas_tenant_db_prefix($slug);
    $options_table = $tenant_dbprefix . "options";

    $flag_query = "INSERT INTO `" . $options_table . "` (`id`, `name`, `value`, `autoload`) VALUES (NULL, 'fq_saas_installation_secured', 'true', '0')";
    fq_saas_raw_query($flag_query, $dsn_array, false, true);
}
 ...
 * @return bool
 */
function fq_saas_is_tenant_installation_secured($slug, $dsn_array, $drop_option_table_if_not_fit = false)
{
    $tenant_dbprefix = fq_saas_tenant_db_prefix($slug);
    $options_table = $tenant_dbprefix . "options";
    ...
    try {
        ...
        $r = fq_saas_raw_query("SELECT `value` FROM $options_table WHERE `name`='fq_saas_installation_secured'", $dsn_array, true);
        $all_options = fq_saas_raw_query("SELECT `id` FROM $options_table", $dsn_array, true);
        $version = fq_saas_raw_query_row("SELECT `version` FROM $migration_table LIMIT 1;", $dsn_array, true)->version;
        ...
        if ($drop_option_table_if_not_fit && !empty($version) && $total_options < $mininum_perfex_option_count) {
            fq_saas_raw_query("TRUNCATE TABLE $options_table", $dsn_array);
        }
 ...
 * @return void
 */
function fq_saas_setup_modules_for_tenant($verbose = true)
{
    $CI = &get_instance();
    if (!fq_saas_is_tenant()) throw new \Exception("Error Processing Request: Invalid context for fq_saas_setup_modules_for_tenant", 1);

    $tenant = fq_saas_tenant();
    $modules = fq_saas_tenant_modules($tenant, false, false, false, false, true);
    if ($verbose)
    ...
    $disabled_modules = array_diff(fq_saas_tenant_modules($tenant, false, true, true), $modules);
 ...
 * @return void
 */
function fq_saas_reset_demo_instances()
{
    $CI = &get_instance();

    $key = 'fq_saas_demo_instance';
    ...
    foreach ($instances as $slug) {
        ...
        try {
            fq_saas_remove_company($company, true);
 ...
 * 
 */
function fq_saas_sync_tenant_with_seed($company)
{
