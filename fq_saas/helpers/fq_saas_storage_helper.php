 ...
 */
function fq_saas_get_directory_size($path)
{
    $total_size = 0;
 ...
 */
function fq_saas_storage_size_units()
{
 ...
 */
function fq_saas_format_storage_size($size)
{
 ...
 */
function fq_saas_convert_formatted_size_to_bytes($formatted_size)
{
    $units = fq_saas_storage_size_units();
 ...
 * @return string
 */
function fq_saas_get_upload_path_by_type($type, $fullpath = true)
{
    if ($type == 'module:backup') {
        $path = FQ_SAAS_TENANT_UPLOAD_BASE_FOLDER . 'backups' . '/';
        if (!is_dir($path)) {
            fq_saas_create_dir($path, FCPATH . 'backups/', true);
        }
        return $path;
    }
 ...
 * @return string
 */
function fq_saas_tenant_upload_base_path($tenant)
{
 ...
 */
function fq_saas_calculate_tenant_storage_size($tenant)
{
    ...
    $upload_base_path = fq_saas_tenant_upload_base_path($tenant);
    $upload_size_in_bytes = fq_saas_get_directory_size($upload_base_path);

    // Calculate media storage size.
    $media_base_path = fq_saas_tenant_media_base_path($tenant);
    $media_size_in_bytes = fq_saas_get_directory_size($media_base_path);
 ...
 * Use fq_saas_calculate_tenant_storage_size() for active calculation .Note that can often take longer
 *
 * @param object $tenant
 * @param bool $format
 * @return string The size in byte if not format or in human readable formatted when $format is true
 */
function fq_saas_tenant_used_storage($tenant, $format = false)
{
    // Get tenant's storage information.
    $storage_info = (object)($tenant->metadata->storage ?? []);
    $size = $storage_info->total ?? 0;
    return $format ? fq_saas_format_storage_size($size) : $size;
}
 ...
 * @return string
 */
function fq_saas_tenant_storage_limit($tenant)
{
    $size = $tenant->package_invoice->metadata->storage_limit->size ?? 0;
    $unit = $tenant->package_invoice->metadata->storage_limit->unit ?? fq_saas_storage_size_units()[0];
 ...
 * @return bool
 */
function fq_saas_tenant_storage_is_unlimited($tenant)
{
    $quota = fq_saas_tenant_storage_limit($tenant);
 ...
 */
function fq_saas_update_tenant_storage_size($tenant)
{
    // Calculate the current storage size for the tenant.
    $storage = fq_saas_calculate_tenant_storage_size($tenant);
 ...
 */
function fq_saas_tenant_has_enough_storage($tenant, $uploaded_files = [])
{
    // Calculate total used space including the currently uploading files.
    $total_used_space = fq_saas_tenant_used_storage($tenant, false);
    ...
    $max_allowed_space =  fq_saas_convert_formatted_size_to_bytes(fq_saas_tenant_storage_limit($tenant));
 ...
 * @return void
 */
function fq_saas_handle_tenant_404_static_file_request($tenant, $upload_folder, $tenants_upload_folder)
{
    ...
    $_url = fq_saas_url_origin($_SERVER, true, true); // full url with query
    $url = $_url;
    $base_url = fq_saas_url_origin($_SERVER);
    ...
    $path_signature = fq_saas_tenant_url_signature($tenant_slug);
        ...
        if (file_exists(FCPATH . urldecode($requested_file))) {
            ...
            if (!$is_tenant_upload_path) {
                ...
                if (!empty($shared_fields)) {
                    ...
                    $values = (array)fq_saas_get_options($shared_fields, false);
 ...
 * @return void
 */
function fq_saas_create_dir($path, $blueprint = '', $force_htaccess = false)
{
