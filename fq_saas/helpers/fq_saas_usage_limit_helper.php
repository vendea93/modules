 ...
 */
const FQ_SAAS_LIMIT_FILTERS_TABLES_MAP = [
...
const FQ_SAAS_LIMIT_FILTERS_TABLES_CREATED_DATE_MAP = [
    'contracts' => 'dateadded',
 ...
 */
function fq_saas_register_limitation_filters()
{
    foreach (FQ_SAAS_LIMIT_FILTERS_TABLES_MAP as $event => $table) {
        // Set priority to 0 as we want this to run before any other attached hooks to the filter.
        hooks()->add_filter($event, 'fq_saas_validate_limits', 0);
    }
    ...
    if (!fq_saas_tenant_storage_is_unlimited($tenant)) {
        ...
        $storage_validation_handler = function () {
            $tenant = fq_saas_tenant();
            load_admin_language();
            fq_saas_limit_reached_middleware(
                'storage',
                _l('fq_saas_storage_exhausted', fq_saas_tenant_used_storage($tenant, true) . '/' . fq_saas_tenant_storage_limit($tenant)),
                _l('fq_saas_quota_exhausted_cron', [$tenant->slug, "storage"])
            );
        };
        ...
        if (!empty($uploaded_files = $_FILES)) {
            ...
            $tenant = fq_saas_update_tenant_storage_size($tenant);

            // Check if user have enough storage left
            if (!fq_saas_tenant_has_enough_storage($tenant, $uploaded_files)) {
        ...
        hooks()->add_action('before_make_backup', function () use ($storage_validation_handler) { // Backup folder limit
            if (!fq_saas_tenant_has_enough_storage(fq_saas_tenant())) {
                $storage_validation_handler();
            }
        });
    }
 ...
 */
function fq_saas_validate_limits($data, $admin = null)
{
    // Get the active filter
    $filter = hooks()->current_filter();

    // Get the filter table
    $limit_name = FQ_SAAS_LIMIT_FILTERS_TABLES_MAP[$filter];
    ...
    $quota = fq_saas_tenant_resources_quota($tenant, $limit_name);

    // Ulimited pass
    if ($quota === -1) return $data;

    // Get count for the active tenant from table and match against package
    $usage = fq_saas_get_tenant_quota_usage($tenant, [$limit_name])[$limit_name];

    // If quota is exceeded, set flash and redirect back
    $reached_limit = $quota <= $usage;

    if ($reached_limit) {
        $msg_key = 'fq_saas_quota_exhausted';
        return fq_saas_limit_reached_middleware(
            $limit_name,
 ...
 */
function fq_saas_get_tenant_quota_usage($tenant, $limits = [], $package = null)
{
    $tenant_slug = $tenant->slug;

    /** The tenant db prefix */
    $dbprefix = fq_saas_tenant_db_prefix($tenant_slug);

    $usage_list = [];
    $queries = [];

    $limits = empty($limits) ? array_values(FQ_SAAS_LIMIT_FILTERS_TABLES_MAP) : $limits;
    $dsn = fq_saas_get_company_dsn($tenant, $package);
    ...
    foreach ($limits as $limit) {
        ...
        if ($apply_period_limit && $limit !== 'items') {
            $date_field = FQ_SAAS_LIMIT_FILTERS_TABLES_CREATED_DATE_MAP[$limit] ?? 'datecreated';
    ...
    $queries = hooks()->apply_filters('fq_saas_tenant_quota_usage_queries', $queries, ['tenant' => $tenant, 'limits' => $limits, 'package' => $package]);

    // Run queries and set limits
    $usages = fq_saas_raw_query($queries, $dsn, true);

    foreach ($usages as $limit => $usage) {
        $usage_list[$limit] = (int)$usage[0]->total;
    }

    $usage_list = hooks()->apply_filters('fq_saas_tenant_quota_usage', $usage_list, ['tenant' => $tenant, 'limits' => $limits, 'package' => $package]);
 ...
 * @return void
 */
function fq_saas_limit_reached_middleware($limit_name, $msg, $cron_msg)
{
    if (!defined('CRON')) {
 ...
 * @return int
 */
function fq_saas_tenant_resources_quota($tenant, $resources)
{
    ...
    $config = fq_saas_feature_limits_config();
    $plan  = fq_saas_feature_limits_plan_key($tenant);
    if (isset($config[$plan][$resources]['limit'])) {
        $cap = (int) $config[$plan][$resources]['limit'];
        if ($cap >= 0) {
            $quota = min($quota, $cap);
        }
    }
...
/**
 * Declarative limits from config/feature_limits.php (per plan slug).
 *
 * @return array<string, array<string, array{label?: string, limit: int}>>
 */
function fq_saas_feature_limits_config(): array
{
    static $cache = null;
    if ($cache !== null) {
        return $cache;
    }
    $path = module_dir_path(FQ_SAAS_MODULE_NAME) . 'config/feature_limits.php';
    $loaded = file_exists($path) ? include $path : [];
    $cache = is_array($loaded) ? $loaded : [];
    $cache = hooks()->apply_filters('fq_saas_feature_limits_config', $cache);

    return $cache;
}

function fq_saas_feature_limits_plan_key($tenant): string
{
    $meta = $tenant->package_invoice->metadata ?? (object) [];
    $key   = $meta->slug ?? ($meta->name ?? 'default');

    return (string) hooks()->apply_filters('fq_saas_feature_limits_plan_key', $key, ['tenant' => $tenant]);
}
 ...
 * @return int
 */
function fq_saas_get_tenant_instance_limit($package_invoice)
{
