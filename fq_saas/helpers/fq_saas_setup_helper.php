 ...
 */
function fq_saas_require_signature($file)
{
 ...
 */
function fq_saas_require_in_file_template($path, $only_if_exists = false)
{
    if ($only_if_exists) {
 ...
 */
function fq_saas_file_put_contents($path, $content)
{
 ...
 * @param bool    $only_if_exists When true, wrap require in file_exists(#path) so a removed module does not fatal the app.
 *
 * @return void
 */
function fq_saas_require_in_file($dest, $requirePath, $force = false, $position = false, $only_if_exists = false)
{
    if (!file_exists($dest)) {
        fq_saas_file_put_contents($dest, "<?php defined('BASEPATH') or exit('No direct script access allowed');\n");
    }

    if (file_exists($dest)) {
        $content = file_get_contents($dest);  // Fetch the content inside the file

        $template = fq_saas_require_in_file_template($requirePath, $only_if_exists);

        $exist = preg_match(fq_saas_require_signature($requirePath), $content);
        if ($exist && !$force) { // Check if this process has run once or not
            return;
        }

        $content = fq_saas_unrequire_in_file($dest, $requirePath);
        ...
        fq_saas_file_put_contents($dest, $content);
    }
}
 ...
 */
function fq_saas_unrequire_in_file($dest, $requirePath)
{
    if (file_exists($dest)) {
        $content = file_get_contents($dest); // Fetch the content inside the file
        $content = preg_replace(fq_saas_require_signature($requirePath), '', $content);
        fq_saas_file_put_contents($dest, $content);
        return $content;
    }
}
 ...
 * @return boolean
 */
function fq_saas_write_custom_constant($constant_name, $new_value, $allow_empty_defination = false)
{
    ...
    return fq_saas_file_put_contents($dest, "$content$signature");
}
 ...
 */
function fq_saas_db_driver_hook($forward)
{
    $path = BASEPATH . 'database/DB_driver.php';
    $find = '$this->_execute($sql)';
    $replace = '$this->_execute(fq_saas_db_query($sql))';
 ...
 * @throws Exception If an error occurs during the modification process.
 */
function fq_saas_config_constants_hook($forward)
{
    try {
        ...
        $files = [
            ...
            [
                ...
                'replace' => ["fq_saas_get_upload_path_by_type('ticket')"]
            ],
            [
                'path' => FCPATH . 'modules/backup/backup.php',
                'find' => ["FCPATH . 'backups' . '/'"],
                'replace' => ["fq_saas_get_upload_path_by_type('module:backup')"]
            ],
            [
                'path' => APPPATH . 'libraries/pdf/PDF_Signature.php',
                'find' => ["FCPATH . 'uploads/company/'"],
                'replace' => ["fq_saas_get_upload_path_by_type('company')"]
            ],
            [
                'path' => APPPATH . 'helpers/files_helper.php',
                'find' => ['FCPATH . $path'],
                'replace' => ["fq_saas_get_upload_path_by_type('project')"]
            ],
            [
                'path' => FCPATH . 'modules/exports/services/ContactCSVExport.php',
                'find' => ["'uploads/client_profile_images/'"],
                'replace' => ["fq_saas_get_upload_path_by_type('contact_profile_images',false)"]
            ],
            [
                'path' => APPPATH . 'helpers/staff_helper.php',
                'find' => ["'uploads/staff_profile_images/'"],
                'replace' => ["fq_saas_get_upload_path_by_type('staff',false)"]
            ],
            [
 ...
 * @return void
 */
function fq_saas_db_config_constant_hook($forward)
{
 ...
 */
function fq_saas_setup_master_db($forward, $return_queries = false)
{
    $queries = [];
    if ($forward === false) {
        ...
        if (!empty($dbs)) {
            try {
                $tenants = fq_saas_raw_query('SELECT * from ' . fq_saas_table('companies'), [], true);
                foreach ($tenants as $tenant) {
                    $db_name = fq_saas_db($tenant->slug);
    ...
    if (!empty($queries)) {
        fq_saas_raw_query($queries, [], false, false, null, true, false);
    }
}

function fq_saas_module_loader_custom_hook($forward)
{
 ...
 * The files are meant for customization by perfex. Exception to this is the DB driver hook. See: fq_saas_db_driver_hook()
 * 
 ...
 * @return void
 */
function fq_saas_install()
{
    hooks()->do_action('fq_saas_before_installer_run');

    $CI = &get_instance();
    require_once(__DIR__ . '/../install.php');

    // Trigger asset whitelabel path cache clearing to ensure new asset files get copied
    fq_saas_asset_url('', true);

    // Run database setups
    fq_saas_setup_master_db(true);

    // Require the base config (guarded so deleting the module folder does not brick the CRM before uninstall hooks run)
    fq_saas_require_in_file(APPPATH . 'config/app-config.php',  "FCPATH.'modules/" . FQ_SAAS_MODULE_NAME . "/config/app-config.php'", false, false, true);

    // Require the SAAS routes and hooks
    fq_saas_require_in_file(APPPATH . 'config/my_routes.php', "FCPATH.'modules/" . FQ_SAAS_MODULE_NAME . "/config/my_routes.php'", false, false, true);

    // Require the SAAS system level hooks
    fq_saas_require_in_file(APPPATH . 'config/my_hooks.php', "FCPATH.'modules/" . FQ_SAAS_MODULE_NAME . "/config/my_hooks.php'", false, false, true);

    // Set driver hook
    fq_saas_db_driver_hook(true);

    // DB Config constant replacements
    fq_saas_db_config_constant_hook(true);

    // Config constants control
    fq_saas_config_constants_hook(true);

    // Add Custom module loading control : @since v0.1.4
    fq_saas_module_loader_custom_hook(true);

    hooks()->do_action('fq_saas_after_installer_run');
}
 ...
 * @return void
 */
function fq_saas_uninstall($clean = false)
{
    hooks()->do_action('fq_saas_before_uninstaller_run', $clean);

    // Remove the base config and middleware
    fq_saas_unrequire_in_file(APPPATH . 'config/app-config.php', "FCPATH.'modules/" . FQ_SAAS_MODULE_NAME . "/config/app-config.php'");

    // Remove the SAAS routes and hooks
    fq_saas_unrequire_in_file(APPPATH . 'config/my_routes.php', "FCPATH.'modules/" . FQ_SAAS_MODULE_NAME . "/config/my_routes.php'");

    // Remove the SAAS system level hooks
    fq_saas_unrequire_in_file(APPPATH . 'config/my_hooks.php', "FCPATH.'modules/" . FQ_SAAS_MODULE_NAME . "/config/my_hooks.php'");

    // Now set driver hook, inject database hook to every query if multitenant option is enabled
    fq_saas_db_driver_hook(false);

    // Remove all config constant replacements
    fq_saas_db_config_constant_hook(false);

    // Remove config constants control
    fq_saas_config_constants_hook(false);

    // Remove module loading control : @since v0.1.4
    fq_saas_module_loader_custom_hook(false);

    if ($clean === true) {
        // Remove all data in the current active table and destroy all created company databases (if running super DB user)
        fq_saas_setup_master_db(false);
    ...
    hooks()->do_action('fq_saas_after_uninstaller_run', $clean);
}