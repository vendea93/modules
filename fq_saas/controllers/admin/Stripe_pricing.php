...
class Stripe_pricing extends AdminController
{
    ...
    function index()
    {
        ...
        if (empty($data['packages']))
            return redirect($single_pricing_mode ? admin_url(FQ_SAAS_ROUTE_NAME . '/pricing') : admin_url(FQ_SAAS_ROUTE_NAME . '/packages'));
        ...
        $data['title'] = _l('fq_saas_saas_pricing');
        $this->load->view('stripe/pricing', $data);
    }

    /**
     * Trigger mass synchronization of all Stripe-enabled packages.
     * Introduced in 0.3.8.
     *
     * Must be called via POST (CSRF protected by Perfex core). Returns JSON.
     */
    public function mass_sync()
    {
        ...
        try {
            $report = $this->fq_saas_stripe_model->mass_sync_packages_on_stripe();
    ...
    private function savePackageStripePricing()
    {
        ...
        foreach ($form_data['stripe']['manual_pricing'] as $key => $value) {
            ...
            if (in_array($value, $checked_price_ids))
                throw new \Exception(_l('fq_saas_stripe_unique_manual_price'), 1);
        ...
        $_id = $this->fq_saas_model->add_or_update('packages', $data);
        if ($_id) {

            try {
                $package = $this->fq_saas_model->packages($package_id);
                $this->fq_saas_stripe_model->setup_package_on_stripe($package);

                fq_saas_trigger_cron_process(FQ_SAAS_CRON_PROCESS_PACKAGE, $_id);
                set_alert('success', _l('updated_successfully', _l('fq_saas_package')));
            } catch (\Throwable $th) {
                set_alert('danger', $th->getMessage());
            }
