...
class Packages extends AdminController
{
    ...
    public function index()
    {
        ...
        if (!staff_can('view', 'fq_saas_packages')) {
            return access_denied('fq_saas_packages');
        }

        $data['title'] = _l('fq_saas_packages');
        $data['packages'] = $this->fq_saas_model->packages();
        $this->load->view('packages/manage', $data);
    }

    public function pricing()
    {
        if (!staff_can('edit', 'fq_saas_packages')) {
            return access_denied('fq_saas_packages');
        }

        $id = '';

        $default_package = $this->fq_saas_model->default_package();
        if (!$default_package) {

            $this->db->where('is_private !=', 1);
            $packages = $this->fq_saas_model->packages();
            if (!empty($packages[0])) {
                $this->fq_saas_model->mark_package_as_default($packages[0]->id);
                $default_package = $this->fq_saas_model->default_package();
            }
        }
    ...
    public function create()
    {
        ...
        if (!staff_can('create', 'fq_saas_packages')) {
            return access_denied('fq_saas_packages');
        }
    ...
    public function edit($id)
    {
        ...
        // Load package data and display edit form
        $data['title'] = _l('fq_saas_packages');
        $data['package'] = $this->fq_saas_model->packages($id);
        $data['staff']     = $this->staff_model->get('', ['active' => 1]);
    ...
    /**
     * Read-only view of declarative feature limits (config/feature_limits.php).
     */
    public function feature_limits()
    {
        if (!staff_can('view', 'fq_saas_packages')) {
            return access_denied('fq_saas_packages');
        }
        ...
        $data['title']   = _l('fq_saas_feature_limits_compare');
        $data['path']    = $path;
        $data['limits']  = file_exists($path) ? include $path : [];
        $data['limits']  = is_array($data['limits']) ? $data['limits'] : [];
        $this->load->view('packages/feature_limits', $data);
    }
    ...
    private function create_or_edit_package($id = '')
    {
        ...
        $this->form_validation->set_rules('price', _l('fq_saas_price'), 'required');
        ...
        $package = null;
        if (!empty($id)) {
            $package = $this->fq_saas_model->packages((int)$id);
        }
        ...
        $slug = fq_saas_generate_unique_slug($slug, 'packages', $id, 0, ['skip_table_compact' => true, 'max_length' => 150, 'delimiter' => $slug_delimiter]);
        $form_data['slug'] = $slug;
        ...
        try {
            ...
            if (isset($form_data['is_private']) && isset($form_data['is_default']) && $form_data['is_private'] == '1' && $form_data['is_default'] == '1')
                throw new \Exception(_l('fq_saas_no_private_default_package'), 1);
                ...
                if (!empty($pools)) {
                    ...
                    for ($i = 0; $i < count($pools['host']); $i++) {
                        if (!empty($pools['host'][$i]) && !empty($pools['user'][$i]) && !empty($pools['dbname'][$i])) {
                            ...
                            $dsn_string = fq_saas_dsn_to_string($pool);
                    ...
                    for ($j = 0; $j < count($_db_pools); $j++) {
                        ...
                        $valid = fq_saas_is_valid_dsn($pool);
                        if ($valid !== true) {
            ...
            $_id = $this->fq_saas_model->add_or_update('packages', $form_data);
            if ($_id) {

                if ($form_data['is_default'] == '1')
                    $this->fq_saas_model->mark_package_as_default($_id);
                ...
                if (!empty($id) && !empty($package)) {
                    ...
                    $this->invoices_model->db
                        ->where(fq_saas_column('packageid'), $id)
                    ...
                    fq_saas_trigger_cron_process(FQ_SAAS_CRON_PROCESS_PACKAGE, $_id);

                    hooks()->do_action('fq_saas_after_package_update', $package);
                }

                set_alert('success', _l(empty($id) ? 'added_successfully' : 'updated_successfully', _l($single_pricing_mode ? 'fq_saas_pricing' : 'fq_saas_package')));
                return redirect($single_pricing_mode ? uri_string() : admin_url(FQ_SAAS_ROUTE_NAME . '/packages/edit/' . $_id));
            }
        } catch (\Exception $e) {
    ...
    public function clone($id)
    {
        ...
        if (!empty($id)) {
            $clone_id = $this->fq_saas_model->clone('packages', (int)$id);

            hooks()->do_action('fq_saas_after_package_clone', ['id' => $id, 'new_clone_id' => $clone_id]);
        }

        return redirect(admin_url(FQ_SAAS_ROUTE_NAME . '/packages'));
    }
    ...
    public function delete()
    {
        ...
        if (!staff_can('delete', 'fq_saas_packages')) {
            return access_denied('fq_saas_packages');
        }
        ...
        if (!empty($id)) {
            ...
            $invoices = $this->invoices_model->get('', [fq_saas_column('packageid') => $id]);
            if (!empty($invoices)) {

                set_alert('danger', _l('fq_saas_can_not_delete_package_with_invoices'));
                return redirect(admin_url(FQ_SAAS_ROUTE_NAME . '/packages'));
            }

            if ($this->fq_saas_model->delete('packages', $id))
                set_alert('success', _l('deleted', _l('fq_saas_package')));
        }
    ...
    public function add_user_to_package()
    {
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('packageid', _l('fq_saas_package'), 'required');
            ...
            if ($this->form_validation->run() !== false) {
                ...
                try {
                    ...
                    $invoice = $this->fq_saas_model->generate_company_invoice($clientid, $packageid);
                    if (isset($invoice->action_url))
                        return redirect($invoice->action_url);

                    if (!empty($invoice)) {
                        set_alert('success', _l('added_successfully', _l('fq_saas_customer')));
