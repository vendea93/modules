 ...
 */
class Fq_saas_client extends ClientsController
{
    use Tenant_trait;
    ...
    public function __construct()
    {
        ...
        if (!fq_saas_client_can_use_saas()) {
    ...
    {
        if ($this->input->post() && fq_saas_contact_can_manage_instances()) {
    ...
    public function subscribe($packageslug)
    {
        ...
        if (!empty($ref)) {
            $this->session->set_userdata('fq_saas_ref_code', strtoupper(trim((string) $ref)));
        }
        $coupon = $this->input->get('coupon', true);
        if (!empty($coupon)) {
            $this->session->set_userdata('fq_saas_checkout_coupon', strtoupper(trim((string) $coupon)));
        }
    ...
    public function my_account()
    {
        ...
        if (!$invoice) {
            set_alert('warning', _l('fq_saas_no_invoice_client_for_client'));
    ...
    public function cancel_saas_subscription()
    {
        ...
        $redirect = fq_saas_default_base_url('clients');
    ...
    public function resume_saas_subscription()
    {
        ...
        $redirect_url = base_url('clients/' . (fq_saas_is_single_package_mode() ? 'my_account' : '?subscription'));
        ...
        // If there is no resumable invoice (e.g. Stripe subscription is fully canceled
        // and cannot be reactivated via Stripe's resume API), send the user to the
        // pricing/packages page so they can start a new subscription instead of being
        // silently bounced back with an error alert.
        if (isset($service['error']) && !empty($service['resume_redirect'])) {
            return redirect($service['resume_redirect']);
        }
    ...
    private function get_auth_company_by_slug($slug)
    {
        ...
        $company = $this->fq_saas_model->get_company_by_slug($slug, $clientid);
    ...
    private function client_redirect($redirect_url)
    {
        ...
        if ($this->session->has_userdata('magic_auth') && stripos($redirect_url, parse_url($this->redirect_url)['host']) === false) {
            if (stripos($redirect_url, '://') === false)
                $redirect_url = fq_saas_default_base_url($redirect_url);
