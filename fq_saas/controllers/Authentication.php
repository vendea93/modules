...
class Authentication extends ClientsController
{
    ...
    public function magic_auth($slug, $urlmode = 'path')
    {
        ...
        if (!is_client_logged_in() || fq_saas_is_tenant()) {
            fq_saas_show_tenant_error(_l('fq_saas_permission_denied'), _l('fq_saas_authentication_required_for_magic_login'), 404);
        }

        $company = $this->fq_saas_model->get_entity_by_slug('companies', $slug, 'parse_company');
        if (!$company) {
            fq_saas_show_tenant_error(_l('fq_saas_permission_denied'), _l('fq_saas_page_not_found'), 404);
        }

        // Ensure the company belongs to the logged in client
        if ($company->clientid !== get_client_user_id()) {
            fq_saas_show_tenant_error(_l('fq_saas_permission_denied'), '');
        }

        // Ensure the contact have access to login
        if (!fq_saas_contact_can_magic_auth($company)) {
            fq_saas_show_tenant_error(_l('fq_saas_permission_denied'), '');
        }

        $auth_code = fq_saas_generate_magic_auth_code($company->clientid);

        $support_custom_domain_magic_login = get_option('fq_saas_enable_cross_domain_bridge') == "1";
        $links = fq_saas_tenant_base_url($company, '', 'all');
        ...
        if ((time() - strtotime($company->created_at)) < 60 * 10 && $urlmode != 'path') { // if created less than 20min
            $package = $this->fq_saas_model->get_company_invoice($company->clientid);
            if (!empty($package->db_scheme) && in_array($package->db_scheme, ['cpanel', 'plesk'])) {

                $can_optimize = false;

                if ($package->db_scheme === 'cpanel')
                    $can_optimize = fq_saas_cpanel_enable_addondomain();

                if ($package->db_scheme === 'plesk')
                    $can_optimize = fq_saas_plesk_enable_aliasdomain();

                $integration_mode = get_option('fq_saas_' . $package->db_scheme . '_addondomain_mode');
    ...
    public function client_magic_auth()
    {
        ...
        try {
            ...
            if (fq_saas_is_tenant() || get_option('fq_saas_enable_client_bridge') !== "1") {
                throw new \Exception(_l('fq_saas_permission_denied'), 1);
            }
            ...
            $clientid = fq_saas_validate_and_authorize_magic_auth_code();
            $contact = fq_saas_get_primary_contact($clientid);

            if (!$contact) {
                throw new \Exception(_l('fq_saas_error_finding_primary_contact'), 1);
            }
            ...
            fq_saas_show_tenant_error(_l('fq_saas_authentication_error'), $th->getMessage());
        }
    }
    ...
    public function tenant_admin_magic_auth()
    {
        try {
            ...
            if ((int)fq_saas_tenant()->clientid !== $clientid) {
                throw new \Exception(_l('fq_saas_permission_denied'), 1);
            }

            // Sign in as the current tenant instance's admin
            fq_saas_tenant_admin_autologin();
