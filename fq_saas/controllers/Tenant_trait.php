...
trait Tenant_trait
{
    ...
    public function deployTenants(?int $clientid)
    {
        ...
        if ($is_status_check) {
            ...
            $status = fq_saas_get_deploy_step($slug);
        ...
        // Trigger deploy
        return fq_saas_deployer('', $clientid);
    }
    ...
    public function subscribeToPackage(int $clientid, string $packageslug)
    {
        try {
            ...
            $invoice = null;
            $package = $this->fq_saas_model->get_entity_by_slug('packages', $packageslug);
            if (!empty($package->id ?? ''))
                $invoice = (object)$this->fq_saas_model->generate_company_invoice($clientid, $package->id);
            ...
            if (!$invoice) {
                return [
                    'error' => _l('fq_saas_error_creating_invoice'),
            ...
            if (empty($companies)) {
                ...
                if (get_option('fq_saas_autocreate_first_company') == '1') {
                    ...
                    $data = hooks()->apply_filters('fq_saas_create_instance_data', $data);
                    ...
                    if (!empty($custom_domain)) {
                        $company = $this->fq_saas_model->companies($_id);
                        fq_saas_send_customdomain_request_notice($company, $custom_domain, $invoice);
                    }

                    hooks()->do_action('fq_saas_after_client_create_instance', $_id);
                }
            }

            $message = '';
            $redirect = base_url('clients?companies');
            if (!in_array($invoice->status, [Invoices_model::STATUS_PAID]) && !fq_saas_invoice_is_on_trial($invoice))
                $redirect = base_url(fq_saas_get_invoice_payment_endpoint($invoice));
    ...
    public function updateSubscription(int $clientid, $invoice = null, $package = null)
    {
        ...
        if (!$invoice)
            $invoice = $this->fq_saas_model->get_company_invoice($clientid);

        // Check if on trial, cancel trial and subscribe
        if (isset($invoice->on_trial) && $invoice->on_trial) {
            $invoice = (object)$this->fq_saas_model->generate_company_invoice($clientid, $invoice->trial_package_id);
        }
        ...
        if (empty($invoice->db_scheme)) {
            return [
                'error' => _l('fq_saas_no_invoice_client_for_client'),
                'redirect' => base_url('clients/?subscription')
            ];
        }
        ...
        if (!fq_saas_is_single_package_mode() && ($package->metadata->allow_customization ?? '') === 'no') {
            return [
                'error' => _l('fq_saas_permission_denied'),
                'redirect' => base_url('clients/?subscription')
            ];
        }
        ...
        try {
            ...
            if ($on_trial)
                $invoice = $this->fq_saas_model->generate_company_invoice($clientid, $package->id);
            ...
            /** @var Fq_saas_custom_limit $customization */
            $customization = $this->fq_saas_model->prepare_invoice_customization_items($package, $data_source);
            ...
            try {
                ...
                if (!empty($_client_metadata_dirty_keys)) {
                    ...
                    $client_metadata = fq_saas_get_or_save_client_metadata($clientid, $_client_metadata_update);
                }
            } catch (\Throwable $th) {
            ...
            $has_new_customization = fq_saas_arrays_are_different($new_customizations, $old_customizations);
             ...
             */
            $deferred_billing_status = get_option('fq_saas_deferred_billing_status');
            if (!empty($deferred_billing_status) && empty($invoice->subscription_id) && in_array($invoice->status, [Invoices_model::STATUS_PAID])) {
                ...
                $this->fq_saas_model->cancel_company_invoice($clientid);
                ...
                $re_create_at = fq_saas_get_recurring_invoice_next_date($invoice);
                if ($re_create_at !== false)
                    $extra_custom_data['duedate'] = $re_create_at;

                $invoice = $this->fq_saas_model->generate_company_invoice($clientid, $package->id, $extra_custom_data);
            ...
            // Update the invoice
            $update = $this->fq_saas_model->update_company_invoice($invoice, $clientid, $customization);
            if ($update) {
                ...
                $metadata = fq_saas_get_or_save_client_metadata($clientid, $data);
                if (!empty($metadata)) {
                    $invoice = $this->fq_saas_model->get_company_invoice($clientid);
                }

                $redirect = current_url();
                if (!in_array($invoice->status, [Invoices_model::STATUS_PAID, Invoices_model::STATUS_DRAFT]) && !fq_saas_invoice_is_on_trial($invoice))
                    $redirect = base_url(fq_saas_get_invoice_payment_endpoint($invoice));

                return [
                    'success' => _l('updated_successfully', _l('fq_saas_subscription')),
                    'redirect' => $redirect,
                    'invoice' => $invoice,
                    'package' => $package
                ];
    ...
    public function cancelSubscription(int $clientid)
    {
        try {
            ...
            $invoice = $this->fq_saas_model->cancel_company_invoice($clientid, $type);
            ...
            if ($invoice) {
                ...
                return [
                    'success' => _l('fq_saas_pricing_cancelled'),
                    'redirect' => '',
                    'invoice' => $invoice,
                    'package' => $package
                ];
    ...
    public function resumeSubscription(int $clientid)
    {
        try {
            // Check if the client has a subscription i.e invoice
            $invoice = $this->fq_saas_model->resume_company_invoice($clientid);
            ...
            throw new \Exception(_l('fq_saas_no_resumable_invoice'), 1);
        } catch (\Throwable $th) {
    ...
    private function validateClientInvoice(int $clientid)
    {
        ...
        if ($on_trial && $days_left < 1) {
            return [
                'error' => _l('fq_saas_trial_invoice_over_not')
            ];
        }

        if (!$on_trial && fq_saas_is_invoice_overdue_for_payment($invoice)) {
            return [
                'error' => _l('fq_saas_clear_unpaid_invoice_note'),
                'redirect' => base_url(fq_saas_get_invoice_payment_endpoint($invoice))
            ];
        }
    ...
    private function createOrUpdateCompany(int $clientid, $tenantid = '')
    {
        ...
        try {
            ...
            if ($_id) {
                ...
                $message = _l(empty($tenantid) ? 'added_successfully' : 'updated_successfully', _l('fq_saas_company'));
                return [
                    'success' => $message,
                    'redirect' => $this->redirect_url
                ];
