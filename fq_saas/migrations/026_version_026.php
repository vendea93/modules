...
class Migration_Version_026 extends App_module_migration
{
    ...
    public function up()
    {
        ...
        $clients = get_option('fq_saas_exempted_clients_id');
        $clients = empty($clients) ? [] : (array)json_decode($clients);
        $clients = array_filter($clients);
        if (!empty($clients)) {
            add_option('fq_saas_client_restriction_mode', 'exclusive');
        }

        // Rename
        $CI->db->where('name', 'fq_saas_exempted_clients_id');
        $CI->db->update(db_prefix() . 'options', ['name' => 'fq_saas_restricted_clients_id']);

        delete_option('fq_saas_enable_preloader');
