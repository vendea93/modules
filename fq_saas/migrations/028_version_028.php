...
class Migration_Version_028 extends App_module_migration
{
    ...
    public function up()
    {
        ...
        $api_key = get_option('fq_saas_api_key');

        $CI->fq_saas_model->db->where('token', $api_key);
        $existing = $CI->fq_saas_model->get(fq_saas_table('api_users'));
        ...
        if (!empty($api_key) && count($existing ?? []) <= 0) {
            ...
            $CI->fq_saas_model->add_or_update('api_users', $data);
        }

        delete_option('fq_saas_api_key');
        ...
        foreach ($drafts as $invoice) {
            if (fq_saas_get_or_save_client_metadata($invoice['clientid'], [
        ...
        foreach ($cancelled as $_invoice) {
            if ($_invoice[$package_column] > 0 && (int)$_invoice['recurring'] > 0 && empty($_invoice['is_recurring_from']))
                $CI->invoices_model->db->update(fq_saas_master_db_prefix() . 'invoices', ["recurring" => "0"], ['id' => $_invoice['id']]);
        }
    }
