...
class Migration_Version_007 extends App_module_migration
{
    ...
    private function migrationv007()
    {
        ...
        foreach ($databases as $dsn) {
            ...
            $sql = "SELECT DISTINCT " . FQ_SAAS_TENANT_COLUMN . " FROM " . $dbprefix . 'options';

            // Get all tenants on the table
            $tenants = fq_saas_raw_query($sql, $dsn, true);
            ...
            if (count($tenants) == 1 && $tenants[0]->{FQ_SAAS_TENANT_COLUMN} === $master_slug) {
            ...
            foreach ($tenants as $k => $tenant) {
                ...
                $tenant_id = $tenant->{FQ_SAAS_TENANT_COLUMN};
                ...
                fq_saas_setup_dsn($dsn, $tenant_id, $dsn);

                foreach ($tables as $key => $row) {

                    // Table name with prefix.
                    $table = $row->{$col};
                    $new_table = fq_saas_tenant_db_prefix($tenant_id, $table);

                    if (str_starts_with($table, fq_saas_table('')) || !str_starts_with($table, $dbprefix)) continue;

                    try {
                        fq_saas_raw_query('TRUNCATE TABLE ' . $new_table, $dsn, false, false, null, true, false);
                    } catch (\PDOException $th) {
                        //throw $th;
                    }

                    // Migrate tenant data from the original table to the new prefixed table
                    $sql_migrate = "SET SESSION sql_mode = ''; INSERT INTO `$new_table` SELECT * FROM `$table` WHERE `" . FQ_SAAS_TENANT_COLUMN . "` = '$tenant_id';";
                    if (fq_saas_raw_query($sql_migrate, $dsn, false, false, null, true)) {
                        fq_saas_raw_query("DELETE FROM `$table` WHERE `" . FQ_SAAS_TENANT_COLUMN . "` = '" . $tenant_id . "'", $dsn, false, false, null, true);
                ...
                $_query = "UPDATE `" . fq_saas_tenant_db_prefix($tenant_id, 'leads_email_integration') . "` SET `id` = '1' WHERE `id` != '1' LIMIT 1;";
                fq_saas_raw_query($_query, $dsn, false, false, null, true, false);
            }
            ...
            if ($has_master === false)
                foreach ($tables as $k => $row) {
                    ...
                    fq_saas_raw_query("DROP TABLE $table", $dsn, false, false, null, true, false);
                }
        }
